...
//				_asm int 3;
	...
	static unsigned long runtimes = 0;
	runtimes++;
	unsigned long AgentUserCount = g_pServerTable->ShowServerStatus();
	unsigned long isServerAvailable = g_pServerTable->isServerAvailable();
	if(runtimes >= 1) {
		runtimes = 0;
		RETCODE retCode;
		...
		extern char LogServerName[256];
		sprintf(szQuery, "update server set usercount = %d, serveravailable = %d where servername = '%s'", AgentUserCount, isServerAvailable, LogServerName);
