...
unsigned long CServerTable::isServerAvailable() {
	unsigned long AvailableAgentServer =0;
	unsigned long AvailableRajaDB = 0;
	LP_SERVER_DATA pCur = this->GetServerListHead();
	while(pCur) {
		if(pCur->dwStatus != STATUS_ACTIVATED)
			break;
		switch(pCur->dwServerType) {
		case SERVER_TYPE_AGENT:
			AvailableAgentServer++;
			break;
		case SERVER_TYPE_DB:
			AvailableRajaDB++;
		default:
	...
	if((AvailableAgentServer > 0) && (AvailableRajaDB > 0))
		return 1;
	else
		return 0;
}

unsigned long CServerTable::ShowServerStatus()
{
	unsigned long AgentUserCount = 0;
	...
	while( pCur )
	{
		switch(pCur->dwServerType) {
		case SERVER_TYPE_AGENT:
			AgentUserCount += pCur->dwNumOfUsers;
			break;
		default:
			break;
		}
...
void CServerTable::ShowListenerStatus()
{
	...
	while (cur)
	{
		...
		MyLog( LOG_JUST_DISPLAY, "[%2d] SERVER_TYPE_RMLISTENER / %15s / %5d /  (○)"      //Ｘ○
	...
	while (cur)
	{
		...
		MyLog( LOG_JUST_DISPLAY, "[%2d] SERVER_TYPE_RMLISTENER / %15s / %5d /  (Ｘ)     <<Disconnected!!>>"   
			, ncount , cur->szIP , PROXY_SERVER_CONNECTION_PORT); 
