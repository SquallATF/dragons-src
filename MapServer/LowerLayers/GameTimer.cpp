...
void very_long_terms()	// 1000 ms (1초)
{
...
// 1당 150 ms
	GameTimerList[0].SetGameTimer("Very Short Terms",	1, false,	100, very_short_terms	);	//100ms 초당 12-14번
	GameTimerList[1].SetGameTimer("Short Terms",		2, false,	300, short_terms		);	//300ms 초당 6번
	GameTimerList[2].SetGameTimer("Normal Terms",		3, false,	450, normal_terms		);	//450ms 초당 4번
	GameTimerList[3].SetGameTimer("Long Terms",			4, false,	600, long_terms			);	//600ms 초당 4,2,4,2,4 (패턴)
	GameTimerList[4].SetGameTimer("Very Long Terms",	7, false,	1000, very_long_terms	);	//1000ms 초당 1번
	GameTimerList[5].SetGameTimer("Quest30Second",		10, false, 	1000, QuestSecond	);		//100ms 10초에 7번정도(조정flag가 존재하는것으로 추정) => 1000ms로 수정 by KCH
