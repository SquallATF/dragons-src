...
void __stdcall orgReceivedMsgFromServer(DWORD dwConnectionIndex, char* pMsg,DWORD dwLength);
...
void __stdcall ReceivedMsgFromServer(DWORD dwConnectionIndex, char* pMsg,DWORD dwLength) {
	...
	__try {
		orgReceivedMsgFromServer(dwConnectionIndex, pMsg, dwLength);
	}
	//__except(pException = GetExceptionInformation())
	__except (DumpException(GetExceptionInformation(), "Exception Raised on HadleCommand()", pMsg, dwLength))
	{
		//acer7
	...
	case PTCL_MAP_TO_MAP:
		{
			...
			if( (dwLength-1) != sizeof(t_header)+packet->h.header.size ) {
				MyLog( LOG_FATAL, "PTCL_MAP_TO_MAP :: Invalid Packet Size(%d != %d), Packtype = %d", dwLength-1, sizeof(t_header)+packet->h.header.size, packet->h.header.type);
				break;
			}
			...
			case CMD_MAP_CHANGE_REQUEST:
				{
					...
					else 
					{
						...
						int i = 0;
						for(; i < 1000 ; i ++)	if( tch->Item[i] == 0) break;
						tch->ItemMax = i;
						//----------------------------------------------------
