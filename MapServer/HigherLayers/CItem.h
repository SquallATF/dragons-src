...
enum IA2_TYPE
{
	...
	IA2_ITEMMALL_ITEM = 0x02000000,		// 040601 YGI
...
//<050224_KCH 마일리지몰 작업
const int IATTR_ITEM_MALL_IDX = IATTR_RESERVED0;	//TotalDB의 Item_to_game의 idx값으로 Limit중복값을 구별하기 위해 사용(복템구별).
const int IATTR_ITEM_MALL_EndUseTime = IATTR_RESERVED1;	//마일리지 아이템의 기간제한을 표시
...
#define		RESET_DUAL_TO_CC	DUAL_CHANGE_ITEM		// add by taniey
...
public:
	virtual int Getmouse_Mr_Wave() { return Mouse_Mr_Wave; }		// 040621 YGI
	virtual int getAllData(void);
	virtual void print_data(void) {
		printf("\n%d %s %s %d", Item_id, Name, Han_Name, lv);
	}
...
extern _eRT_INV_ITEMMALL SearchInvForItemMall(CHARLIST *pCh, int &a, int &b, int &c, const ItemAttr _UserItem);	//050224_KCH 마일리지몰 작업
