...
//< 040712-kjy 국지전 일스 처리 삭제하기 위한 전역 변수 추가
...
//	char	MapServerConfigFileName[MAX_PATH];
	...
	//< 040712-kjy 국지전 일스 처리 삭제하기 위한 전역 변수 데이타 입력
...
/*	if (GetPrivateProfileString("Network","path","",NetworkDir,MAX_PATH, MAP_SERVER_INI_)<=0)
	...
	////////////////////////////////////////////////////////
	// 040623 YGI
	if (GetPrivateProfileString("option","max_time_guild_item_menu_level","", tempString2, MAX_PATH, MapServerConfigFileName) > 0 )
	{

		MAX_TIME_USE_GUILD_ITEM_MENU  = atoi(tempString2);
		Dbg_Assert( MAX_TIME_USE_GUILD_ITEM_MENU  > 0 );
		MyLog( 0, " MAX_TIME_USE_GUILD_ITEM_MENU  = %d", MAX_TIME_USE_GUILD_ITEM_MENU );
	}
	else
	{
		MAX_TIME_USE_GUILD_ITEM_MENU  = 30;
	}
	////////////////////////////////////////////
...
void RecvCMD_INC_USER_LOCALWARFIELD(t_packet *p, t_connection c[], int cn )
{
	...
	*/
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR,"참여인원 변동 __ 번호 : %d, 국적 : %d , 인원 : %d",
...
void CalcGainedFame(CHARLIST* ch)// LTS DUAL FAME
{
	...
	else 
	{
		if (ch->GainedFame>0)
		{
			g_pLogManager->SaveLogChange_Fame(ch,ch->fame,ch->fame+ch->GainedFame,LF_LOCALWAR);		// 020909 LTS
...
VOID SaveNationRankPoint(int aPoint[NW_NATION_COUNT])
{
	...
	wsprintf(szQuery, "EXEC up_get_nation_rank_point %d", SUM_TYPE_NUM);
	...
	{
		::MyLog(LOG_FATAL,"SaveNationRankPoint, up_get_nation_rank_point, WarStartUpTBL Table Query Error!!");
	...
	if(retCode != SQL_SUCCESS && retCode != SQL_SUCCESS_WITH_INFO)
	{
		::MyLog(LOG_FATAL,"SaveNationRankPoint, up_get_nation_rank_point, WarStartUpTBL Table Fetch Error!!");
		SQLFreeStmt(hStmt,SQL_DROP);
		return;
	}

	int nSumPoint[NW_NATION_COUNT] = {0,};
	int nI;
	...
	if(retCode != SQL_SUCCESS && retCode != SQL_SUCCESS_WITH_INFO)
	{
		::MyLog(LOG_FATAL,"SaveNationRankPoint, up_get_nation_rank_point, WarStartUpTBL Table SQL Return Error!!(%d)", retCode);
		SQLFreeStmt(hStmt,SQL_DROP);
		return;
	}
	...
	//< LTH-040314-KO 국지전 포인트 순위 점수를 로그로 남기자구!!
	/*040721_KJHuNs g_pLogManager로 이동(정리목적)
	...
	*/
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR,"<!-- Nation Rank Point Information -->");
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR,"** Previous Sum Nation Rank Point\t- VY : %d, ZY : %d, YL : %d **", \
					nSumPoint[0], nSumPoint[1], nSumPoint[2]);
	
	for (nI = 0; nI < NW_NATION_COUNT; ++nI)
	{
		nSumPoint[nI] += aPoint[nI];
	}
	//> LTH-040314-KO

	// Update Point
	//< LTH-040413-KO 값이 이상하게 들어간다 -_-; 다시 한번 꼭 확인!!!
	struct tm *today;
	time_t lTime;
	int wday;
	...
	switch (wday)
	{
	case 0: sprintf(szTemp, "Sunday"); break;
	case 1: sprintf(szTemp, "Monday"); break;
	case 2: sprintf(szTemp, "Tuesday"); break;
	case 3: sprintf(szTemp, "Wednesday"); break;
	case 4: sprintf(szTemp, "Thursday"); break;
	...
	*/
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR, "** Plus Nation Rank Point\t- %s VY : %d, ZY : %d, YL : %d **", 
					szTemp, aPoint[0], aPoint[1], aPoint[2]);
	//> LTH-040409-KO
	...
	if(retCode != SQL_SUCCESS && retCode != SQL_SUCCESS_WITH_INFO)
	{
		::MyLog(LOG_FATAL,"SaveNationRankPoint, up_set_nation_rank_point_today, WarStartUpTBL Table Query Error!!");
		SQLFreeStmt(hStmt,SQL_DROP);
		return;
	}
	...
	*/
	...
	if(retCode != SQL_SUCCESS && retCode != SQL_SUCCESS_WITH_INFO)
	{
		::MyLog(LOG_FATAL,"SaveNationRankPoint, up_set_nation_rank_point, WarStartUpTBL Table Query Error!!");
		SQLFreeStmt(hStmt,SQL_DROP);
		return;
	}
...
{	//const NationTBL[3]={ 3,4,6 };
	...
	//< LTH-040220-KO 순위에 따른 점수를 세 국가에 할당한다.
...
void CheckEventLocalWarDeath(CHARLIST* Attacker,CHARLIST* Defencer)
{
	...
	//< LTH-040729-CN. 이벤트 국지전 수정. 이벤트 포인트를 MapServerConfig.ini에서 읽어오도록 한다. 점수 일률적으로 줌.
...
: m_nEventPoint(0)			// LTH-040729-CN 이벤트 국지전 수정.
{
...
//	char	MapServerConfigFileName[MAX_PATH];
	...
	//< LTH-040729-CN 이벤트 국지전 수정. 중국만 해당.
	if (LocalMgr.IsAbleNation(CHINA))
	{
		m_nEventPoint = GetPrivateProfileInt("EventLocalWar", "EventPoint", 1, MapServerConfigFileName);
		if (m_nEventPoint == 0)
		{
			::MyLog(LOG_FATAL, "%s의 [EventLocalWar], EventPoint 설정이 잘못되어 있습니다.", MapServerConfigFileName);
			m_CheckLoadData=0;
			return;
		}
	}
...
void RecvCMD_EVENT_LOCALWAR_JOIN(t_packet *p, t_connection c[], int cn )
{
	...
	{
		//< LTH-040729-CN. 이벤트 국지전 수정. 중국일 경우 이벤트 조건에만 맞으면 이벤트 진행.
		if (LocalMgr.IsAbleNation(CHINA))
		{
			packet.h.header.type=CMD_EVENT_LOCALWAR_JOIN;
			packet.u.LocalWar.EventJoin.Nation=c[cn].chrlst.name_status.nation;
			packet.u.LocalWar.EventJoin.Level=c[cn].chrlst.GetLevel();
			packet.h.header.size=sizeof(t_EventJoin);
			SendCMD_REQUEST_LOCAL_DELIVERY(g_EventMapPort,&packet,c,cn);
		}
		else
...
/*040719_KJHuNs g_pLogManager 로 이동.
...
void SaveLogChange_DualFame(CHARLIST* pTarget, const int nOldDualFame, const int nNewDualFame, eLDF_TYPE type)
{	//< CSD-040224
}	//> CSD-040224
void SaveLogChange_Fame(CHARLIST* pTarget, int old_fame, int new_fame, eLF_TYPE type)
{	//< CSD-040224
}	//> CSD-040224
void SaveLogScriptLog(CHARLIST* pTarget, const int nQuestNo, const int nQuestStep, const char* msg) //kyo
{	//< CSD-040224
}	//> CSD-040224
*/
...
{	//< CSD-040310
	...
	case CMD_REQUEST_CHECK_DUAL_FAME:
		{	// LTS DUAL FAME
...
void SaveLogChange_DividendMoney(CHARLIST* pTarget, DWORD dwOldMoney, DWORD dwNewMoney, bool bBank)
{	//< CSD-TW-030624
}	//> CSD-TW-030624
// 030506 YGI
