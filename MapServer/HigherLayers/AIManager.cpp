...
int CAIManager::Load_AI_Data()
{
	...
	//< LTH-040505-KO 종료시에 에러가 난다. m_pNpc_AI_Data[0]이 할당되지 않고 참조하려한다. 
	// NULL이 아닌 쓰레기주소로 설정된다
	m_iAI_DataCount = 0;
...
int CAIManager::LoadMagicBag()
{
	...
	//< LTH-040305-KO 종료시에 에러가 난다. m_pMagicBag[0]이 할당되지 않고 종료시에 제거하려한다. 
	// NULL이 아닌 쓰레기주소로 설정된다
	m_iMagicBagCount = 0;
...
int CAIManager::LoadBossData()
{
	...
	//< LTH-040505-KO 종료시에 에러가 난다. m_pBossStatusData[0]이 할당되지 않고 참조하려한다. 
	// NULL이 아닌 쓰레기주소로 설정된다
	m_iBossStatusCount = 0;
...
int CAIManager::LoadAttackBag()
{
	...
	//< LTH-040505-KO 종료시에 에러가 난다. m_pAttackBag[0]이 할당되지 않고 참조하려한다. 
	// NULL이 아닌 쓰레기주소로 설정된다
	m_iAttackBagCount = 0;
...
int CAIManager::LoadActionBag()
{
	...
	//< LTH-040505-KO 종료시에 에러가 난다. m_pActionBag[0]이 할당되지 않고 참조하려한다. 
	// NULL이 아닌 쓰레기주소로 설정된다
	m_iActionBagCount = 0;
...
int** CAIManager::GetNpcAIData()
{
	return m_pNpc_AI_Data;
}

int* CAIManager::GetNpcAIData(int AI_Type)
{
	return m_pNpc_AI_Data[AI_Type];
}

int CAIManager::GetNpcAIData(int AI_Type,int Field)
{
...
void CAIManager::SetAI(CHARLIST* Npc,int AIPattern)			// LTS DRAGON MODIFY
{
	if (!Npc->ChairNum) return;
...
void CAIManager::SetTarget(CHARLIST* Npc,int Target)
{
	if (Target!=-1)
	{
		Npc->targetid=Target;
...
inline int CAIManager::GetAttackBagNo(CHARLIST*Npc)
{
	...
	//< LTH-040506-KO 이놈 0 이면 안된다.. -_-;
	int nPhase = Npc->CurrentPhase;	
	if ( nPhase <= 0 )
		nPhase = 1;

	return m_pBossStatusData[nPhase][AttackMethod];
	//> LTH-040506-KO
