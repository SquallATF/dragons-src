...
//<050425_KCH (메모리 해킹)에서 Item 내구도 감소로 깨질때 캐릭에 적용된 데미지가 감소되지 않는 버그 유발됨
// 기존의 DecItemDur는 여러가지로 혼란스럽게 사용되고 있음, 그래서 착용아이템의 내구도 감소함수만 따로 뺌.
int CItemMgr::DecItemDur_ForEquip(ITEMATTR &item, const DWORD iValue)//양수로 넣을것 음수로 넣으면 내구도 증가
{
	CItem* t = ConvertItemNoToCItem(item.item_no);
	if(!t){return 0;}
	...
	else
	{
		...
		if(wNow <= iValue)//뺄 값이 남은거 보다 많다면
		{
			//<050425_KCH 이부분이 일반내구도 감소와 다르게 적용. 
			//착용아이템은 리턴값에 따라서 해당아이템정보를 토대로 캐릭에 적용된 뎀쥐를 먼저빼고, 아템삭제처리
			// 아템삭제후, 캐릭적용된 뎀쥐는 Skip되는 버그.
			//DeleteItem( item );
			//>050425_KCH
			return 1;
		}//빼도 되는 거라면
