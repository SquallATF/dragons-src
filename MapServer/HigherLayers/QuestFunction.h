...
public:
	...
	CSpellMove*	FindSpellUser(const int cn);				//유저 찾기
...
//Counter에 관련된 내용.
	...
	CScriptCounter* FindUserInScriptCounter(const int& iId, const char* name, const int iType);	//해당 유저가 해당타입의 카운터를 실행하고 있는가?
...
//Npc를 죽이며 받게 되는 보상 requital_list에 저장되어 있는 값을 준다. 
	...
	table_requital_list* FindRequitalNpc(const int iNpcIndex);	//해당 npc가 테이블에 존제하는가?
...
public:
	...
	table_requital_Item* GetRequital_ItemIttr(const int index);
	//> BBD 040329

	//REQUITALLISTITER	GetRequital_KillNpc(const int index);			//npc를 죽였을때 주는 해택의 종류	
	table_requital_list* GetRequital_KillNpc(const int index);			//npc를 죽였을때 주는 해택의 종류	
};
