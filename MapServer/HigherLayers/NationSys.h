...
//	bool			LoadNationWar_Exp();							// LTS NEW LOCALWAR
  ...
  void      WeaponDataList();   // 디버그용
	...
	void SendUserGoOutsideBBS(int nMsg);			// LTH-040507-KO
	void UserGoOutside();							// LTH-040512-KO
	void ClearAllGuard();							// LTH-040513-KO
...
class cNation
{
	...
	DWORD						m_dwWarRemainTime[WI_MAX_WARFIELD];	// LTH-040212-KO 각 전쟁 남은 시간
...
public :
	...
	void			GetDayofWeek();
	VOID NeoCheckAndActive(INT nIndex);	// LTH-040206-KO 새 전쟁터중에 설원전쟁터를 제외하고 적용
...
public:
	...
	VOID CheckWinNationPoint(INT aPoint[NW_NATION_COUNT]);	// LTH-040219-KO
...
{
protected:	// LTH-040216-KO 상속 받기 위해
	int				m_iWarfieldNo;
	int				m_iTeamNo;
