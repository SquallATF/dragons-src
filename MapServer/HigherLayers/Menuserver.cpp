...
ok__:
	...
	//soto-tai
	//p.h.header.size = sizeof(kein_server_other_ch_inv) - sizeof( short int )*(15-count);
...
void CheckInitSkill(CHARLIST *ch)		 // 클레스 스킬은 확실하게...	// 어느정도 들어가면 빼야 하는 루틴...
{
	...
	{
	case J_TAILOR:
		for (i = 0; i < MAX_SKILLMAIN; i++) if (SkillTbl[i].Select_Tailoring == 1)
		{
			ch->Skill[i] = 1; makemyjobexpsetting(i, ch);
		}
		break;
	case J_ALCHEMIST:
		for (i = 0; i < MAX_SKILLMAIN; i++)
	...
	for (i = 0; i < MAX_SKILLMAIN; i++)
	{
		if (SkillTbl[i].Howto == 100)
		{
			...
			case 110:
				...
				break;
			case 111:
				if (ch->skillexp[HERBDIGGING].skillexp == 1000000
					&& ch->skillexp[ALCHEMING].skillexp == 1000000) ok = 1;

				break;
			case 112:
				if (ch->skillexp[FISHING].skillexp == 1000000
