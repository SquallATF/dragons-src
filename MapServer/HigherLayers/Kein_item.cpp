...
int GetTotalItemWeight(CHARLIST *ch)
{
	...
	for (a = 0; a < 6; a++)
		if (ch->quick[a].item_no)
			weight += GetWeight(ch->quick[a]);

	for (a = 0; a < 8; a++)
		if (ch->equip[a].item_no)
			weight += GetWeight(ch->equip[a]);

	if (ch->handheld.item_no)
		weight += GetWeight(ch->handheld);
...
int GetItemView(int item_no, int is_man)
{
	...
	return !t ? 0 : t->GetItemShow(is_man);
	//if( !t )
	//	return 0;
	//else
	//	return t->GetItemShow( is_man );
}
/////////////////////////////////// //0101 //////////////////////////////////////
//////////////////////////// Item POS Set /////////////////////////////////
...
{
	//<050224_KCH 마일리지몰 작업	마일리지 아이템은 맵에 떨어뜨리지 못한다.
