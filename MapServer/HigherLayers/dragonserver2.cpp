...
int CheckHandleByKein( t_packet *p, t_connection c[], int cn )
{
	...
	//< LTH-040810-KO 1.04p 감옥 시스템
	case CMD_BAD_USER_IMPRISON:
		g_cpPrison->RecvBadUserImprison(p, cn);
		break;
	case CMD_BAD_USER_RELEASE:
		g_cpPrison->RecvBadUserRelease(p, cn);
		break;
	case CMD_UPDATE_IMPRISON_TERM:
		g_cpPrison->RecvUpdateImprisonTerm(cn);
		break;
	case CMD_REQUEST_MOVE_TO_PRISON:
		{
			int nNation = p->u.kein.tagRequestMoveToPrison.nNation;
			::EatRearWhiteChar(p->u.kein.tagRequestMoveToPrison.szUserId);
			//::EatRearWhiteChar(p->u.kein.tagRequestMoveToPrison.szCharName);
			g_cpPrison->MoveToPrison(p->u.kein.tagRequestMoveToPrison.szUserId, nNation);
		}
		break;
	case CMD_REQUEST_MOVE_TO_VILLAGE:
		{
			int nNation = p->u.kein.tagRequestMoveToPrison.nNation;
			::EatRearWhiteChar(p->u.kein.tagRequestMoveToPrison.szUserId);
			//::EatRearWhiteChar(p->u.kein.tagRequestMoveToPrison.szCharName);
			g_cpPrison->MoveToVillage(p->u.kein.tagRequestMoveToPrison.szUserId, nNation);
		}
		break;
	//> LTH-040810-KO
	default:	
...
void SetMenuFunctionsAll()
{
	...
	// 040601 YGI
	SetMenuFunctions( CMD_SEND_ITEM_MALL_ITEM_LIST, RecvItemMallItemList );
