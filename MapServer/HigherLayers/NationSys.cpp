...
LPWARFIELDINFO	g_pcWarfieldInfo;					// LTH-040206-KO 관리 맵서버도 아니고 전쟁터 맵서버도 아닐때 사용
int				g_naWarfieldState[WI_MAX_WARFIELD];	// LTH-040206-KO 각 전쟁터 상황
int				g_aJoinNation[NW_NATION_COUNT];			// LTH-040309-KO 전쟁 참여 가능국가.
CWarfieldMgr*	g_pcWarfieldMgr = NULL;				// LTH-040210-KO 전쟁터 관리자. 관리서버가 아니다. 각 전쟁터로 통하는 하나의 통로
//< LTH-040206-KO



// Tile Concern__________________________________________________________________________________	
const POINTS Map0_133[MAP0_133] =
{	// 010927 LTS
...
const POINTS Map0_268[MAP0_268] =
{	// 010927 LTS
...
const POINTS Map2_E279[8] =
{
	{276,281},{277,280},{278,279},{279,278},{276,280},{277,279},
	{278,278},{279,277}
};
// Tile Concern End_______________________________________________________________________________

const POINTS AttackPoint[3][4] =
{
	{ {321,338}, {182,357}, {382,169}, {382,169} },		// Nation1
	{ {629,629}, {425,657}, {667,445}, {667,445} },		// Nation2
	{ {415,407}, {388,303}, { 11, 11}, { 16,126} }
};	// Nation3
const POINTS DefencePoint[3] =
{
	{127,120},		// Nation1
	{449,456},		// Nation2
	{214,312},
...
void InitNationValues()
{
	...
	else
	{
		...
		NW_SQUAD2_LEVEL_MAX = 90;
		NW_SQUAD3_LEVEL_MIN = 80;
		NW_SQUAD3_LEVEL_MAX = 120;
		NW_SQUAD4_LEVEL_MIN = 110;
		NW_SQUAD4_LEVEL_MAX = 150;
		//> LTH-040421-KO	
...
//< LTH-040207-KO 1.4 패치 이후 새로운 전쟁터인가?
...
bool CheckWarLoopProcess()		// 전쟁 루프가 진행되는지 확인한다.
{
	if (isNationManageServer())
	{
		...
		if (g_pNation->GetNewWarfieldStatus(WI_HADES_WARFIELD) > NW_PEACE) return TRUE; // LTH-040207-KO
	}
	else
	{
		...
		if (g_naWarfieldState[WI_HADES_WARFIELD] > NW_PEACE) return TRUE;	// LTH-040206-KO
	}
	return false;
}
...
int ReturnWarfieldNoByWarLoopProcess()		// CheckWarLoopProcess와 쌍으로 쓰인다.. 포함해도 된다..
{
	...
	if (isNationManageServer())
	{
		if (g_pNation->GetWarfieldStatus(0) > NW_PEACE)
		{
			nWarfieldNo = 0;
			return nWarfieldNo;
		}
		if (g_pNation->GetWarfieldStatus(1) > NW_PEACE)
		{
			nWarfieldNo = 1;
			return nWarfieldNo;
		}
		if (g_pNation->GetWarfieldStatus(2) > NW_PEACE)
		{
			nWarfieldNo = 2;
			return nWarfieldNo;
		}
		if (g_pNation->GetNewWarfieldStatus(0) > NW_PEACE)
		{
			nWarfieldNo = 3;
			return nWarfieldNo;
		}
		if (g_pNation->GetNewWarfieldStatus(WI_HADES_WARFIELD) > NW_PEACE)
		{
	...
	{
		if (g_pWarfieldStatus[0].Status > NW_PEACE)
		{
			nWarfieldNo = 0;
			return nWarfieldNo;
		}
		if (g_pWarfieldStatus[1].Status > NW_PEACE)
		{
			nWarfieldNo = 1;
			return nWarfieldNo;
		}
		if (g_pWarfieldStatus[2].Status > NW_PEACE)
		{
			nWarfieldNo = 2;
			return nWarfieldNo;
		}
		if (g_pNewWarfieldStatus[0] > NW_PEACE)
		{
			nWarfieldNo = 3;
			return nWarfieldNo;
		}
...
bool IsWar()
{
	...
	{	//< LTH-040528-KO 생성되지 않은 포인터 변수를 참조하지 못하도록!!
		if (NULL != g_pWarfield)
		{
	...
	//< LTH-040209-KO 1.4 패치 이후 새전쟁터는 매니저에게 질의를 통해 값을 얻는다
...
bool CanDestroyTarget(CHARLIST* pCaster, CHARLIST* pTarget)		// CAttackMagic::Result
{
	...
	//< LTH-040210-KO 1.4 패치 이후 새전쟁터이면
	if (IsNeoWarfieldServer())
	{
		tagOpponent tagOpp;
...
void UpdateWarTime()			// 011015 LTS 
{
	...
	//> LTH-040206-KO Elpased Time from this Sunday to current day
	g_dwCurrWeekElapsedSec = (today->tm_wday * 86400)
		+ (today->tm_hour * 3600)
		+ (today->tm_min * 60)
		+ (today->tm_sec);
	//> LTH-040206-KO
...
bool CanGoBonus()
{
	...
	//< LTH-040210-KO 1.4 패치 이후 새전쟁터는 매니저에게 질의를 통해 값을 얻는다
	// 상태를 얻어오는 메시지 CWarfieldMgr::WPM_GET_WARFIELD_STATE를 보내어 전달받는다.
	if (IsNeoWarfieldServer())
	{
		INT nStatus = 0;
...
bool InitNationSystem()
{
	...
	//< LTH-040206-KO 1.4 Patch이후 전쟁터 정보 전달자 클래스 생성.
	g_pcWarfieldInfo = new CWarfieldInfo;
	g_pcWarfieldInfo->Init(WI_MAX_WARFIELD);	// 맵이동 레벨 제한이나 부대제한 정보 로딩
	//> LTH-040206-KO
	...
	//< LTH-040325-KO 1.4 패치 이후 새 전쟁터를 생성한다
	if (IsNeoWarfieldServer())
	{
		if (NULL == g_pcWarfieldMgr)
		{
...
bool UpdateNationWarData()
{
	...
	if (isNewWarfieldServer())
	{
		if (g_pNewWarfield == NULL)
		{
			...
			if (g_pNewWarfield == NULL)
			{
				JustMsg("g_pNewWarfield Not Inialized..!!(Port:%d)", g_wMapServerPort);
				return false;
			}
			if (!g_pNewWarfield->InitWarfield())
			{
...
void CloseNationSystem()		// LTS 대만//020205 lsw
{
	...
	SAFE_DELETE(g_pcWarfieldInfo);	// LTH-040206-KO 전쟁터 정보 전달자 제거
	SAFE_DELETE(g_pcWarfieldMgr);	// LTH-040210-KO 전쟁터 관리자. 관리서버가 아니다.
...
//< LTH-040209-KO 현재 부대의 인원을 카운트 해라...
void SendCMD_CHECK_WARFIELD_SQUAD_MEMBERCOUNT(t_packet *p, t_connection c[], const int cn)
{
	t_packet packet;
	packet.h.header.type = CMD_CHECK_WARFIELD_SQUAD_MEMBERCOUNT;
	packet.u.NationWar.CheckSquadMemberCount.cSquad = c[cn].chrlst.NWCharacter.SquadNo;
	packet.u.NationWar.CheckSquadMemberCount.cWarfieldNo = p->u.NationWar.NWMapMove.WarfieldNo;
	packet.h.header.size = sizeof(t_CheckSquadMemberCount);

	INT nWarfieldNo = p->u.NationWar.NWMapMove.WarfieldNo;
	SendCMD_REQUEST_DELIVERY(WP_BASE_PORT + nWarfieldNo - 1, &packet, c, cn);
}
//> LTH-040209-KO

//< LTH-040313-KO 몬스터 국가전 참여국가 판단
...
void RecvCMD_NW_MAP_MOVE(t_packet *p, t_connection c[], const int cn)
{
	...
	if (isNationManageServer())
	{
		...
		{
			//< LTH-040207-KO 지하 전쟁터의 추가로 설원전쟁터를 따로 처리
	...
	else
	{
		...
		else if (WarfieldNo >= 3)
		{
			...
			//else
			//
			//< LTH-040206-KO 지하 전쟁터 추가로 설원 전쟁터를 따로 처리
			if (IsNeoWarfield(WarfieldNo))
	...
	if (WarfieldNo >= 0 && WarfieldNo <= 2)
	{
		...
		if (WarfieldNo >= 3)
		{
			//< LTH-040206-KO ch->GetLevel()의 잦은 호출을 막기 위해 변수로 대체했다
			INT nCharacterLv = ch->GetLevel();
			if (IsNeoWarfield(WarfieldNo))
			{
				// 새전쟁터중 설원전쟁터가 아니면 정보 전달자로 부터 제한사항을 받는다
				//< LTH-040226-KO 맵 이동 레벨제한
				...
				//< LTH-040309-KO 전쟁 참여 가능국가만 이동을 허락한다.
				...
				//< LTH-040226-KO 지하전쟁터는 자동으로 부대가 지정된다
				...
				//< LTH-040226-KO 1.4 지하전쟁터는 국가인원이 아닌 부대 인원이 제한이다
			...
			{
				if (nCharacterLv < MoveLevelMin[WarfieldNo - 3] || nCharacterLv > MoveLevelMax[WarfieldNo - 3])
				{	//< CSD-030806
					SendCMD_NW_MAP_MOVE_FAIL(c, cn, 1);
					return;
				}	//> CSD-030806
...
void CallNWMapMove(const int WarfieldNo, t_connection c[], const int cn)  //클라이언트의 인터페이스 맵이동 // 011217 LTS
{
	...
	else
	{
		if (WarfieldNo >= 0 && WarfieldNo <= 2)
			...
			if (WarfieldNo >= 3)
			{
				...
				//else
				//
				//< LTH-040206-KO 지하 전쟁터의 추가로 설원전쟁터 따로 처리
				if (IsNeoWarfield(WarfieldNo))
					WarfieldStatus = g_naWarfieldState[WarfieldNo];
				else
					WarfieldStatus = g_pNewWarfieldStatus[WarfieldNo - 3];
	...
	if (WarfieldNo == WI_HADES_WARFIELD) sprintf(tempMapName, "Nation5");	// LTH-040207-KO
	...
	if (WarfieldStatus != NW_WAR)
	{
		if (WarfieldNo >= 3)			// 평화시 어떻게 하지??? ㅡ.ㅡ
		{
			...
			//}
			//< LTH-040225-KO 1.4 패치 이후엔 전쟁상황이 아니면 다음 메시지를 띄운다
			if (IsNeoWarfield(WarfieldNo))
	...
	else
	{
		if (WarfieldNo >= 3)
		{
			//< LTH-040213-KO 신규전쟁터중 설원전쟁터만 따로 처리
			if (IsNeoWarfield(WarfieldNo))
			{
 ...
 * @brief	전쟁 관리 서버에게 전쟁터 소유국가와 전쟁터 상태를 요청한다.
 * @param	const int nWarfieldNo. 상태를 요청할 전쟁터 번호
 * @param	const WORD wPort. 요청한 맵서버 포트
 * @return	void.
 */
 //< LTH-040504-KO.
void SendCMD_WARFIELD_INFO(const int nWarfieldNo, const WORD wPort, const int nUserID)
{
	t_packet packet;
	packet.h.header.type = CMD_WARFIELD_INFO;
	packet.u.NationWar.WarfieldInfoToManager.nWarfieldNo = (BYTE)nWarfieldNo;
	packet.u.NationWar.WarfieldInfoToManager.nUserID = nUserID;
	packet.u.NationWar.WarfieldInfoToManager.wPort = wPort;
	packet.h.header.size = sizeof(tagWarfieldInfo);
	::SendPacket2NWManager(&packet);
}	//> LTH-040504-KO.

/**
 * @fn		RecvCMD_WARFIELD_INFO().
 * @brief	관리서버에서 전쟁터 소유자와 상태를 물어본 녀석에게 전송해준당.
 ...
 */
 //< LTH-040504-KO.
void RecvCMD_WARFIELD_INFO(t_packet* p, t_connection c[], const int cn)
{
	if (!isNationManageServer())
		return;

	int nWarfieldNo = p->u.NationWar.WarfieldInfoToManager.nWarfieldNo;
	int nWarfieldPossession = g_pNation->GetWarfieldNationCode(nWarfieldNo);
	int nWarfieldStatus = -1;

	if ((nWarfieldNo >= WI_MOUNTAIN_WARFIELD) && (nWarfieldNo <= WI_ISLAND_WARFIELD))
		nWarfieldStatus = g_pNation->GetWarfieldStatus(nWarfieldNo);

	t_packet packet;
	packet.h.header.type = CMD_WARFIELD_INFO_RESULT;
	packet.u.NationWar.WarfieldInfoFromManager.nWarfieldNo = nWarfieldNo;
	packet.u.NationWar.WarfieldInfoFromManager.nUserID = p->u.NationWar.WarfieldInfoToManager.nUserID;
	packet.u.NationWar.WarfieldInfoFromManager.nWarfieldPossession = nWarfieldPossession;
	packet.u.NationWar.WarfieldInfoFromManager.nWarfieldStatus = nWarfieldStatus;
	packet.h.header.size = sizeof(tagWarfieldInfoResult);
	WORD wPort = p->u.NationWar.WarfieldInfoToManager.wPort;

	::SendPacket2NWMap(wPort, &packet);
}	//> LTH-040504-KO.


/**
 * @fn		ProcessCMD_NW_MAP_PEACE_MOVE().
 * @brief	평화 기간에 전쟁터 이동 요청시에 처리하는 함수.
 * @param	t_packet* tagReceivePacket. 받은 패킷.
 * @param	t_packet* tagReturnPacket. 반환할 패킷.
 * @return	void.
 */
 //< LTH-040503-KO.
void ProcessCMD_NW_MAP_PEACE_MOVE(t_packet* tagReceivePacket, t_packet* tagReturnPacket)
{
	int nCn = tagReceivePacket->u.NationWar.WarfieldInfoFromManager.nUserID;
	LPCHARLIST lpChar = ::CheckServerId(nCn);
	if (NULL == lpChar)
		return;

	int nWarfieldNo = tagReceivePacket->u.NationWar.WarfieldInfoFromManager.nWarfieldNo;
	int nNation = lpChar->name_status.nation;
	int nWarfieldPossession = tagReceivePacket->u.NationWar.WarfieldInfoFromManager.nWarfieldPossession;
	int nWarfieldStatus = tagReceivePacket->u.NationWar.WarfieldInfoFromManager.nWarfieldStatus;

	tagReturnPacket->h.header.type = CMD_NW_MAP_PEACE_MOVE_RESULT;
	tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nResult = NPM_MAX;
	tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nWarfieldNo = -1;
	...
	if (g_wMapServerPort != BASE_WARFIELD_PORT + nWarfieldNo)
	{
		...
		if (NW_PEACE == nWarfieldStatus)
		{
			...
			else if (nNation == NW_YL)	// 일스 국가 유저인가
			{
				// 은행에 돈이 있는가? 있다면 지불!!
				...
				{
					tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nResult = NPM_FAIL_NOT_BANK_MONEY;
					tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.dwYlMoveMoney = dwMoveMoney;
					tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nWarfieldNo = nWarfieldNo;
				}
			}
			else
			{
				tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nResult = NPM_FAIL_NO_POSSESSION;
				tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nWarfieldNo = nWarfieldNo;
				tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nPossessionNation = nWarfieldPossession;
			}
		}
		else
		{
			tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nResult = NPM_FAIL_NOT_PEACE;
			tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nWarfieldNo = nWarfieldNo;
		}
	}
	else
	{
		tagReturnPacket->u.NationWar.WarfieldPeaceMoveR.nResult = NPM_FAIL_SAME_WARFIELD;
	}

	tagReturnPacket->h.header.size = sizeof(tagWarfieldPeaceMoveResult);
}
//> LTH-040503-KO.

/**
 * @fn		RecvCMD_WARFIELD_INFO_RESULT().
 * @brief	전쟁 관리서버로부터 전쟁터 소유국가와 상태를 전달 받아 처리.
 * @param	t_packet* p.
 * @param	t_connection c[].
 * @param	const int cn.
 * @return	void.
 */
 //< LTH-040504-KO.
void RecvCMD_WARFIELD_INFO_RESULT(t_packet* p, t_connection c[], const int cn)
{
	int nWarfieldNo = p->u.NationWar.WarfieldInfoFromManager.nWarfieldNo;
	int nCn = p->u.NationWar.WarfieldInfoFromManager.nUserID;

	char szTempMapName[10] = { 0, };
	//< LTH-040623-KO 구조적으로 바꿈
	switch (nWarfieldNo)
	{
	case WI_MOUNTAIN_WARFIELD:	sprintf(szTempMapName, "Nation1");	break;
	case WI_PLAIN_WARFIELD:		sprintf(szTempMapName, "Nation2");	break;
	case WI_ISLAND_WARFIELD:	sprintf(szTempMapName, "Nation3");	break;
	default:					return;
	}	//> LTH-040623-KO

	t_packet packet;
	::ProcessCMD_NW_MAP_PEACE_MOVE(p, &packet);
	QueuePacket(connections, nCn, &packet, 1);

	LPCHARLIST lpChar = ::CheckServerId(nCn);
	if (packet.u.NationWar.WarfieldPeaceMoveR.nResult == NPM_OK)
	{
		POINTS xyTempPoint = AttackPoint[nWarfieldNo][0];		// LTH-040817-KO 유저 로딩 위치를 공격자 위치로 변경
		::MapMove(nCn, szTempMapName, xyTempPoint.x, xyTempPoint.y);
	}
	else
	{
		//< LTH-0406-23-KO 로그 강화!!
		char szTempReason[128] = { 0, };
		switch (packet.u.NationWar.WarfieldPeaceMoveR.nResult)
		{
		case NPM_FAIL_NOT_PEACE:		sprintf(szTempReason, "Move failure to warfield in peace!!");	break;
		case NPM_FAIL_NO_POSSESSION:	sprintf(szTempReason, "Failure, Cuz they don't have battlefield!!");	break;
		case NPM_FAIL_NOT_BANK_MONEY:	sprintf(szTempReason, "Failure, Cuz U didn't save your money in the bank!!");	break;
		case NPM_FAIL_SAME_WARFIELD:	sprintf(szTempReason, "You're in warfield!!");	break;
		default:						sprintf(szTempReason, "What's up!!");	break;
		}

		g_pLogManager->SaveMoveToWarfield(MTWT_CHARACTER_INFO, "User Name : %s Nation : %d Warfield : %s Failure Reason : %s",
			lpChar->Name, lpChar->name_status.nation, szTempMapName, szTempReason);
		//> LTH-040623-KO
	}
}	//> LTH-040504-KO.
 ...
 * @brief	평화 기간에 전쟁터 이동을 클라이언트가 요청했을때의 처리.
 * @param	t_packet* p. 패킷.
 * @param	t_connection c[]. 서버 연결 정보.
 * @param	const int cn. 유저 ID.
 * @return	void.
 */
 //< LTH-040423-KO.
void RecvCMD_NW_MAP_PEACE_MOVE(t_packet* p, t_connection c[], const int cn)
{
	int nWarfield = p->u.NationWar.WarfieldPeaceMove.nWarfieldNo - 1;
	int nUserID = p->u.NationWar.WarfieldPeaceMove.nUserID;

	// 관리서버에게 전쟁터 상태와 소유국가를 요청. 나머지 이동은 관리서버로 부터 받는 곳에서 해결
...
void ProcessPeace(const int UserNation, t_packet* ReturnPacket)			// 011025 LTS
{
	if (UserNation == NW_YL)
	{
		...
		//< LTH-040302-KO 1.4 Patch. 신 전쟁터 정보전송
		...
		//> LTH-040302-KO
		//< LTH-040316-KO 국지전 포인트도 평상시 실어보낸다
		int aSumPoint[NW_NATION_COUNT] = { 0, };
		::GetSumNationPoint(aSumPoint);
		int nI;
		for (nI = 0; nI < NW_NATION_COUNT; ++nI)
			ReturnPacket->u.NationWar.NoticeWarPeace.aLocalWarPoint[nI] = aSumPoint[nI];
		//> LTH-040316-KO
...
void ProcessSquadMemberCount(t_packet* ReceivedPacket, t_packet* SendPacket)
{
	if (IsNeoWarfieldServer())
	{
		...
		{
			//< LTH-040510-KO 전쟁시작후 30분이 지났다면 이동 불가
...
void ProcessNewWarfieldStatus(t_packet* ReceivedPacket, t_packet* SendPacket)
{
	...
	//< LTH-040209-KO 지하전쟁터가 추가되어 비교대상을 하나더 늘렸다
	for (int i = 0; MAX_NEW_WARFIELD + 1 > i; i++)
	{
		if (i == 0)
			SendPacket->u.NationWar.NWStatus.Status[i] = g_pNation->GetNewWarfieldStatus(i);
		else
			SendPacket->u.NationWar.NWStatus.Status[i] = g_pNation->GetNewWarfieldStatus(i + 3);
	}
	//> LTH-040209-KO
...
{
	//< LTH-040217-KO 새전쟁터에서 설원전쟁터를 제외하고 다른 패킷구조체를 채워 보낸다
	...
	else
	{
		...
		SendPacket->u.NationWar.NWarfieldData1.nWarfieldNo = nWarfieldNo;	// LTH-040311-KO 필요해서 실어보낸다.
...
void RecvCMD_REQUEST_DELIVERY(t_packet *p, t_connection c[], const int cn)
{
	...
	case CMD_CHECK_WARFIELD_SQUAD_MEMBERCOUNT:
		ProcessSquadMemberCount(&tempPacket, &SendPacket);
		SendCMD_ANSWER_DELIVERY(UserID, MapServerPort, &SendPacket);
		break;
		//> LTH-040209-KO
...
void RecvCMD_ANSWER_DELIVERY(t_packet *p, t_connection c[], const int cn)
{
	...
	case CMD_CHECK_WARFIELD_NATION_MEMBERCOUNT:
		...
		//< LTH-040209-KO
	case CMD_CHECK_WARFIELD_SQUAD_MEMBERCOUNT:
		if (tempPacket.u.NationWar.CheckSquadMemberCountR.cResult)	// 맵이동 허락
		{
			CallNWMapMove(tempPacket.u.NationWar.CheckSquadMemberCountR.cWarfieldNo, c, UserID);
		}
		else														// 맵이동 불가
			SendCMD_NW_MAP_MOVE_FAIL(c, UserID, 7);	// "전쟁이 시작된지 30분이 지나 이동할 수 없습니다"
		break;
		//> LTH-040209-KO
	}
...
void RecvCMD_REQUEST_TILE_DONT(t_packet *p, t_connection c[], const int cn) //전쟁서서만 패킷을 받는다. // 맵이동을 했는데 전쟁서버이면 받는다.
{
	...
	//< LTH-040528-KO 평화기간 중에 이동한다면 돈트를 모드 없앤다.
	else
...
void RecvCMD_SQUAD_CHAT(t_packet *p, t_connection c[], const int cn)
{
	...
	if (!IsNeoWarfieldServer()) return;	// LTH-040217-KO
	CHARLIST *ch = CheckServerId(cn);
	if (!ch)
	{
		return;
...
{
	//< LTH-040225-KO 1.4 Patch. 신 전쟁터 서버 추가!!
...
void RecvCMD_CHECK_AM_I_DEFEAT(t_packet *p, t_connection c[], const int cn)		// 011015 LTS
{
	...
	//< LTH-040623-KO 전쟁터 이동이 가능할때 전쟁이 끝난 후에 일스 국민이면 무조건 마을로 내보낸다.
	// 이것은 일정 비용을 지불하고 전쟁터로 이동해야하는 일스 국민들인데 전쟁에 참여하지 않고 이동하는
	// 사람들을 생각해본 것임.
	if (ch->name_status.nation == NW_YL)
	{
...
void RecvCMD_REQUEST_NOTICE(t_packet *p, t_connection c[], const int cn)
{
	if (isNationManageServer())
	{
		...
		if (CheckWarLoopProcess())	//전쟁루프가 진행중이면 평화상태가 아니다.
		{
			...
			ProcessPeace(Nation, &SendPacket);						// 평화상태이다.
...
{	//< CSD-CN-031213
	...
	{	//< LTH-040206-KO 새로운 전쟁터 전쟁 시작 메시지 중에 설원전쟁터와 구분하여 상태 변환
		INT nState = NW_WAR;
...
void RecvCMD_REQUEST_NEW_WARFIELD_TILE_DONT(t_packet *p, t_connection c[], const int cn)
{
	...
	if (IsNeoWarfieldServer())
	{
		...
		if (nState == NW_WAR)
		{
			...
			// LTH-040225-KO 팀이 나뉘게 되면 팀정보도 메시지에 실어보내자
			//< LTH-040505-KO 운영자 캐릭은 산타옷 그대로...
...
{
	//< LTH-040206-KO 새 전쟁터중에 설원전쟁터가 아니면 맵서버 상태 변환하고 전역 상태 변수도 구분하여 변환
	if (IsNeoWarfield(p->u.NationWar.CommonDataC.Data))
	{
		...
		//> LTH-040324-KO
		//< LTH-040312-KO 국가전 참가 자격도 초기화한다
		INT nI;
		for (nI = 0; nI < NW_NATION_COUNT; ++nI)
...
{
	//< LTH-040206-KO 새로운 전쟁터중에 설원전쟁터를 제외하고 지하 전쟁터와 같은 나머지 전쟁터의 상태를 따로 변환한다
...
{
	if (isNewWarfieldServer() || IsNeoWarfieldServer())
	{	//< LTH-040225-KO 1.4 패치 신규국가전도 같은곳에서 처리
		t_packet packet;
		ProcessNewWarfieldData(p, &packet);
		QueuePacket(c, cn, &packet, 1);
	}	//> LTH-040225-KO
	else
	{	//< LTH-040225-KO 전쟁 번호를 다루는 것이 다르기때문에 구별한다
...
int CheckHandleByNationWar(t_packet *p, t_connection c[], const int cn)
{
	...
	case CMD_GUARD_BROKE:
		...
		break;
		//< LTH-040423-KO 평화 기간에 전쟁터 이동
	case CMD_NW_MAP_PEACE_MOVE:
		RecvCMD_NW_MAP_PEACE_MOVE(p, c, cn);
		break;
		//> LTH-040423-KO
		//< LTH-040504-KO 관리 서버에게 전쟁터 소유국가와 상태 요청
	case CMD_WARFIELD_INFO:
		RecvCMD_WARFIELD_INFO(p, c, cn);
		break;
		//> LTH-040504-KO.
		//< LTH-040504-KO 관리 서버가 전쟁터 소유국가와 상태를 요청한 맵서버에게 반환
...
cWarfield::cWarfield()
{
	...
	if (!isNationManageServer())				// 관리서버가 아니고 국가전 전쟁터이면 실행한다.
	{
		...
		//LoadWarfieldMonsterData(g_wMapServerPort);	//몬스터 데이터를 로드한다. LTH-040512-KO Hunt Table로 바꾼다.
...
bool cWarfield::InitWarfield()
{
	...
	m_bFirstMakeMonster = true;						//몬스터가 만들어 진 적이 없다.
...
void cWarfield::InitWarLoop()	// 011023 LTS
{
	...
	ClearCastleGateDont();	// LTH-040622-KO 성문의 돈트도 없앤다.
	ClearAllGuard();		// LTH-040528-KO 모든 가드들이 평화기간엔 없어진다.
 ...
 */
 //< LTH-040818-KO.
void cWarfield::ClearCastleGateDont()
{
	int nI;

	for (nI = 0; nI < m_cNpcCount; ++nI)
	{
		if ((m_tWeaponData[nI].NPC_Index >= 63) && (m_tWeaponData[nI].NPC_Index <= 67))		// 가드인가?
		{
			m_tWeaponData[nI].Status = 0;		// 그럼 죽여라 -_-;
		}

		if ((m_tWeaponData[nI].NPC_Index >= 65) && (m_tWeaponData[nI].NPC_Index <= 67))		// 성문인가?
		{
			SetTileDont(m_tWeaponData[nI].MonPointIndex, 0);			// 돈트를 푼다.
			SendTileDontChange(m_tWeaponData[nI].MonPointIndex, 0);		// LTH-040528-KO 돈트가 변경되었다고 클라이언트에게 보낸다.
		}
	}
}	//> LTH-040818-KO.
...
{
	int nI;
	LPCHARLIST lpChar = NULL;	// LTH-040528-KO 가드 제거를 위해
	...
	for (nI = 0; nI < m_cNpcCount; ++nI)
	{
		...
		if ((m_tWeaponData[nI].NPC_Index >= 63) && (m_tWeaponData[nI].NPC_Index <= 67))
		{
			...
			g_pLogManager->SaveMoveToWarfield(MTWT_GUARD_INFO, "Delete Guard!! NPC Index : %d", m_tWeaponData[nI].NPC_Index);	// LTH-040524-KO 로그 강화
		}
...
void cWarfield::ChangeWarfieldStatus(int Result)		//결과에 의해 상태를 바꾼다.
{
	...
	case NW_WAIT_VOTE:									// 투표를 기다리는 상태인데..
		...
		else											// 결과가 투표로 들어가면 안된다이면 지금 다른 전쟁루프가 돌고 있다.
		{
			...
			ClearAllGuard();		// LTH-040514-KO 모든 가드들이 평화기간엔 없어진다.

			//< LTH-040531-KO 몬스터 출현
			if (m_bFirstMakeMonster == false)
			{
				g_pRegenManager->Ready(CGroupInfo::ET_NORMAL);
				m_bFirstMakeMonster = true;
				g_pLogManager->SaveMoveToWarfield(MTWT_MONSTER_INFO, "Create Warfield Monster!!");	// LTH-040524-KO 1.04+ 로그 강화.
			}
			//> LTH-040531-KO
	...
	case NW_WAIT_PREPARE:
		...
		//< LTH-040622-KO 전쟁 준비 기간이 되었기 때문에 몬스터들을 모두 없앤다
		if (m_bFirstMakeMonster == true)
		{
			NPC_AutoCreate();
			g_pRegenManager->Remove(CGroupInfo::ET_NORMAL);
			m_bFirstMakeMonster = false;
			g_pLogManager->SaveMoveToWarfield(MTWT_MONSTER_INFO, " Delete Warfield Monster!!");	// LTH-040524-KO 1.04+ 로그 강화.
		}
		//> LTH-040622-KO
...
void cWarfield::SetStatusVote()								// 관리서버에서 언제시작할지를 체크하다. 보내준다.
{
	...
	SendWarBBS2NationMaps(tempNationCode, &WarBBS);			// 공격자 맵에만 보낸다. 이때... 전쟁 상황버튼을 클라이언트에서 감빡이게 하면 된다.
	SendWarBBS2NationClients(tempNationCode, &WarBBS);		// 전쟁맵에 있는 공격국 유저에게도 보낸다.	
	SendUserGoOutsideBBS(NGOB_DEMEND_GO_OUT);			// LTH-040507-KO 전쟁터에서 방출한다는 공지 뿌리기.
}
...
void cWarfield::KickDefeatUser2Home()		// 패배한 유저들을 고향으로 ?는다.
{
	for (int i = DRAGON_CONNECTIONS_START; i < DRAGON_MAX_CONNECTIONS; i++)
	{
		if (CheckServerId(i))
 ...
 * @brief	전쟁터의 유저를 방출한다는 메시지를 뿌린다.
 * @return	void.
 */
 //< LTH-040507-KO.
void cWarfield::SendUserGoOutsideBBS(int nMsg)
{
	t_WarBBS WarBBS;
	WarBBS.BBSType = BBS_USER_GO_OUTSIDE;
	WarBBS.LanNo = 0;
	WarBBS.ArgType = BBS_NUM;
	WarBBS.Size = 1;
	WarBBS.Arg[0] = nMsg;
 ...
 * @brief	전쟁터의 유저 방출, 1분 마다 정해진 숫자의 유저를 내보낸다.
 * @return	void.
 */
 //< LTH-040512-KO.
void cWarfield::UserGoOutside()
{
	if (!isNationWarfieldServer())
		return;
	...
	if (m_dRemainTime % 60)
	{
		...
		for (nI = DRAGON_CONNECTIONS_START; nI < DRAGON_MAX_CONNECTIONS; ++nI)
		{
			if (::CheckServerId(nI))
			{
				...
				{
					SendUserGoOutsideBBS(NGOB_RETURN_VILLAGE);	// LTH-040507-KO 마을로 돌려보낸다는 공지
					//< LTH-040524-KO 로그 강화
...
void cWarfield::UpdateProgressofWar()
{
	...
	case NW_VOTE:
	{
		...
		//< LTH-040507-KO 정해진 시간에 따라 유저를 방출한다는 메시지를 뿌린다.
	...
	case NW_PREPARE:								// 전쟁 준비 상태가 진행중이다.
	{
		...
		UserGoOutside();		// LTH-040512-KO 전쟁터의 유저 방출.
	}
	break;
...
void cWarfield::UpdateStatusofWar()
{
	...
	case NW_VOTE:
		...
		break;
	case NW_PREPARE:
		MakeWeaponData();		// 전쟁무기의 상태를 바꾼다.  // 결계석 버그 때문에... // LTH-040512-KO 1.04+부터 전쟁터에 전쟁이 시작하면 성문/수호석을 만든다.
	...
	case NW_WAR:
		...
		//MakeWeaponData();						// 전쟁무기의 상태를 바꾼다. // LTH-040512-KO 1.04+의 전쟁터에서 몬스터 출현을 위해 성문/수호석을 나중에 만들자!!
		...
		ClearAllGuard();								// LTH-040513-KO 모든 가드들을 제거한다.
		break;
	}
...
void cWarfield::CalcNationWar_Exp()			// 020115 LTS		// 국가전 버그
{
	for (int i = DRAGON_CONNECTIONS_START; i < DRAGON_MAX_CONNECTIONS; i++)
	{
		...
		if (m_NationCode == ch->name_status.nation)
		{
			...
			g_pLogManager->SaveLogChange_Fame(ch, ch->fame, ch->fame + ch->GainedFame, LF_NATIONWAR);		// 020909 LTS
...
void cWarfield::DeleteAllSquadMember()
{
	...
	for (int i = 0; i < NW_SQUAD_KIND; i++)				// 부대장, 커맨더의 Squad hash를 지운다.
	{
...
void cWarfield::FirstMakeMonster()		// 모든  NPC (성문, 수호석이 살아있을때 만들어진다.)
{
	...
	//m_bFirstMakeMonster=true;	// LTH-040528-KO 사용 안하기루...
}
...
void cWarfield::ActiveBonusTime()
{
	m_bBonusActive = true;
	m_dBonusLoopTime = 0;
	//< LTH-040528-KO Hunt table로 변경하고 1.4+에서는 소유국가의 전쟁터 이동 시에 몬스터 출현을 위해 삭제!!
	if (m_bFirstMakeMonster == false)
	{
		g_pRegenManager->Ready(CGroupInfo::ET_NORMAL);
		m_bFirstMakeMonster = true;
...
void cNation::SetLocalWarStatus2(int isStart)	// SendSealStoneBlock()에서 들어온다 시간에 의한	// LTS NEW LOCALWAR
{
	...
	/*040721_KJHuNs g_pLogManager로 이동(정리목적)
...
int cNation::CheckWinNation()
{
	...
	//	m_LocalWarDefencePoint[1], m_LocalWarPoint[1],  m_nAllLocalWarSealStoneHP[1] );
	//MyLog(0, "LocalWar End :::: YL DefencePoint = %d, WarPoint = %d, Seal All HP = %d",
	//	m_LocalWarDefencePoint[2], m_LocalWarPoint[2],  m_nAllLocalWarSealStoneHP[2] );
	//

	//leave log
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR, "LocalWar End :::: BY DefencePoint = %d, WarPoint = %d, Seal All HP = %d", \
		m_LocalWarDefencePoint[0], m_LocalWarPoint[0], m_nAllLocalWarSealStoneHP[0]);
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR, "LocalWar End :::: ZY DefencePoint = %d, WarPoint = %d, Seal All HP = %d", \
		m_LocalWarDefencePoint[1], m_LocalWarPoint[1], m_nAllLocalWarSealStoneHP[1]);
	g_pLogManager->SaveLog_List(LT_LOCAL_WAR, "LocalWar End :::: YL DefencePoint = %d, WarPoint = %d, Seal All HP = %d", \
		m_LocalWarDefencePoint[2], m_LocalWarPoint[2], m_nAllLocalWarSealStoneHP[2]);
...
//< LTH-040220-KO 세국가의 국가전 포인트에 따른 획득 점수 정의
...
//< LTH-040219-KO 국가전 종료 후에 국가전 포인트에 따라 순위를 매겨 점수를 반환한다
// 같은 국가는 같은 점수이다
VOID cNation::CheckWinNationPoint(INT aPoint[NW_NATION_COUNT])
{
	...
	for (; ltScoreCount != ltScoreCountEnd; ltScoreCount++)
	{
		...
		{
			//< LTH-040316-KO 국지전 포인트가 없다면 순위 점수도 없닷!!!! 어딜!!
...
//< LTH-040206-KO 1.4 패치후 새 전쟁터 때문에 헤더파일에서 이전
bool cNation::CheckSecretPlaceOpen()
{
	...
	if (g_naWarfieldState[WI_HADES_WARFIELD] == NW_SOPEN) return TRUE;	// LTH-040207-KO
	return false;
}
...
void cNation::SetRemainTime(int WarfieldNo, DWORD RemainTime)
{
	if (WarfieldNo >= 0 && WarfieldNo <= 2)
		...
		if (WarfieldNo >= 3)
		{
			//< LTH-040207-KO
			if (IsNeoWarfield(WarfieldNo))
			{	//< LTH-040326-KO m_dwWarRemainTime이 세팅되기도 전에 CMD_SET_REMAIN_TIME을 받아 세팅하려 했다.
...
void cNation::CheckStartWarfieldWar()
{
	...
	for (int i = 0; i < m_iNewStartWarTimeCount; i++)
	{
		NewCheckAndActive(i);
	}

	//< LTH-040206-KO 일주일에 일어나는 전쟁들이 현재 일어났는지를 판별
...
{	//< CSD-030804
	...
	//< LTH-040206-KO 전쟁 시간 제한 사항을 전역적으로 호출한다
...
{
	//< LTH-040213-KO 신규전쟁터중에 설원전쟁터만 따로 처리
...
bool cNation::InitNewWarfield()
{
	...
	//< LTH-040209-KO 걍 전역으로 설정된 전쟁터 상태를 모두 평화 상태로 초기화 한다.
	INT nI;
...
void cNation::CheckAndSendWarStart(t_packet* p)
{
	...
	case 3:
		...
		//< LTH-040207-KO 1.4 패치이후 모든 전쟁터를 여기에서 처리
	case WI_HADES_WARFIELD:
	...
	case NW_PEACE:
	{
		...
		{
			//< LTH-040207-KO 새 전쟁터중에 설원전쟁터만 따로 처리
			if (IsNeoWarfield(WarfieldNo))
			{
...
void CTeam::GiveFame(LPCHARLIST pCaster, LPCHARLIST pTarget, int Range, int AddFame)
{
	...
	for (TeamItor itor = m_TeamList.begin(); itor != m_TeamList.end(); itor++)
	{
		...
		if (pCaster->GetServerID() != cn)
		{
			...
			{
				g_pLogManager->SaveLogChange_Fame(lpChar, lpChar->fame, lpChar->fame + AddFame, LF_NATIONWAR);
...
void CGuardStone::Clear()
{
	...
	m_iTypeCount[1] = 0;
	m_iTypeCount[2] = 0;
	m_iTypeCount[3] = 0; // LTH-040311-KO 외성수호석이 추가
...
bool CGuardStone::LoadGData()
{
	...
	while (ret == SQL_SUCCESS)
	{
		...
		++m_iTypeCount[m_pGData[index].NPCType];	// LTH-040217-KO
		index++;
		ret = SQLFetch(hStmt);
	}
	SQLFreeStmt(hStmt, SQL_DROP);
...
bool CGuardStone::MakeGuardStone(const int iIndex, const int iCallType)//030102 lsw //FirstMakeNPCPtr() MakeNPCPtr() 에서만 호출 합니다.
{
	...
	//< LTH-040331-KO 결계석과 성문 생성을 summon에서 바꿨다 skb로...
	...
	//< CSD-030211
	//< LTH-040322-KO 설원전쟁터와 몬스터 국가전의 가드들의 에너지를 높인다. 1/10 제한을 없앤다. 한국에도 적용
	...
	//{
	int iRookieNationWarSealStoneHp = pGuardStone->HpMax / 3;
	//}
	//> LTH-040322-KO
	//> CSD-030211
...
void CGuardStone::SendBrokeBBS(int Team, int Type, int No, LPCHARLIST pTarget)
{
	...
	packet.u.NationWar.GuardBroke.nWarfieldNo = m_iWarfieldNo;	// LTH-040303-KO 1.4 Patch.
...
void CGuardStone::MakeDontTable()
{
	...
	{
		SQLAllocStmt(g_hDBC_DragonDB, &hStmt);
		wsprintf(query_stmt, "SELECT   COUNT(*) AS fieldcount FROM DontTable WHERE DontNo=%d", i + 4);
		...
		SQLAllocStmt(g_hDBC_DragonDB, &hStmt);
		wsprintf(query_stmt, "SELECT * FROM DontTable WHERE DontNo=%d", i + 4);
		ret = SQLExecDirect(hStmt, (UCHAR*)query_stmt, SQL_NTS);
		ret = SQLFetch(hStmt);
		int Index = 0;
		while (ret == SQL_SUCCESS)
	...
	FILE* fp = fopen("Output/LTS000.Dat", "ab");	// LTH-040314-KO 1.4 패치 전쟁터 돈트를 생성하기 위해 변경
	if (fp)
	{
		//fwrite(&Size,sizeof(int),1,fp);
		long temp = 1 + ((sizeof(int) + (sizeof(POINT) * 28)) * 2) + ((sizeof(int) + (sizeof(POINT) * 16)) * 2);
		fseek(fp, temp, SEEK_SET);				// LTH-040314-KO 돈트 종류는 직접수정하고 설원만큼 건너뛴다
		for (int i = 0; i < Size; i++)
		{
...
//< LTH-040303-KO 1.4 Patch. 신전쟁터의 종료조건을 만들기 위해 인자 추가
...
void CNewWarfield::SetWarfieldStatus(int Status)			// Packet Concern....
{
	...
	case NW_WAR:
	{
		...
		::SendCMD_UPDATE_WARTIME(m_iWarfieldNo + 3, m_iStatus, GetRemainTime());	// LTH-040426-KO 남은 시간을 제대로 표시하기 위해...
		m_pGuard[0].MakeNPCPtr();
		m_pGuard[1].MakeNPCPtr();
		KickAllUser2Home();
	}
	...
	case NW_SOPEN:
	{
		...
		m_64LoopTime = g_WarTime + m_Time.SOpenTime;
	}
	break;
	}
	m_iStatus = Status;
...
{
	long long RemainTime = m_64LoopTime - g_WarTime;
...
void CNewWarfield::GiveFame(int TeamNo, LPCHARLIST pCaster, LPCHARLIST pTarget)
{
	...
	{
	case 64:
		g_pLogManager->SaveLogChange_Fame(pCaster, pCaster->fame, pCaster->fame + 20, LF_NATIONWAR);
		pCaster->fame += 20;
		SendFameChange(pCaster, 20);
		m_pTeam[TeamNo].GiveFame(pCaster, pTarget, Range, 4);
		break;
	case 63:
		g_pLogManager->SaveLogChange_Fame(pCaster, pCaster->fame, pCaster->fame + 15, LF_NATIONWAR);
		pCaster->fame += 15;
	...
	case 66:
	case 67:
		g_pLogManager->SaveLogChange_Fame(pCaster, pCaster->fame, pCaster->fame + 10, LF_NATIONWAR);
		pCaster->fame += 10;
...
{
	//< LTH-040225-KO 1.4 Patch 후에 새로운 전쟁터인지 비교를 더 한다
	if (!isNewWarfieldServer() && !IsNeoWarfieldServer())
		return;
...
{
	//< LTH-040225-KO 1.4 패치 신규국가전 추가로 if문 증가
	if (!isNewWarfieldServer() && !IsNeoWarfieldServer())
		return;
