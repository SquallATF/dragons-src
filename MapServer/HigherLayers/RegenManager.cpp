...
CRegenManager::CRegenManager() : m_bReady(false) // CSD-040317
{
	m_mpGroup.clear();
}
...
void CRegenManager::Ready(int nHunt, int nEvent)
{	//< CSD-040310
	...
	if (m_mpGroup[nHunt]->GetEventType() == nEvent)
	{
...
void CRegenManager::Remove(int nEvent)
{	//< CSD-040310
	for (ITOR_GROUP i = m_mpGroup.begin(); i != m_mpGroup.end(); ++i)
	{
		CGroupInfo* pGroup = i->second;
...
void CRegenManager::Remove(int nHunt, int nEvent)
{	//< CSD-040310
	if (!IsExistHunt(nHunt))
	{
		return;
