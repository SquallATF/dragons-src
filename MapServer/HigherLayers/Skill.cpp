	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		while (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
		{
			...
			for (count = 0; count < 4; count++)
			{
				int iDummy = 0;
				retCode = SQLGetData(hStmt, ++column, SQL_C_LONG, &iDummy, 0, &cbValue);
				ItemMutantKind[i].Insurance[count] = iDummy;
			}
	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		while (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
		{
			...
			for (count = 0; count < 4; count++)
			{
				int iDummy = 0;
				retCode = SQLGetData(hStmt, ++column, SQL_C_LONG, &iDummy, 0, &cbValue);
				ItemGrade[i].InsuranceAmount[count] = iDummy;
			}
...
void SkillTargetTypeItemMake(const int cn, t_client_skill_info *sp)
{
	...
	CheckItem(cn);	//050331_KCH Memory Direct Hacking에 따른 아이템 체크 강화.(아이템 합치다 정상적인 아이템인지 먼저 확인)
	//>050425_KCH
...
int skill_Snooping(const int cn, const int skillno, const int x, const int y, const int targettype, const int target_id)
{
	...
	//<050328_KCH 배낭뒤지기 스킬을 당분간 사용못하게 처리(이는 유저의 요청으로, 이후에 좋은 아이디어 나오면 그때 수정하자.)
	goto FAIL_;
	//>050328_KCH
...
int SkillLevelExpLoad()
{
	...
	while (ret == SQL_SUCCESS)
	{
		...
		for (int i = 2; i < 103; i++)
		{
			...
			SkillLevelExp[sCustID][i - 2] = exp * 100;  //coromo 2007/05/01 묏鱗세콘쒔駱 
		}
		c++;
		ret = SQLFetch(hStmt);
	}
...
void DecreaseEquipDuration(CHARLIST *ch, const DWORD dwDecValue, const int iItemPos, const int iTargetSprNo, const bool bIsAttacker, bool bIsNoCount)
{
	...
	int iSendValue = 0;
	const int iValue = abs((int)dwDecValue);
	...
	else	//내구도가 아닌 아이템
	{
		...
		{
			ch->dec_duration++;
			if (ch->dec_duration > 27) ch->dec_duration = 0;
			else return;
			ret = (int)log(abs(iValue) + 1);
			if (ret > 14) ret = 14;
			if (ret < 1) ret = 1;
		}
