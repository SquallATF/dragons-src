...
//#include "CItem.h"	//050224_KCH 마일리지몰 작업
#include "OP_Magic.h"
#include "Op_Battle.h"
...
int CStrike::CalcAttack(BYTE nType) const
{
	...
	if (!m_pTarget->IsIgnorePhysicalAttack())
	{
		...
		case BLOOD_WILL:
		case FLEET_MISSILE:
		case FIRE_EXTREME: // CSD-040831
		case ICE_EXTREME: // CSD-040831
		{
			nPercent += m_pCaster->CorrectCombatAttack(nCaster, nResult);
			break;
		}
		case MULTIPLE_FIRE://多重火
		case ICING_BLAST: //冰暴
		{
			nPercent -= (100 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 这里是攻击
			break;
		}
		case SWORD_N_ROSES: //剑与玫瑰
		{
			nPercent -= (75 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 这里是攻击
			break;
		}
		case DOUBLE_ATTACK: //双重攻击
		{
			nPercent -= (90 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 这里是攻击
			break;
		}
		case CHERROY_SHADE:
		{ // 타격 대상자는 질병의 효과
...
{	//< CSD-TW-030606 : 대상자에게 Packet 보내기
	...
	case CMD_MAGIC_RESULT:
	{
		...
		g_pLogManager->SaveLogBattle_Ballancing(m_pCaster, m_pTarget, nMagic, 0, 0, 0, 0, 0);
		break;
	}
	case CMD_MAGIC_RESULT_T:
	{
	...
	case CMD_MAGIC_RESULT_H:
	{
		...
		g_pLogManager->SaveLogBattle_Ballancing(m_pCaster, m_pTarget, nMagic, nLife, 0, 0, 0, 0);
		break;
	}
	case CMD_MAGIC_RESULT_M:
	{
		...
		g_pLogManager->SaveLogBattle_Ballancing(m_pCaster, m_pTarget, nMagic, nLife, nMana, 0, 0, 0);
		break;
	}
	}
...
int CCombat::CalcAttack(BYTE nType) const
{
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case TWISTER:
		{
			nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult) / 100;   // 攻击数字小反而高
			break;
		}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		{
			nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult) / 65;  // 攻击数字小反而高 这里修改了究极之电
			break;
		}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case WIND_EXTREME:
		{
			nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult) / 200;  // 攻击数字小反而高 这里修改了 究极之风70
			break;
		}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case EARTH_EXTREME:
		{
			nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult) / 37;  // 攻击数字小反而高 这里修改了 究极大地
			break;
		}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case WHILWIND:
		{
			nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult) / 200;   // 攻击数字小反而高 这里修改了 回风45
			break;
		}
		}
	}
...
void CCombat::Correct(BYTE nType, int& rDamage)
{
	...
	{
		//if (m_pTarget->name_status.king || m_pTarget->IsCounselor())  //coromo 2005/05/06 去除KING特权
		//{
		//	rDamage /= 30;
		//}
	}
...
SEND:
	...
	if (m_pCaster->IsPlayer())
	{
		...
		m_packet.u.combat.server_combat_result_h.bDoubleExpUp = m_pCaster->IsDoubleExpUp();
