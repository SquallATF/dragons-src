...
int CStrike::CalcAttack(BYTE nType) const
{
	...
	if (!m_pTarget->IsIgnorePhysicalAttack())
	{
		...
		case MULTIPLE_FIRE://뜩路삽
		case ICING_BLAST: //깥괬
			{
				nPercent -= (100 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 侶쟁角묑샌
				break;
			}
		case SWORD_N_ROSES: //숲宅천밧
			{
				nPercent -= (75 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 侶쟁角묑샌
				break;
			}
		case DOUBLE_ATTACK: //崗路묑샌
			{
				nPercent -= (90 - m_pCaster->CorrectCombatAttack(nCaster, nResult));  //100 - m_pCaster 侶쟁角묑샌
				break;
			}
		case CHERROY_SHADE:
			{ // 타격 대상자는 질병의 효과
...
int CCombat::CalcAttack(BYTE nType) const
{
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case TWISTER:
			{
				nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult)/100;   //묑샌鑒俚鬼럽랍멕
				break;
			}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case LIGHTNING_EXTREME:
			{
				nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult)/65;  //묑샌鑒俚鬼럽랍멕 侶쟁錦맣죄씩섐裂든
				break;
			}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case WIND_EXTREME:
			{
				nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult)/200;  //묑샌鑒俚鬼럽랍멕 侶쟁錦맣죄 씩섐裂루70
				break;
			}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case EARTH_EXTREME:
			{
				nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult)/37;  //묑샌鑒俚鬼럽랍멕 侶쟁錦맣죄 씩섐댕뒈
				break;
			}
		}
	}
	...
	{ // 전투스킬에 의한 공격력 보정
		...
		case WHILWIND:
			{
				nAttack = nAttack*m_pCaster->CorrectCombatAttack(nCaster, nResult)/200;   //묑샌鑒俚鬼럽랍멕 侶쟁錦맣죄 쀼루45
				break;
			}
		}
	}	
