...
#define CMD_MONSTER_RAID_START					26502	// LTH-040224-KO 몬스터 마을 습격을 시작한다 신규전쟁터맵서버->해당 맵서버
#define CMD_MONSTER_RAID_END					26503	// LTH-040224-KO 몬스터 마을 습격을 끝낸다. 신규전쟁터 맵서버-> 해당 맵서버.
#define CMD_GUARD_CAN_NOT_BROKE					26504	// LTH-040317-KO 성문이나 수호석을 부술 수 없다
#define CMD_CHECK_REMAIN_GUARD					26505	// LTH-040318-KO 남은 성문이나 수호석을 반환한다.

// LTH-040207-KO 기존 전쟁터3개 + 설원전쟁터1개 + 지하전쟁터 1개 총 5개의 전쟁터
...
{
	DWORD dwRemainTime;
	INT nWarfieldNo;				// LTH-040311-KO 전쟁번호를 실어보낸다.
	INT nTeamCount[2];
	INT nRemainGuardCount[2][4];		// 각각의 남은 Guard 수 0 : 최종수호석, 1 : 내성수호석, 2 : 성문 3 : 외성수호석
	CHAR cGuardStatus[2][NEO_NUMOFGUARD];	// Hades War에 맞춰져있다. 외성 : 4, 성문(좌) : 4, 성문(우) : 4, 내성 : 4, 최종 : 1
};	//> LTH-040212-KO.
...
//<------------------------------- 1.04+ addition ------------------------------------------
#define CMD_NW_MAP_PEACE_MOVE			26506	// LTH-040421-KO 평화 기간의 전쟁터 이동 요청 C -> M
#define CMD_NW_MAP_PEACE_MOVE_RESULT	26507	// LTH-040421-KO 평화 기간에 요청한 전쟁터 이동 결과 M -> C
#define CMD_WARFIELD_INFO				26508	// LTH-040504-KO 전쟁터 소유자와 전쟁터 상태 요청 M -> WM
#define CMD_WARFIELD_INFO_RESULT		26509	// LTH-040504-KO 전쟁터 소유자와 전쟁터 상태 요청 WM -> M
...
struct NEW_WARFIELD_STATUS
{
	...
	char	Status[MAX_NEW_WARFIELD + 1];	// LTH-040212-KO 지하전쟁터 추가...
};

struct NEW_WARFIELD_DATA1
{
	DWORD	RemainTime;
	int		nWarfieldNo;	// LTH-040311-KO 전쟁터 번호를 실어보낸다.
...
struct NOTICE_WAR_PEACE
{
	char	Type;
	int     WarNo;					// 현재 전쟁번호
	char    RemainTime[4][2];		// 일반전쟁터 2개 , 설원전쟁터 1개, 지하전쟁터 1개 LTH-040207-KO
...
struct NEWWAR_START
{
	...
	unsigned char	btNationPoint;	// LTH-040313-KO 국지전 국가 포인트
};

struct GUARD_BROKE
{
	int				nWarfieldNo;	// LTH-040303-KO 1.4 Patch.
	int				aRemainGuard[2][4];	// LTH-040314-KO 정보 표시를 위해
	char			BrokeTeamNo;
...
typedef union NationWar_P
{
	...
	t_CheckSquadMemberCount		CheckSquadMemberCount;	// LTH-040209-KO Hades War 부대 인원수
	t_CheckSquadMemberCountR	CheckSquadMemberCountR;	// LTH-040209-KO Hades War 부대 인원체크 결과
	NEO_WARFIELD_DATA			NeoWarfieldData;		// LTH-040212-KO	전쟁터 데이터 전송용
	NEO_WAR_RESULT				NeoWarResult;			// LTH-040225-KO 1.4 새 전쟁터 결과 정보
	GUARD_CAN_NOT_BROKE			GuardCanNotBroke;		// LTH-040317-KO 가드를 부술 수 없는 이유 전송
	REMAIN_GUARD				RemainGuard;			// LTH-040318-KO 남은 가드들의 수를 반환
	tagWarfieldPeaceMove		WarfieldPeaceMove;		// LTH-040423-KO 1.4+ 평화기간 전쟁터 이동.
	tagWarfieldPeaceMoveResult	WarfieldPeaceMoveR;		// LTH-040423-KO 1.4+ 평화기간 전쟁터 이동 결과.
	tagWarfieldInfo				WarfieldInfoToManager;	// LTH-040504-KO 1.4+ 전쟁 관리서버에게 전쟁터 소유자와 상태를 요청.
	tagWarfieldInfoResult		WarfieldInfoFromManager;	// LTH-040504-KO 1.4+ 전쟁 관리서버가 전쟁터 소유자와 상태를 반환.
} t_NationWar;
