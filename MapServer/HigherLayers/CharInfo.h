...
struct CHAR_DATA
{
	...
	//  Client을 위한 Data
	short int		SprNo;					//050413_KCH 0:여자 1 : 남자 (gender와 같아야함)
...
public:
	...
	void Message(BYTE nType, BYTE nKind, WORD nNumber, BYTE nStep, BYTE nCharCls);   // add by taniey
	...
	void GetStepInfo(unsigned char* pData, int nSize);
	void SetStepInfo(unsigned char* pData, int nCount);
	...
	bool ChangeUpAddExp(int nExp, bool bDoubleExpUp = false); // CSD-040803
	bool ChangeUpAddExpBonus(int nExp); // CSD-TW-030624 
	bool ChangeUpTacExp(int nType, int nExp, bool bDoubleExpUp = false); // CSD-040803
...
public:
	...
	void SetDualClass(int nDual) { m_nDualCls = unsigned __int8(nDual); }
	void SetClassStep(int nStep) { m_nClsStep = unsigned __int8(nStep); }
	void SetExperienceStep(int nStep) { m_nExpStep = unsigned __int8(nStep); }
	void SetTacticsStep(int nIndex, int nStep) { m_aTacStep[nIndex] = unsigned __int8(nStep); }
	...
	// 040715_KJHuNs 함수를 cpp에서 구현, include 파일이 꼬일수가 있기에.
	...
	void SetIncExp(bool bFlag) { bCanExpInc = bFlag; }
	bool IsIncExp() { return bCanExpInc; }		// can inc exp
...
private:
	...
	unsigned __int8  m_nDualCls;          // 듀얼 클래스 종류
	unsigned __int8  m_nClsStep;          // 클래스 단계
	unsigned __int8  m_nExpStep;          // 경험치 단계
	unsigned __int8  m_aTacStep[NO_TAC];  // 택틱 경험치 단계
