...
// 040715_KJHuNs g_pLogManager를 사용하기 위함.
#include "LogManager.h"
#include "../LowerLayers/servertable.h"	//050413_KCH g_pServerTable->GetOwnServerData()->wPort 맵포트번호 알기위함

extern int NPC_MaleName_Count;
extern cLocalWarfield* g_pLocalWarfield;
extern NPCName_by_Gender NPC_Name_Ref[Num_Of_NPC_Name];

int MAX_EXP_LEVEL = 200;  // CSD-040204
int MAX_TAC_LEVEL = 500;  // CSD-040204  coromo
int MAX_TIME_USE_GUILD_ITEM_MENU = 30;		// 040623 YGI
...
{	//< CSD-030806 : 단계 데이타 재설정
	...
	int nCombatPoint = unsigned __int8(pData[CSP_STEP]);
...
bool CCharInfo::ChangeUpAddExp(int nExp, bool bDoubleExpUp)
{	//< CSD-040803
	if (IsNpc())
	{
		return false;
...
bool CCharInfo::ChangeUpTacExp(int nType, int nExp, bool bDoubleExpUp)
{	//< CSD-040803
