...
//040715_KJHuNs g_pLogManager를 사용하기위함.
...
void CArenaState::CalcWinTeamLaderScore(CArenaTeam* pTeam)
{
	...
	{
		CHARLIST* pMember = *i;
		g_pLogManager->SaveLogChange_Ladder(pMember, pMember->LadderScore, pMember->LadderScore + nPoint, CLT_WIN_COLOSSUS);
...
void CArenaState::CalcLoseTeamLaderScore(CArenaTeam* pTeam)
{
	...
	for (CArenaTeam::ITOR_MEMBER i = ltMember.begin(); i != ltMember.end(); ++i)
	{
		...
		// 020830 YGI
		g_pLogManager->SaveLogChange_Ladder(pMember, pMember->LadderScore, pMember->LadderScore - nPoint, CLT_LOSE_COLOSSUS);
...
{	//< CSD-030723
	...
	{	
		...
		g_pLogManager->SaveLogChange_BattingMoney(pGambler, dwOldMoney, dwNewMoney);
	}
}	//> CSD-030723
...
{	//< CSD-030723
	...
	for (CArenaTeam::ITOR_GAMBLER i = mpGambler.begin(); i != mpGambler.end(); ++i)
	{
		...
		if (pGambler->GetBankMoney() + dwDividendMoney < MAX_BANK_MONEY)
		{
			...
			const DWORD dwNewMoney = pGambler->GetBankMoney();
			g_pLogManager->SaveLogChange_DividendMoney(pGambler, dwOldMoney, dwNewMoney, true);
...
DWORD CArenaGambling::GetBattingMoney()
{
	return m_dwBattingMoney;
}
