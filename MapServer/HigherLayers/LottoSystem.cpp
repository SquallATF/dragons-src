...
//	m_nGiveItemNumber = SADONIX_NO;//기본은 사드 오닉스를 준다. 하지만 테이블에서 읽어온다.
	...
	//< 040615_KJHuNs 새로운 회차의 로또 이벤트 등록 메세지를 DBDemon에 한번만 보내기위함
...
//< LTH-040825-KO 로또 회차 문제로 수정. order by 추가
	...
	{
		SQLFreeStmt(hStmt, SQL_DROP);
		::MyLog(0,"CLottoSystem::LoadTable() Lotto_Event Query Error!!");
		return 0;
	}	
	...
	{
		SQLFreeStmt(hStmt, SQL_DROP);
		::MyLog(0,"CLottoSystem::LoadTable() Lotto_Event Table Fetch Error!!");
		return false;
	}
		...
		for(int i = 0; i < nEventCount;++i)
		{
			...
			if(CurDate.tm_wday == LotteryEnd.nDay)//추첨 끝날 일이랑 같으면.
			{
				if(CurDate.tm_hour == LotteryEnd.nHour)//추첨 끝날 시간이랑 같을때.
				{
					...
					{
						//< 040615_KJHuNs
						//기존은 SQL에 같은 회차에 여러번 등록되어서 정보 Sync 가 안맞았음.
						//수정내용은 실행되는 1분안에 1번만 보내게 처리됨
						// 선결조건:	1. DBDemon과 MapServer간에는 패킷 손실이 없다.
						//				2. DBDemon의 Packet Queue에서의 Overflow는 없다.
						if (false == m_bIsNewEventRegistToOnce ) //< 040615_KJHuNs
						{
							SendDBNewEvent();//새 이벤트를 진행하자.
							//< 040615_KJHuNs 새로운 회차의 로또 이벤트 등록 메세지를 DBDemon에 한번만 보냄
...
void CLottoSystem::RecvDBNewEvent(LOTTO_EVENT_INFO* pInfo)
{
	m_EventStatus = EVENT;
	m_bGenNumbers = false;
	m_bIsNewEventRegistToOnce = false;		//< 040614_KJHuNs 다음 회차 로또 등록하기위함
