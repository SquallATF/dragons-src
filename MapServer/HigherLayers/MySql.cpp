...
HENV  g_hEnv_DragonDB = NULL;	//050224_KCH 마일리지몰 작업
HDBC  g_hDBC_DragonDB = NULL;	//050224_KCH 마일리지몰 작업
HENV  g_hEnv_TotalDB = NULL;	//050224_KCH 마일리지몰 작업
HDBC  g_hDBC_TotalDB = NULL;	//050224_KCH 마일리지몰 작업
...
void ConvertLoad(unsigned char* pData, int nSize = MAX_STEP)
{	//< CSD-TW-030620
	CStepMigration stepMigration;
	stepMigration.Load(pData);
}	//> CSD-TW-030620

void ConvertSave(unsigned char* pData, int nSize = MAX_STEP)
{	//< CSD-TW-030620
	CStepMigration stepMigration;
	stepMigration.Save(pData);
}	//> CSD-TW-030620
int Init_DragonRajaDB_SQL(LPSTR szDSN, LPSTR szUID, LPSTR szUPW)
{
	RETCODE		retCode;

	SQLAllocEnv(&g_hEnv_DragonDB);
	SQLAllocConnect(g_hEnv_DragonDB, &g_hDBC_DragonDB);

	retCode = SQLConnect(g_hDBC_DragonDB, (UCHAR *)szDSN, SQL_NTS, (UCHAR *)szUID, SQL_NTS, (UCHAR *)szUPW, SQL_NTS);
...
int Init_TotalDB_SQL(LPSTR szDSN, LPSTR szUID, LPSTR szUPW)
{
	...
	SQLAllocEnv(&g_hEnv_TotalDB);
	SQLAllocConnect(g_hEnv_TotalDB, &g_hDBC_TotalDB);

	retCode = SQLConnect(g_hDBC_TotalDB, (UCHAR *)szDSN, SQL_NTS, (UCHAR *)szUID, SQL_NTS, (UCHAR *)szUPW, SQL_NTS);
...
{
	if (g_hDBC_ChrLogDB != NULL)	SQLDisconnect(g_hDBC_ChrLogDB);
	if (g_hDBC_ChrLogDB != NULL)	SQLFreeConnect(g_hDBC_ChrLogDB);
	if (g_hEnv_ChrLogDB != NULL)	SQLFreeEnv(g_hEnv_ChrLogDB);
...
{
	if (g_hDBC_DragonDB != NULL)
		SQLDisconnect(g_hDBC_DragonDB);
	SQLFreeConnect(g_hDBC_DragonDB);
	SQLFreeEnv(g_hEnv_DragonDB);

	if (g_hDBC_TotalDB != NULL)
		SQLDisconnect(g_hDBC_TotalDB);
	SQLFreeConnect(g_hDBC_TotalDB);
	SQLFreeEnv(g_hEnv_TotalDB);
...
int Querry_SQL(LPSTR szQuerry, HDBC hdbc /*= NULL */)
{
	HSTMT		hStmt = NULL;
	RETCODE		retCode;
	if (hdbc == NULL) hdbc = g_hDBC_DragonDB;
	...
	char *login_id, char *name)		// 1219 YGI
{
	...
	if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
		...
		if (retcode == SQL_NEED_DATA)
		{
			...
			//retcode= SQLTransact(SQL_NULL_HENV, g_hDBC_DragonDB, SQL_COMMIT);
...
int updateCharacterStatus(t_connection c[], int char_id)
{
	...
	sprintf(query,
		...
		"gender=%d,sprno=%d,face=%d "		//050413_KCH 성전환 아이템때문에 추가.
		"WHERE name= '%s'",
		chr->GetLevel(),
		chr->GetGuildCode(),
		chr->social_status,
...
{
	t_packet p;
	ZeroMemory(&p.u.update_char_db, sizeof(p.u.update_char_db));	//050413_KCH 초기화
...
SkillMain SkillTbl[MAX_SKILLMAIN] = { 0, };	//050329_KCH 초기화(DragonDB의 skillmain자체에 빈idx가 존재하기 때문에 꼭필요);
...
int UpdateCharStatusByKein(k_char_update_data *ch)		// 0410 YGI
{
	...
	sprintf(szQuerry, "UPDATE chr_info2 SET god_cast_level=%d, believe_god=%d, faith=%d "
		" WHERE name='%s'",
		ch->god_cast_level, ch->believe_god, ch->faith,
		ch->name);
...
{//021113 lsw
	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		// YGI-040510
		extern bool EncodeByKey(WORD key, char *filename, void *pData, int size);
		EncodeByKey(MAP_INFO_MYCODE, "./output/map_info.bin", map, sizeof(t_MapInfo)*MAX_MAP_);
...
{	//< CSD-030326
	char szQuerry[512];
	sprintf(szQuerry, "SELECT code, active, first, mark_num, make, info_num, name,"
		" degree0, degree1, degree2, degree3, degree4, nation FROM guild_list ORDER BY code");
	...
	while (SQLOK(retCode))
	{
		...
		//pGuild->info_num,
		//pGuild->guild_name,
		//pGuild->guild_degree[0],
		//pGuild->guild_degree[1],
		//pGuild->guild_degree[2],
		//pGuild->guild_degree[3],
		//pGuild->guild_degree[4],
		//pGuild->nation,
		//nkkk++);
		//MyLog(LOG_NORMAL, "\n\t050222_KCH Test\n%s\n",strTemp);
...
int GetRowLineOfSQL(const DBType _DBType, const char *table, const char *field, int *RowCount, const char *condition)
{
	...
	switch (_DBType) //050224_KCH 마일리지몰 작업
	{
	case DRAGON_DB:
		SQLAllocStmt(g_hDBC_DragonDB, &hStmt);
		break;
	case TOTAL_DB:
		...
		break;
	default:
#ifdef _ASSERTBADDATA
		Dbg_Assert(0 || "GetRowLineOfSQL() Don't Know _DBType");
#endif
		break;
	}
...
{	//< CSD-030517
	...
	for (int i = 0; i < MAX_GAMBLE_ITEM_KIND; i++)
	{
		for (int j = 0; j < MAX_GAMBLE_ITEM_TYPE; j++)
		{
			...
			size = 0;
			GetRowLineOfSQL(DRAGON_DB, "gamble_item", "house", &size, szQuerry);
...
int LoadNationItem(int week)
{
	...
	GetRowLineOfSQL(DRAGON_DB, "nation_item", "item_no", &row, szQuerry);
	if (row <= 0) return 0;
...
void InitItemLog(int port)
{
	...
	if (GetOwnPort() == port)
	{
		...
		Querry_SQL(qurry, g_hDBC_ChrLogDB);
	}
}
int SaveItemLog(k_item_log *pData)
{
	if (!g_hDBC_ChrLogDB) return -100;		// 연결 되어 있지 않은 상태이다.
	...
	sprintf(szQuerry, "INSERT INTO log_item_event VALUES ( %d, %d, %d, %d, %d, %d, %d, "
		...
		GetServerSetNum());
	return SQLOK(Querry_SQL(szQuerry, g_hDBC_ChrLogDB));
}
...
int Init_ChrLogDB_SQL(LPSTR szDSN, const char* szUID, const char* szUPW)
{
	...
	SQLAllocEnv(&g_hEnv_ChrLogDB);
	SQLAllocConnect(g_hEnv_ChrLogDB, &g_hDBC_ChrLogDB);

	retCode = SQLConnect(g_hDBC_ChrLogDB, (UCHAR *)szDSN, SQL_NTS, (UCHAR *)szUID, SQL_NTS, (UCHAR *)szUPW, SQL_NTS);
...
int LoadDungeonMap()
{
	...
	GetRowLineOfSQL(DRAGON_DB, "scenario_dungeon_map", "*", &nRow, szQuerry);
	if (nRow <= 0) return 0;
...
int CEventMgr::LoadNpcItem()
{
	...
	GetRowLineOfSQL(DRAGON_DB, "Event_MonsterItem", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadNpcCreate()
{
	...
	int row = 0;
	sprintf(szQuerry, "map = '%s'", MapName);
	GetRowLineOfSQL(DRAGON_DB, "Event_Npc", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadEventMap()
{
	...
	int row = 0;
	sprintf(szQuerry, "map = '%s'", MapName);
	GetRowLineOfSQL(DRAGON_DB, "Event_Object", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadEventSound()
{
	...
	GetRowLineOfSQL(DRAGON_DB, "Event_Sound", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadEventMessage()
{
	...
	GetRowLineOfSQL(DRAGON_DB, "Event_Message", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadEventNpcScript()
{
	...
	int row = 0;
	sprintf(szQuerry, "map = '%s'", MapName);
	GetRowLineOfSQL(DRAGON_DB, "Event_NpcItem", "*", &row, szQuerry);
	if (row <= 0) return 0;
...
int CEventMgr::LoadItemBag()
{
	...
	int row = 0;
	SetItemBagCount(0);
	GetRowLineOfSQL(DRAGON_DB, "Event_NpcItem_Bag", "*", &row, NULL);
	if (row <= 0) return 0;
...
int CEventMgr::LoadEventMoveMap()
{
	...
	int row = 0;
	GetRowLineOfSQL(DRAGON_DB, "event_mapmove", "*", &row, NULL);
	if (row <= 0) return 0;
...
{   //< CSD-030522
	...
	g_pRegenManager->Ready(CGroupInfo::ET_NORMAL); // CSD-040310
	return true;
}	//> CSD-030522
...
bool LoadEventTreasureXY()
{
	...
	GetRowLineOfSQL(DRAGON_DB, "Event_treasure_xy", "*", &nRow, szQuerry);
