...
#include "CItem.h"			////050224_KCH 마일리지몰 작업 ItemPosType 구조체
...
extern int SendDeleteItem(ItemAttr *pItem, POS *pos, CHARLIST *ch, int broken = 0);		//050224_KCH 마일리지몰 작업 broken 0:그냥 삭제, 1:내구도감소로삭제, 2:아이템몰사용만기로 삭제.
...
extern void SendPutMenuString(int type, int str_num, short int cn, char* pName = NULL);
...
//>050408_KCH
extern void RecvGetItemMalltoDB(t_packet *p, short int cn);		//050224_KCH 마일리지몰 작업
extern void RecvItemMallItemList(t_packet *p, short int cn);

// 040602-3 YGI
extern void SendItemDropMessage(short int cn, short int item_no);
