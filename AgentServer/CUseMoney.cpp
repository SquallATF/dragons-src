...
int CAsyncClient::AddPayUser( USERINFOPAY& stUser )
{
	...
	USERINFOPAY* it = FindUserIterator(stUser.szUserId);
	if( it != NULL )
	{
		return 1;
	}	
...
//	return NULL;
//}
USERINFOPAY* CAsyncClient::FindUserIterator(const char* szUserName)
{
	LISTUSERIT itFind = find_if(m_vtUser.begin(), m_vtUser.end(), bind2nd(IsUserHere(), szUserName));
	if (itFind != m_vtUser.end())
	{
...
*/
	...
	USERINFOPAY* it = FindUserIterator(szUserName);
	if( it == NULL )
	{
		return;
	}
