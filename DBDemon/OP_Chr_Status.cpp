...
///
int InitPartyMemberServerToServer( CHARLIST *ch );		//  ÀÌ¸§¸¸ µðºñ¿¡ ÀúÀåÇÏ°í ÀÌ¸§¸¸ ²¨³»¿Â´Ù.
...
int increaseAbility(CHARLIST *chr, t_packet *packet)
{
	...
	// Å¬¶óÀÌ¾ðÆ®°¡ ºÄ¿Â Áõ°¡°ªµéÀÇ ÃÑÇÕÀÌ ÇöÀç ³²¾ÆÀÖ´Â ºÐ¹èÁ¡¼öº¸´Ù ¸¹À¸¸é ¿¡·¯
...
int SetItemEmpty( CHARLIST *ch,  int item_no )		// ºó ÀÎº¥Åä¸®¿¡ ¾ÆÀÌÅÛÀ» »ý¼ºÇØ¼­ Áý¾î ³Ö´Â´Ù. //1215
{
...
// ÃÖÃÊ Ä³¸¯ÅÍÀÇ  »ý¼ºÀ§Ä¡, ±âº»¼öÄ¡, ItemµéÀ» SettingÇÑ´Ù. 
...
/////////////////////////////////////////////////////////5/////////////////////////////////////////////////////////////////////
	...
	memset( ch->Ws,			0,	SIZE_OF_WS			);	//	¸¶¹ý
	memset( ch->Ps,			0,	SIZE_OF_PS			);				//	½Å¹ý
...
//	memset( ch->tac_skillEXP,(unsigned int)tac_skillexp, SIZE_OF_TAC_SKILL_EXP );  //coromo Õ½¼¨¾­Ñé
	...
	ch->Job					= p->job;						// 0:Àü»ç 1:±Ã¼ö 2:µµÀû 3:¼ºÁ÷ÀÚ 4:¸¶¹ý»ç
	ch->Class				= p->Class;													
	...
	ch->Condition		=	0;
	strcpy(ch->MapName, "SCHOLIUM" );  //coromo ÐÂ½¨ÈËÎïËùÔÚµØÍ¼
	...
	ch->Money = 1000000;  //coromo ÐÂ½¨ÈËÎïµÄÇ®
	if( ch->Spell == WIZARD_SPELL )
	{
		SetItemEmpty( ch,  10025 );				// ¸¶¹ýÃ¥
	}													// 001215 YGI -----------------------------------------
	SetMySkill( ch );
	...
	ch->IDWhoImAttacked	=	0;	// ³ª¸¦ Ä£³ðÀÌ ´©±¸³Ä..
	ch->nCurrentAction	=	0;
	...
	*/
	...
	ch->viewtype		= 0;
	ch->reserved_point	= 0;  //coromo ÐÂ½¨ÈËÎïÄÜÁ¦µã
	
	//< CSD-011006
	memset(ch->aStepInfo, 1, sizeof(ch->aStepInfo));//ÀüÅõ½ºÅ³ ÃÊ±â¿¡ -1 µÇ¾î ÀÖ´Â ¹ö±×.
	//> CSD-011006
	
	ch->openhouse = -1; // ÃÖÃÊ¿¡ Áý¹Û¿¡¼­ ½ÃÀÛÇÏ¹Ç·Î  ´ÝÇôÀÖ´Â°ÍÀ¸·Î ½ÃÀÛÇØ¾ßÁö...
}
...
// Å¬¶óÀÌ¾ðÆ®°¡ Á¢¼ÓÇÒ¶§, ±âº»Á¤º¸¸¦ Å¬¶óÀÌ¾ðÆ®·Î º¸³»ÁØ´Ù
...
/////////////////////////////////////////////////////////5/////////////////////////////////////////////////////////////////////
	...
	// 0410 YGI
void SendThrowDice( char type, short int cn )//ÄÜÁ¦È·¶¨,TYPEÊÇÈËÎïµÄÀàÐÍ,±»CH->CLASSÈ¡Öµ
	{
