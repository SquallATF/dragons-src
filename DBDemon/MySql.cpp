...
///////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////
char SQLerrorS[200];//定义发送外挂错误的字符串/////////////////////////////////////////////
...
int GetCharacterNameInID(LPSTR id, char name[MAX_CHARACTEROFID][NM_LENGTH])
{
	...
	//sprintf(szQuerry, "select name1, name2, name3, name4 name5, name6, name7, name8 "
	...
	for (int i = 0; i < MAX_CHARACTEROFID; i++)
	{
		...
		if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
		{
			...
			else
			{
				...
				retCode = SQLGetData(hStmt, 1, SQL_C_CHAR, &id, ID_LENGTH, &cbValue);
...
int CreateChar2_SQL(t_connection c[], int cn)		// 0302 YGI
{
	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		//sprintf(szQuerry, "select name1, name2, name3, name4, name5, name6, name7, name8" 
		//						" from chr_log_info where login_id='%s'", c[cn].id);
		/////////////////////////////////////////////////////////////////////////////
		...
		retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
		{
			if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
			{
				...
				if (retCode == SQL_SUCCESS)
				{
					...
					{
						*select_name = 0;
						retCode = SQLGetData(hStmt, i + 1, SQL_C_CHAR, select_name, NM_LENGTH, &cbValue);
...
int DeleteChar_SQL2(const char *szID, const char *szName, const char* szSecretKeyCode)		// 0302 YGI//020725 lsw
{
	...
	SDWORD cbValue;
	char szQuerry[255];
	char log_id[ID_LENGTH] = { 0, };
	char ori_log_id[ID_LENGTH];
	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		if (retCode == SQL_SUCCESS)
		{
			retCode = SQLGetData(hStmt, 1, SQL_C_CHAR, log_id, ID_LENGTH, &cbValue);
		}
	}
	SQLFreeStmt(hStmt, SQL_DROP);
	...
	// login_id를 이용하여 4명의 이름을 읽어 온다.
	//sprintf(szQuerry, "select name1, name2, name3, name4, name5, name6, name7, name8" 
	//						" from chr_log_info where login_id='%s'", log_id);
	/////////////////////////////////////////////////////////////////////////////
...
int update_BinaryData_to_Chr_Info2(UCHAR *bankitem, char *login_id, char *name) // 1225
{
	...
	if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
	{
		...
		while (retcode == SQL_NEED_DATA)
		{
			...
			if (retcode == SQL_NEED_DATA)
			{
				if (pParamData == bankitem)
					SQLPutData(hstmt, bankitem, SIZE_OF_BANKITEM);
			}
			else break;
		}
...
// 있으면 
void RecvIsThereCharName(int cn, char *name)//注册时人物名检测
{
	t_packet p;
	/////////////////截获非法字符/////////////////////////////	
	strcpy(s1, name);	//公会名称
...
int SaveEventItem(char *name, int item_no, int count)
{
	...
	sprintf(szQuerry, " CREATE TABLE [dbo].[%s] (	"
	...
	if (retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		sprintf(szQuerry, "ALTER TABLE [dbo].[%s] WITH NOCHECK ADD"
...
bool isUserLockd(const char* szID) // 050818 天机
{
