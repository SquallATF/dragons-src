...
//  ÀÌ¹ø GetCharGameDB_SQL()´Â °ÔÀÓ Server¿¡¼­¸¸ »ç¿ëµÇ´Â º¯¼ö¸¦ ÀÐ±âÀ§ÇÑ°ÍÀÌ´Ù. 
...
/////////////////////////////////////////////////////////5/////////////////////////////////////////////////////////////////////
	...
	str="·¢ÏÖSQLÂ©¶´¹¥»÷!·Ç·¨ÄÚÈÝ:[%s],  ×¢²áÈËÎïÃû³Æ:[%s]";
	...
	if(i==0)
	{
	sprintf(SQLerrorS,str,cheak_name,cheak_name);
	HackLog(0,SQLerrorS);
	}
	cheak_name[i] = 32;
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	CreateCharacter( c, cn, packet); // ÃÖÃÊ Ä³¸¯ÅÍÀÇ  »ý¼ºÀ§Ä¡, ±âº»¼öÄ¡, ItemµéÀ» SettingÇÑ´Ù. NPC_Pattern.cpp
	
	total_id = GetCharID_SQL();
	if( total_id == 0 )  return -1; // Total ID¸¦ ÇÒ´ç¹ÞÁö ¸øÇß¾î¿ä..
	...
	if(retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
		...
		// Ä³¸¯ÅÍÀÇ Binary Data¸¦ chr_info¿¡ »ý¼ºÇÏ°í ( 1-2´Ü°è )
		// 0218 YGI #######################
		...
		if( ret == 1 ) 
		{
			// Ä³¸¯ÅÍÀÇ ÀÏ¹Ý Data¸¦ chr_info2¿¡ »ý¼ºÇÏ°í ( 2-1´Ü°è )
			ret = CreateChar2_SQL(c, cn);
			if( ret == 1 ) // ¼º°ø..
			{
				// Ä³¸¯ÅÍÀÇ ÀÏ¹Ý Data¸¦ chr_info2¿¡ »ý¼ºÇÏ°í ( 2-1´Ü°è ) -- Character»ý¼º ³¡.
...
int delete_char_create_fail( LPSTR name )		// 0411_2 YGI		// ÀÌ°Ç Àß¸ø »ý¼ºµÈ °æ¿ì »èÁ¦ÇÏ´Â ÇÔ¼ö
{	
	HSTMT		hStmt = NULL;
	RETCODE		retCode;
	char		szQuerry[255];
...
int CreateChar2_SQL(t_connection c[], int cn )		// 0302 YGI
{
	...
	{			
		...
		// login_id¸¦ ÀÌ¿ëÇÏ¿© 4¸íÀÇ ÀÌ¸§À» ÀÐ¾î ¿Â´Ù.
		...
		{			
			...
			{		
				...
				{	
					...
					{
						SQLFreeStmt(hStmt, SQL_DROP);//Áö¿ö¾ß ÇÒ ³ðÀÌ´Ù
						...
						{//Å°ÄÚµå°¡ Æ²·È±â ¶§¹®ÀÌ´Ù Å°ÄÚµå Ã¼Å© ÇÒ¶§ SQLÀº ¸ðµÎ FreeÇßÀ¸´Ï Áö±Ý ÇÁ¸® ÇÏÁö ¾Ê°í ¸®ÅÏ ÇÑ´Ù
							return 0;
						}
					}
				}	
...
/////////////////////////////////////////////////////////////////////////////
//	
// ÇÔ¼ö¼³¸í: Ä³¸¯ÅÍÀÇ binary(Å©±â°¡ Å«¹è¿­µéÀº binary·Î ÀúÀåÇÏ°í binary·ÎÀÐ¾î¿Â´Ù)¸¦ ÀÐ¾î¿À´Â ÇÔ¼ö.
//	
...
int CheckUsedID_SQL( LPSTR szUID, WORD *wPort, DWORD *dwID, DWORD *dwServerSetNum )
{
	...
	// 0403 ÇöÀç ÁÁÀº LogIn/Out¿¡ °üÇÑ ¾Ë°í¸®Áò »ý°¢Áß..........
	// ´çºÐ°£ »ç¿ëÇÏÁö ¾Ê´Â´Ù...................................
...
BOOL			bAlive;
  ...
  char			Name[20];
	...
	DWORD			Exp;
	  ...
	  short int		Class;						// 0:Àü»ç 1:±Ã¼ö 2:µµÀû 3:¼ºÁ÷ÀÚ 4:¸¶¹ý»ç
	  ...
	  short int		Spell;
		...
		short int		Race;						// Á¾Á·
		  ...
		  //ÃÖÃÊÄ³¸¯ÅÍ »ý¼ºÇÒ¶§ÀÇ ¼öÄ¡..
		  short int		StrOld;				//  Èû
		  short int		ConOld;				//  °Ç°­
		  short int		DexOld;				//	¹ÎÃ¸¼º
		  short int		WisOld;				//	ÁöÇý
			...
			short int		Luck;				//	Çà¿î
			  ...
			  short int		Job;
				...
				DWORD			Money;
				  ...
				  short int		VitalPower;					// »ý¸í·Â
				  short int		VitalMax;
				  short int		Mana;					// ¸¶¹ý·Â
				  short int		ManaMax;
				  short int		AttackPower;				// °ø°Ý·Â
				  short int		AttackMax;
										...
										//short int		Item[ 13][100];
										  ...
										  //  ClientÀ» À§ÇÑ Data
											...
											short int		SprType;	
											  ...
											  BYTE			accessory[4];
											  DIRECTION		Direction;
											  DWORD			IDWhoImAttacked;	// ³ª¸¦ Ä£³ðÀÌ ´©±¸³Ä..
...
{
	// acer5 -------------
	int c_check = AddCRC( p, sizeof( t_update_char_db)-4 - (sizeof(short int) * 3), 10 );
								  ...
								  sprintf(szQuerry, "select item_%02d from box_item where id=%d", pos+1, id);
									...
									SQLFreeStmt(hStmt, SQL_DROP);
									  ...
									  */
									  ...
									  //ÇùÅë½Ã½ºÅÛÀÇ ºÎºÐÀ¸·Î Party, Relation, EmploymentÀÇ Data¸¦ÀÐ¾î¿Â´Ù. 
									  ...
									  void RecvIsThereCharName( int cn, char *name )//×¢²áÊ±ÈËÎïÃû¼ì²â
									  {
										...
										str="·¢ÏÖSQLÂ©¶´¹¥»÷!·Ç·¨ÄÚÈÝ:[%s],  ÉêÇëÈËÎïÃû³Æ:[%s]";
										...
										if(i==0)
										{
										sprintf(SQLerrorS,str,name,name);
										HackLog(0,SQLerrorS);
										}
										name[i] = 32;
										}
...
//////////////////////////////////////////////////////////////// 0208 YGI
//
// ¹®ÀÚ¿­Áß Æ¯Á¤ ¹®ÀÚ¿­¿¡ Æ÷ÇÔÇÑ ¹®ÀÚ°¡ ÀÖ´ÂÁö¸¦ Ã£´Â ¸Þ¼Òµå 
//
...
int UpdateEvaName( const char *my_name, char *eva_name )		// ÀüµµÇÑ »ç¶÷ ÀÌ¸§À» µðºñ¿¡ ¾÷µ¥ÀÌÆ® ÇÑ´Ù. // 0405 YGI//020725 lsw
{
	return 0;											// 010414 YGI
	HSTMT		hStmt = NULL;
	RETCODE		retCode;
...
//////////////////////////////////////////////////////////////////////////////////////
// vote Å×ÀÌºí Á¦¾î ¹®
// 1: ·¹ÄÚµå »ðÀÔ( ÈÄº¸µî·Ï )	- ±âÁ¸¿¡ ÈÄº¸°¡ ÀÖ´ÂÁö Ã¼Å©ÇØ¼­ ÈÄº¸¹øÈ£¸¦ ºÎ¿©ÇÑ´Ù.
// 2: ÈÄº¸ Á¤º¸ °¡Á®¿À±â		- ±æµå, ³ª¶ó, ¼ºº°, ·¹º§, Å¬·¹½º¸¦ °¡Á®¿Â´Ù
// 3: ¼±°Å ÁøÇà»óÈ²	- ¼±°Å ¹øÈ£¸¦ °¡Áö°í ÀÖ´Â ¸ðµç ÈÄº¸µéÀÇ ÇöÀç µæÇ¥°ªÀ» °¡Á®¿Â´Ù
...
// 2: ¿ÕÀÌ µÉ ³ðÀÌ Á¢¼ÓÇØ ÀÖ´ÂÁö¸¦ È®ÀÎ( »«´Ù.. ¾ÆÀÌµð¸¦ °¡Á®¿À´Âµ¥ ¹®Á¦°¡ »ý±æ È®·üÀÌ ÀÖÀ¸´Ï±ñ ±×³É °ÔÀÓ ¼­¹ö·Î ¸ðµÎ º¸³½´Ù.
...
// 3 : ÀÏ´Ü °ÔÀÓ ¼­¹ö·Î º¸³½ÈÄ¿¡ ±×³ð Ä³¸¯ÅÍ¿¡ ¾÷µ¥ÀÌÆ®¸¦ ÀÏ´Ü ÇÑ´Ù.
...
// 5: µðºñ Å×ÀÌºíÀ» Á¤¸®ÇÑ´Ù. ( ¿Õµî·Ï¹× ÈÄº¸ »èÁ¦ )
...
{//021108 lsw
	...
	"È°Á¦ÀÛ»ç", "¿¬±Ý¼ú»ç", "ÃÊÀïÀÌ",	"»óÀÎ",		"´ëÀåÀåÀÌ±â¼º",
...
////////////////////////////////////////////////////
//
//	°³ÇÐ ÀÌº¥Æ®¸¦ À§ÇÑ Å×ÀÌºí ÂüÁ¶ openschool
//
...
// 010310 YGI
int GetChocolateEatName( char *name, DWORD total_id )		// total_id·Î ¸ÔÀº ³ð Ã£±â
{
	HSTMT		hStmt = NULL;
	RETCODE		retCode;
	SDWORD		cbValue;
...
{	//< CSD-030423
	...
	sprintf(szQuerry, "SELECT top 1 code FROM guild_list ORDER BY code DESC " );		// ÇöÀç µðºñ¿¡ ÀúÀåµÈ ±æµå ¼ö
	SQLAllocStmt(hDBC, &hStmt);
	retCode = SQLExecDirect(hStmt, (UCHAR *)szQuerry, SQL_NTS);
	if(retCode == SQL_SUCCESS || retCode == SQL_SUCCESS_WITH_INFO)
	{
...
//021030 YGI
// ÀÌº¥Æ® Å×ÀÌºíÀ» ÂüÁ¶ ÇØ¼­ ¾ÆÀÌÅÛÀ» ¹ÞÀ»°Ô ÀÖ´ÂÁö¸¦ Ã¼Å©ÇÑ´Ù.
...
int CheckEventITem( CHARLIST *ch )
{
	...
	int count = 0;	// ¹ÞÀºÀûÀÌ ÀÖ´ÂÁö È®ÀÎÇÏ´Â ·çÆ¾¿¡¼­ »ç¿ë
...
int AddEventItem( CHARLIST *ch, k_startitem_basic* pData )
{
	...
	if( !pData->m_bAll )
	{
		...
		if( pData->m_szTable[0] )
		{
			int nRowCount = 0;
			char szQuerry[512];
			sprintf( szQuerry, "table_name = '%s' AND event_no = %d", connections[ch->server_id].id, pData->m_nEventNo );
			GetRowLineOfSQL( "Event_StartItem", "*", &nRowCount, szQuerry );
			//coromo eventstartitem
	...
	//coromo eventstartitem
	sprintf( query, "DELETE FROM Event_StartItem WHERE event_no = %d", pData->m_nEventNo);
	Querry_SQL( query );
	add_item = 1;
	//coromo eventstartitem
	return add_item;
...
// 030221 YGI
// ¿ÜºÎ¿¡¼­ º¸³½ ¸ÞÀÏ¸¦ ¹Þ´Â »ç¶÷ ÀÌ¸§À» °¡Á®¿Â´Ù. °¡Á®¿Â ÀÌ¸§Àº Áö¿î´Ù.
...
// NPC·¹º§°ú °æÇèÄ¡ °ü·Ã Á¤º¸¸¦ ´ã°í ÀÖ´Â Å×ÀÌºíÀ» DB¿¡¼­ ÀÐ¾î¿À´Â ÇÔ¼ö
int LoadNPCLevTable()
{	//< CSD-030306
	if(Num_Of_NPC_Lev <= 0) 
	{
...
{	
	...
	SDWORD		cbValue=0;
	char		szQuerry[255];
	sprintf( szQuerry, "SELECT islocked FROM chr_log_info WHERE login_id = '%s' ", szID );//020823 lsw
	...
	{
		retCode = SQLFetch(hStmt);
		retCode = SQLGetData(hStmt, 1, SQL_C_LONG, &isLocked, 0, &cbValue);		
	}
	else
	{		
		MyLog( LOG_NORMAL, "Failed Get User Lockinfo chr_log_info in TotalDB (User Name -->'%s')", szID );		
	}
