...
/////////////////////////로그인 메뉴/////////////////////////////////////////////////
	...
	// 에디트 박스의 글씨를 출력만..
	::SMenuFTReg(MN_LOGIN, 3, FT_SPECIAL_WILLDO, SWD_IDEDITBOX, 232,328, 300, 20, MAINSTART_LOGIN_IMG, 7, 7 ,0);
	::SMenuFTReg(MN_LOGIN, 5, FT_DATA_PUT, 0, 338,332 ,0,0,MAINSTART_LOGIN_IMG, 0,DP_IDEDITBOX,0);
	
	::SMenuFTReg(MN_LOGIN, 4, FT_SPECIAL_WILLDO, SWD_PASSEDITBOX, 232,367, 300, 20, MAINSTART_LOGIN_IMG, 8, 8 ,0);
	::SMenuFTReg(MN_LOGIN, 6, FT_DATA_PUT, 0, 338,372, 100, 20, 0, 0, DP_PASSEDITBOX,0);

	//필드 번호 바꾸지 말것 엔터키 안먹음 로그인 하기
	::SMenuFTReg(MN_LOGIN,0,FT_SPECIAL_WILLDO,SWD_LOGON,364,399,75,30,MAINSTART_LOGIN_IMG,1,1,MN_SELECT_CHARACTER);//로그인
	...
	//EXIT버튼 선택시 
	::SMenuFTReg(MN_LOGIN,1,FT_SPECIAL_WILLDO,SWD_QUIT,364,526,75,27,MAINSTART_LOGIN_IMG,4,4,0);
	//NewId버튼 선택시
...
//	인물 선택
	...
	// add by taniey
	::SMenuFTReg(MN_SELECT_CHARACTER, 12, FT_HIDE_SPECIAL_WILLDO_AUTO_PUT, SWD_CHARACTER_SCROLL_LEFT,
		333, 480, 30, 16, SELECT_CHARACTER_IMG, 40, 0);	//
	::SMenuFTReg(MN_SELECT_CHARACTER, 13, FT_HIDE_SPECIAL_WILLDO_AUTO_PUT, SWD_CHARACTER_SCROLL_RIGHT,
		423, 480, 30, 16, SELECT_CHARACTER_IMG, 41, 1);		//

	//// set the character to left roll
	//SMenu[MN_SELECT_CHARACTER].nField[12].nType = FT_SELECT_CHARACTER_SCROLL;    // FT_SELECT_CHARACTER_SCROLL
	//SMenu[MN_SELECT_CHARACTER].nField[12].nSpecialWillDo = SWD_CHARACTER_SCROLL_LEFT;
	//SMenu[MN_SELECT_CHARACTER].nField[12].nWillDo = 0;
	//SMenu[MN_SELECT_CHARACTER].nField[12].x = 333;
	//SMenu[MN_SELECT_CHARACTER].nField[12].y = 480;
	//SMenu[MN_SELECT_CHARACTER].nField[12].nRectImage = 16;
	//SMenu[MN_SELECT_CHARACTER].nField[12].nImageNumber = 16;
	//SMenu[MN_SELECT_CHARACTER].nField[12].nImageType = SELECT_CHARACTER_IMG;
	//::SetRect(SMenu[MN_SELECT_CHARACTER].nField[12].rCheakBox, 333, 480, 360, 496);

	//// set the character to right roll
	//SMenu[MN_SELECT_CHARACTER].nField[13].nType = FT_SELECT_CHARACTER_SCROLL;    // FT_SELECT_CHARACTER_SCROLL
	//SMenu[MN_SELECT_CHARACTER].nField[13].nSpecialWillDo = SWD_CHARACTER_SCROLL_RIGHT;
	//SMenu[MN_SELECT_CHARACTER].nField[13].nWillDo = 1;
	//SMenu[MN_SELECT_CHARACTER].nField[13].x = 423;
	//SMenu[MN_SELECT_CHARACTER].nField[13].y = 480;
	//SMenu[MN_SELECT_CHARACTER].nField[13].nRectImage = 16;
	//SMenu[MN_SELECT_CHARACTER].nField[13].nImageNumber = 16;
	//SMenu[MN_SELECT_CHARACTER].nField[13].nImageType = SELECT_CHARACTER_IMG;
	//::SetRect(SMenu[MN_SELECT_CHARACTER].nField[13].rCheakBox, 423, 480, 453, 496);
...
//15 259
	...
	//메인 인터페이스 메뉴
	{
	::SMenuReg(MN_MAININTERFACE, 0, 371+GABY_SCREEN*2, MAININTERFACE_IMG,0,50,0);
	//메인 메뉴 뛰우기	OK
	SMenu[MN_MAININTERFACE].bActive=TRUE;
	//스테이터스 메뉴 뛰우기 OK
...
// 메인 메뉴 설명 메뉴 
	...
	*/
	//<< 031021 kyo
	::SMenuReg(MN_FACE, 98, 300+GABY_SCREEN*2, NEW_EMOTE_IMG,0,22,0);
...
//> BBD 040112 에이서 이모티콘 변경
	...
	::SMenuFTReg(MN_WISPER_LIST,SELECT_WISPER_MODE,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*2,130,18,	MAININTERFACE_IMG,78,77,SELECT_WISPER_MODE);
	strcpy(SMenu[MN_WISPER_LIST].nField[SELECT_WISPER_MODE].temp,lan->OutputMessage(2,627));//귓속말 상대입력
	::SMenuFTReg(MN_WISPER_LIST,CHAT_TARGET_WISPER1		,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*3,130,18,	MAININTERFACE_IMG,78,77,CHAT_TARGET_WISPER1		);
	::SMenuFTReg(MN_WISPER_LIST,CHAT_TARGET_WISPER2		,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*4,130,18,	MAININTERFACE_IMG,78,77,CHAT_TARGET_WISPER2		);
	::SMenuFTReg(MN_WISPER_LIST,CHAT_TARGET_WISPER3		,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*5,130,18,	MAININTERFACE_IMG,78,77,CHAT_TARGET_WISPER3		);
	::SMenuFTReg(MN_WISPER_LIST,CHAT_TARGET_WISPER4		,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*6,130,18,	MAININTERFACE_IMG,78,77,CHAT_TARGET_WISPER4		);
	::SMenuFTReg(MN_WISPER_LIST,CHAT_TARGET_WISPER5		,FT_SELECT_WISPER,CHATTYPE_WHISPER	,8,	8-18*7,130,18,	MAININTERFACE_IMG,78,77,CHAT_TARGET_WISPER5		);
...
//		::SMenuReg(MN_CHATTING, 10, 355, 0,0,10,0);
		::SMenuReg(MN_CHATTING, 0, 396, NEW_CHAT_BACK_IMG,0,10,0);
		...
		//대화 찍힘
		::SMenuFTReg(MN_CHATTING,6,FT_DO,0,22,-55,0,0,NEW_CHAT_BACK_IMG,0,0,DO_CHATTING_VIEW);
	...
	case DO_CHATTING_EDITBOX :	
		{
			...
			else//채팅 가능 모드
			{
				...
				{
					if((cursor%=12) < 6){Hprint2( iMainX+iFieldX+58+2 , iMainY+iFieldY+4+2, g_DestBackBuf, "%s_",wisper_target );}
					else {	Hprint2( iMainX+iFieldX+58+2 , iMainY+iFieldY+4+2, g_DestBackBuf, "%s",wisper_target );}
					Hprint2( iMainX+iFieldX+58+2 +125, iMainY+iFieldY+4+2, g_DestBackBuf, "%s",temp );
				}
				if(EWndMgr.IsFocus(HWND_CHAT_INPUT))
				{
					if((cursor%=12) < 6){Hprint2( iMainX+iFieldX+58+2 +125, iMainY+iFieldY+4+2, g_DestBackBuf, "%s_",temp );}
					else {	Hprint2( iMainX+iFieldX+58+2 +125, iMainY+iFieldY+4+2, g_DestBackBuf, "%s",temp );}
	...
	case FT_DISPLAY_TAC_EXP:
		{
			...
			Hprint2( iMainX+iFieldX+25, iMainY+iFieldY+2, g_DestBackBuf, explain_tactics[SCharacterData.tatics-88] );
		}break;
	case FT_DISPLAY_MAGIC_EXP:
		{
			...
			DisplayTacGauge(SubMenu,j,iMainX+iFieldX,iMainY+iFieldY,next,exp,lv,refresh,first_exp);
			SetHangulAlign( TA_LEFT );
			Hprint2( iMainX+iFieldX+25, iMainY+iFieldY+2, g_DestBackBuf, explain_tactics[tac] );
		}break;
	case FT_DISPLAY_SKILL_EXP:
		{
			...
			DisplayTacGauge(SubMenu,j,iMainX+iFieldX,iMainY+iFieldY,next,exp,lv,refresh,first_exp);
			SetHangulAlign( TA_LEFT );
			Hprint2( iMainX+iFieldX+25, iMainY+iFieldY+2, g_DestBackBuf, skill[job].han_name );
		}break;
...
//> BBD 040303
	...
	case FT_QUICK_MAGIC_PLUS : 
	{
		...
		else
		{
			...
			magic_x = 379 - 22 * (litter_num - 1);
			::SetRect(rect, magic_x, -52, magic_x + litter_num * 44, 0);
			SMenu[MN_QUICK_MAGIC_SCROLL].bActive = magic_plus_count <= SM_MAGIC_COUNT ? false : true;

			SMenu[MN_QUICK_MAGIC_SCROLL].x = iMainX + 393 - 22 * (litter_num + 1);
			SMenu[MN_QUICK_MAGIC_SCROLL].y = iMainY - 53;

			//FieldTypeNomalPut( iMainX, iMainY,  423+22*4, -52, 22, MAIN_ETC );
			SMenu[MN_TIME_DISPLAY_ON_OFF].x = iMainX + 423 + 22 * (litter_num - 1);
			SMenu[MN_TIME_DISPLAY_ON_OFF].y = iMainY - 52;
		}
	...
	case FT_EXCHANGE_ITEM_ETC:	
		{	// 인벤 창 이외의 메뉴는 다 없애버린다.
			...
			SetHangulAlign( TA_CENTER );			
			::Hprint2( iMainX+306+112/2,iMainY+65, g_DestBackBuf, SCharacterData.sCharacterName );//자신 이름
			::Hprint2( iMainX+23+112/2,iMainY+65, g_DestBackBuf, ExchangeMgr.GetExchangeTargetName() );//교환상대의 이름
		}break;
	case FT_MAN_TO_MAN_INFO_DISPLAY:
		{
			...
			if(LEFT== nWillDo)
			{
				...
				::Hprint2( iMainX+iFieldX+50,iMainY+64, g_DestBackBuf, ch->name );//자리 보정
			}
			else
			{//if(!ch)break; 를 달아 줘야 하지만 오른쪽일 경우는 Hero다 없으면 그냥 튕겨줘
				::Hprint2( iMainX+iFieldX+125,iMainY+64, g_DestBackBuf, ch->name );//자리 보정
			}
			::SetHangulAlign( TA_LEFT );
	...
	case FT_GM_ITEM_EXPLAIN : 
		{
			...
			SetHangulAlign( TA_CENTER );
			int gaby = 0;
			Hprint2( iMainX+137, iMainY+127+gaby*18, g_DestBackBuf, g_GM_Item.GetName(para) );gaby++;
			SetHangulAlign( TA_LEFT);
			Hcolor( FONT_COLOR_NUMBER );
	...
	case FT_MAIL_LIST:
		{	//타입, 보낸사람, 제목, 날짜
			for(int i = 0; MAX_MAIL_PAGE > i; i++)
			{
				...
				::Hprint2(iX+53,iY,g_DestBackBuf,(char*)szSender);
				...
				szPress[20] = '\0';
				::Hprint2(iX+162,iY,g_DestBackBuf,szPress);

				Hcolor(wDefaultColor);
				char szTime[MAX_PATH]= {0,};
				g_MailMgr.GetMailDate(i,false,szTime);
				::Hprint2(iX+299,iY,g_DestBackBuf,szTime);
			}
		}break;
	...
	case FT_VIEW_BATTLE_WAIT_MEMBER: 
		{	//< CSD-030521
			...
			for (CArenaLobby::ITOR_MEMBER i = ltLobby.begin(); i != ltLobby.end(); ++i)
			{
				...
				::Hprint2(iMainX+iFieldX+(ct%4)*100, iMainY+iFieldY+(ct/4)*18, g_DestBackBuf, pName);
				++ct;
			}
...
void lsw_FT_DATA_PUT(SMENU *SubMenu ,int j)
{
	...
	//			::PutItemIcon(iMainX+iFieldX ,iMainY+iFieldY+ 10, gm_make_item_result.item_no);	
			...
			int iPerpectMuch = ItemGrade[iGrade].ResourceAmount[iUpgradeType];
			...
			if( iTargetItemCount > iPerpectMuch || iTargetItemCount == 0)
			{
				Hcolor( FONT_COLOR_PLUS );
			}
			...
			{
				fSuccessRate = float(iTargetItemCount) / float(iPerpectMuch);
	...
	case	DP_RARE_SUCCESS_RATE_DISPLAY_NO_TOTAL:
		{
			...
			if( JoinNo && SkillNo && sno && item_select_result.item_no)
			{
				...
				if(iFuncItem)
				{
					...
					ItemFunction[iFuncItem].NameAdd);
					break;
				}
	...
	case DP_HP_BAR :	
		{
			...
			SetHangulAlign( TA_CENTER );
			Hcolor( FONT_COLOR_WHITE );
			Hprint2( iMainX+iFieldX+30, iMainY+iFieldY+HPMPBarHeight, g_DestBackBuf, "%d",  SCharacterData.nCharacterHP);
		}break;
	case DP_HP :
		{	
...
bool DoButtonCheckOfMenu_lsw( int i, int j )
{
  ...
  case	FT_RARE_GROUP_SCROLL://startpoint 값을 주어서 스크롤
	...
	case FT_SELECT_CHAT_TYPE:
		{//nWillDo를 선택
			if(GetChatTarget())//MN_WISPER_LIST에 따라서 ?히는게 다르다
			{
				SetChatTarget(CHAT_TARGET_NORMAL);//보통 채팅 으로 전환
			}
			else
			{
...
// 퀘스트 메뉴
	...
	SMenu[MN_QUEST].nField[0].x=51;
	SMenu[MN_QUEST].nField[0].y=193;
	SMenu[MN_QUEST].nField[0].nRectImage=5;
	SMenu[MN_QUEST].nField[0].nImageNumber=5;
	::SetRect(SMenu[MN_QUEST].nField[0].rCheakBox,51,193,50+35,193+15);
...
/////////////////////////////////////////////////////////////////	
	...
	SMenu[MN_QUEST_INDEX].nField[0].y=66;
	...
	SMenu[MN_QUEST_INDEX].nField[1].y=66;
...
void DisplayRareExplainSub(const int x,const int y,int &iCount , int &iReultLineCount, int &iBlk,const int iLGab, const int  iWidth,const RareMain *pRareValue,const int ct,const WORD wForceColor)
{
	...
	else
	{
		...
		::Hprint2(x+5,y+iBlk+iLGab*iCount,g_DestBackBuf,pIR->ExplainAdd);//설명만
		iCount += iReultLineCount;
	}
}
...
void DisplayGmRegistRequest(const int iGmQuestIndex, const int iMainX, const int iMainY)
{
	...
	::SetHangulAlign( TA_CENTER );
	int iLineCt = 0;
	::Hprint2( iMainX+137, iMainY+127+iLineCt*18, g_DestBackBuf, pGmMain->name );
	iLineCt++;		// 직업 이름 출력
