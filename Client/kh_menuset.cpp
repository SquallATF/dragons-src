...
void StartMenuSet2()
{
	...
	}
	{
		...
		// left and right character scroll , add by taniey
...
{
	{
		...
		//무기 장착 화면 띄우기
		// SMenu[MN_ITEM].nField[7].nImageNumber=120;  //menu.cpp에서 이미지 바꾸는 루틴 존재함.
...
void lsw_MenuSubProcessType(SMENU *SubMenu, const int j)
{
	...
	case FT_SELECT_OBSERVER_TARGET_TEAM: {	//< CSD-030521
	...
	case FT_FIGHTMAP_START: {	//< CSD-030521
	...
	case FT_VIEW_BATTLE_MEMBER: {	//< CSD-030521
	...
	case FT_VIEW_BATTLE_WAIT_MEMBER: {	//< CSD-030521
		int ct = 0;
...
void lsw_FT_DATA_PUT(SMENU *SubMenu, int j)
{
	...
	{	//결과물 아이템 뽑는 루틴//결과물 아이템의 아이템 이름과 레어만 찍음
		...
		if (pTempRare1->IsDynamicRare
			...
			|| pTempRare2->iHighLevel) {
			break;
		}//하이나 기능 아템은 출력 안시켜.. 틀린거 거든
