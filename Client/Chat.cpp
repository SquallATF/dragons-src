...
void CChatMgr::AddString(const char* szName, const char* szChatData, const int iChatType)//채팅 데이타 추가
{
	...
	case CHATTYPE_GUILD:
	{
		...
		InputData->SetHeader(FONT_COLOR_RARE_MAIN, lan->OutputMessage(0, 601));// "길드");
	}break;//귓말은 이제부터옥색
	case CHATTYPE_COLOSSUS_GLOBAL:
	{
		...
		InputData->SetHeader(FONT_COLOR_BLIGHT_VIOLET, lan->OutputMessage(0, 602));//"아레나1"
	}break;
	case CHATTYPE_COLOSSUS:
	{
		InputData->SetChatType(iChatType, ++m_iTotalCounter, ++m_iColosussCounter);
		InputData->SetData(FONT_COLOR_BLIGHT_VIOLET, szChatData);
		InputData->SetHeader(FONT_COLOR_BLIGHT_VIOLET, lan->OutputMessage(0, 603));//"아레나2"
	}break;
	case CHATTYPE_WHISPER:
	{
		...
		InputData->SetHeader(FONT_COLOR_GREEN, lan->OutputMessage(0, 604));//"귓말"
	}break;//귓말은 이제부터옥색
	case CHATTYPE_LOCAL_WAR:
	{
		...
		InputData->SetHeader(FONT_COLOR_BLIGHT_ORANGE, lan->OutputMessage(0, 605));//"국지전"
	}break;
	case CHATTYPE_NATION_WAR:
	{
		InputData->SetChatType(iChatType, ++m_iTotalCounter, ++m_iNationWarCounter);
		InputData->SetData(FONT_COLOR_BLIGHT_ORANGE, szChatData);
		InputData->SetHeader(FONT_COLOR_BLIGHT_ORANGE, lan->OutputMessage(0, 606));//"국가전"
	}break;
	case CHATTYPE_PARTY:
	{
		InputData->SetChatType(iChatType, ++m_iTotalCounter, ++m_iPartyCounter);
		InputData->SetData(FONT_COLOR_BORA, szChatData);
		InputData->SetHeader(FONT_COLOR_BORA, lan->OutputMessage(0, 607));//"파티"
	}break;
	...
	case CHATTYPE_GUILD_BBS:
	{
		...
		InputData->SetData(FONT_COLOR_RARE_MAIN, szChatData);
		InputData->SetHeader(FONT_COLOR_RARE_MAIN, lan->OutputMessage(0, 608));//"길드공지"
	}break;
...
void SendChatData(LPSTR lpStr, const int iChatType)
{
	...
	{
		if ((!::_stricmp(lpStr, lan->OutputMessage(5, 53)))
			|| (!::_stricmp(lpStr, "The name of Edelblhoy with cosmos and the great storm")))		// 0521 YGI//010216 lsw
		{
			char type = 1;
			::CallServer(CMD_PLEASE_GIVE_LIFE, (char *)&type, sizeof(char));
		}
		else if (::_stricmp(lpStr, kein_GetMenuString(32)) == 0)		// "붉은 장미와 정의의 오렘의 이름으로"
		{
			char type = 2;
			::CallServer(CMD_PLEASE_GIVE_LIFE, (char *)&type, sizeof(char));
		}
#ifdef _DEBUG
		if (!::_stricmp(lpStr, "goodboy"))
		{
	...
	if (!::_stricmp(lpStr, kein_GetMenuString(33)))		// 마법 배우기
	{
	...
	if (!_stricmp(lpStr, lan->OutputMessage(5, 54)))
	{
		const int mx = Hero->x / TILE_SIZE;
		const int my = Hero->y / TILE_SIZE;
...
{	//< CSD-030520		
	...
	case CHATTYPE_NORMAL:
	{
		...
		if (Hero->viewtype != VIEWTYPE_GHOST_)//나는 고스트가 아니고		(난 사람)
		{
			...
			{
				if (!CanViewGhost(Hero)) //내가 고스르? 볼 수 없을때				(내가 유령을 못 보면 말도 못해)
				{
					break;
				}
			}
...
void SendPartyChatByLineCommand(char *str_buf)//SendEarMessageByLineCommand이거를 변형 해서 현재 채팅창에서 메세지 가도록
{
...
int CheckLanguageForMenu(LPSTR lpStr) // 메뉴에서 사용할 언어 시스템
{
	...
	{
		if (_stricmp(lpStr, lan->OutputMessage(7, 96)) == 0
			|| _stricmp(lpStr, lan->OutputMessage(7, 97)) == 0
			|| _stricmp(lpStr, lan->OutputMessage(7, 98)) == 0
			|| _stricmp(lpStr, "YES") == 0)
		{
			ChatMgr.CheckYesNo(1);
			return 0;
		}
		if (_stricmp(lpStr, lan->OutputMessage(7, 99)) == 0
			|| _stricmp(lpStr, "NO") == 0
			|| _stricmp(lpStr, lan->OutputMessage(7, 100)) == 0)//010215 lsw
		{
...
int InsertWisperList(const char* name)//이름 저장후 저장된 자리를 리턴
{
	...
	{
		if (!_stricmp(SMenu[MN_WISPER_LIST].nField[i].temp, name))//비교결과 이름이 다르다면
		{	//이미 그 이름은 저장 하고있어
			return i;//관둬
		}
