    ...
    case ICE_EXTREME:
		{
			...
			pCaster->set_nCurrentFrame(pCaster->nCurrentFrame); //훙膠뾔땡令
...
{	//< CSD-TW-030624
	...
	pCaster->set_nAttackedType(pCaster->nAttackedType); //묑샌잚謹쌔삿
	pCaster->set_nActiveCombat(pCaster->nActiveCombat); //묶돝세콘쌔삿
	...
	pCaster->set_nCurrentAction(pCaster->nCurrentAction); //훙膠땡鱗契槨
	pCaster->set_nCurrentFrame(pCaster->nCurrentFrame); //훙膠뾔땡令
	// 이동중인경우 멈춘다.
	if (pCaster->id >= 10000) 
	{
...
void CBattleMgr::RecvNpcMagic(t_server_npc_magic* pPacket)
{
	...
	if (pCaster != NULL && pTarget != NULL)
	{
		...
		pCaster->set_ready_Magic(pCaster->ready_Magic);			//硫구침랬
		pCaster->set_excute_MagicNum(pCaster->excute_MagicNum);//침랬세콘잚謹쌔삿!
...
{	//< CSD-TW-030606
	...
	pCaster->set_ready_Magic(pCaster->ready_Magic);        //침랬硫구
	pCaster->set_end_Casting(pCaster->end_Casting);
	pCaster->set_excute_MagicNum(pCaster->excute_MagicNum);//침랬세콘잚謹쌔삿!
	pCaster->set_nActiveCombat(pCaster->nActiveCombat); //묶돝세콘쌔삿
	pCaster->set_nActiveLevel(pCaster->nActiveLevel); //묶돝세콘된섬쌔삿
...
{	//< CSD-TW-030606
	...
	pMaster->set_nActiveCombat(pMaster->nActiveCombat); //묶돝세콘쌔삿
	pMaster->set_nActiveLevel(pMaster->nActiveLevel); //묶돝세콘된섬쌔삿
...
{	//< CSD-TW-030606
	...
	pMaster->set_excute_MagicNum(pMaster->excute_MagicNum);//침랬세콘잚謹쌔삿!
	pMaster->set_nActiveCombat(pMaster->nActiveCombat); //묶돝세콘쌔삿
	pMaster->set_nActiveLevel(pMaster->nActiveLevel); //묶돝세콘된섬쌔삿
	pMaster->set_ready_Magic(pMaster->ready_Magic);        //침랬硫구
	pMaster->set_end_Casting(pMaster->end_Casting);        //침랬硫구
	if (pMaster == Hero)
	{ 
		iCSPCMax = iCSPCNow = 0;
		::SetSCDMainAndOtherClear(0);
