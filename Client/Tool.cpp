...
#include "Menuset.h"
#include "Tool.h"
#include "MenuNetWork.h"// 030307 sotoÃß°¡.

extern HWND	g_hMultiDlg;//soto-030603

extern bool	bCharListUp;				// ´É·ÂÄ¡ º¯°æÀ» º¸³»´Ñ´Ù.
extern bool	bTackListUp ;				// ÅÃÆ½½º º¯°æ
extern bool	ScreenCaptureStartFlag;
extern int g_StartMenuOn;				//  ÇöÀç ½ºÅ¸Æ® ¸Þ´ºÀÎÁö, ¾Æ´Ô ¸ÞÀÎ ¸Þ´ºÀÎÁö ÆÇ´Ü...
...
extern DWORD GetExpByLv(int lv);		// ·¹º§À» °æÇèÄ¡·Î º¯È¯ÇØ ÁØ´Ù.
extern bool LoadItemJoinTable();
extern void WarLoopTime(char* Arg);		// 030308 soto

///////////////// SoundUp lkh Å×½ºÆ®¸¦ À§ÇØ ////////////////////
...
{			
	...
	case VK_F4 : 
		...
		JustMsg(" CursorAni.txt¸¦ ÀÐ¾ú½À´Ï´Ù." );
		break;
	...
	case VK_F5 :	
		{
			...
			if( fp )
			{
				...
				{
					fprintf( fp, " --- CharacterInfo(%2d½Ã%2dºÐ%2dÃÊ) --- \n name = %s\n position(x,y) = (%d, %d),(%d, %d),(%d, %d) \n condition = %d\n ",
						hour,min,sec,
						Hero->name,
					...
					fprintf( fp, "nCurrentAction=%d nCurrentFrame=%d sprc=%d sprno=%d dir=%d sp=%p \n\n", 
						...
						Hero->direction,
						Hero->sp );
				Hero->set_nCurrentAction(Hero->nCurrentAction); //ÈËÎï¶¯×÷ÐÐÎª
				Hero->set_nCurrentFrame(Hero->nCurrentFrame); //ÈËÎï»Î¶¯Öµ					
...
{					
	...
	/*					if( SpriteOrderFrame < 27 ) // ÇÑ¼Õ.. ¹æÆÐ.
	{
	Hero->accessory[2] = 61;
	Hero->accessory[3] = 87;
	}
	else if( 27 <= SpriteOrderFrame && SpriteOrderFrame < 35 )  // ¾ç¼Õ.
	{
	Hero->accessory[2] = 74;
	Hero->accessory[3] = 0;
	}
	else if( 36 <= SpriteOrderFrame && SpriteOrderFrame < 71 ) // È°µ¿ÀÛ..
	{
	Hero->accessory[2] = 61;
	Hero->accessory[3] = 87;
	}
...
{		
	SysInfo.music    = 1;		// 1ÀÌ¸é À½¾ÇÀ» ÄÔ 
	SysInfo.effect   = 1;		// 1ÀÌ¸é È¿°úÀ» ÄÔ 
	...
	{	
		...
		//SysInfo.dx		 = 0;		// 1ÀÌ¸é ÀüÃ¼¸ðµå,		0 : Ã¢¸ðµå		// switch screen
		SysInfo.notdead	 = 0;		// 0 : normal           1 : ÁÖÀÎ°øÀº ¹«Àû
		SysInfo.music    = 1;		// 1ÀÌ¸é À½¾ÇÀ» ÄÔ 
		SysInfo.effect   = 1;		// 1ÀÌ¸é È¿°úÀ» ÄÔ 
...
//#define _SWITCH_SCREEN   //coromo ´°¿Ú»¯
...
AGAIN_:	
	...
	if( SysInfo.result == 1 )  // ½ÇÇàÇÏ±â À§ÇØ OK¸¦ ´­·¶´Ù. 
	{
		if( SysInfo.dx == 0 ) // windows mode..
		{		
			if( GetBitsPerPixel() <= 8 ) 
			{	
				JustMsg ( "ÏµÍ³×ÀÃæÑÕÉ«Ì«£¬±ØÐëÊ¹ÓÃ( 65536color)ÒÔÉÏ.\n" );
				goto AGAIN_;
			}

			if( GetBitsPerPixel() > 16 ) 
			{	
				JustMsg ( "ÏµÍ³×ÀÃæÑÕÉ«Ì«¸ß£¬ÇëÊ¹ÓÃ16Î»É«µÄÉèÖÃ.\n" );
				goto AGAIN_;
			}
		}	
	}
	...
	g_DirectDrawInfo.bFullscreen = false;// switch screen//´°¿ÚÄ£Ê½
#else 
	SysInfo.dx = 1;		// switch screen
	g_DirectDrawInfo.bFullscreen = true;// switch screen//È«ÆÁÄ£Ê½
#endif
...
{	
	...
	if( SysInfo.notconectserver == 1 )
	{
		if( lpChar == NULL ) 
		{
			id =0;
			lpChar = Hero;
			JustMsg( " ±×·±  ID´Â ¾ø¾î¿ä.   ID¸¦ 0¹øÀ¸·Î ¹Ù²ß´Ï´Ù. " );
		}
	}
	else
	{
		if( lpChar == NULL ) 
		{
			id =1;
			lpChar = Hero;
			JustMsg( " ±×·±  ID´Â ¾ø¾î¿ä.   ID¸¦ 1¹øÀ¸·Î ¹Ù²ß´Ï´Ù. " );
		}
	}
...
{   //< CSD-031014    
	...
	if( AnotherLineCount != LineCount ) 	JustMsg( "¶óÀÎ¼ö°¡ ´Þ¶ó.." );
	lpChar->AnimationTable[ TempAnimationCount].nCount = LineCount;
}	
...
{											
	...
	case WM_INITDIALOG:	sprintf (temp, "%d", lpChar->sprno );
		...
		GetWindowRect( hDlg, &rect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
		GetWindowRect( g_hwndMain, &grect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
		MoveWindow( hDlg, grect.right+1, 0, rect.right, rect.bottom, true);
	...
	case WM_COMMAND:	
		...
		case IDC_CHANGE_EQUIP :
			...
			{
				JustMsg( "¸ó½ºÅÍ³ª NPC´Â Àåºñ¸¦ ÀåÂøÇÒ¼ö ¾ø½À´Ï´Ù. " );
			}
		...
		case ID_ANIMATION_SAVE	:	
			{
				...
				JustMsg( "%d ¹ø Ä³¸¯ÅÍÀÇ Animation Data¸¦ ÀúÀåÇÕ´Ï´Ù. \n \n[%s]", lpChar->sprno, temp );
		...
		case IDC_CHARACTER_LOAD :
			{
				...
				lpChar->set_mutant(lpChar->mutant);//×ªÖ°º¯Êý½Ø»ñ
	...
	lpChar->set_nCurrentAction(lpChar->nCurrentAction); //ÈËÎï¶¯×÷ÐÐÎª
	lpChar->set_nCurrentFrame(lpChar->nCurrentFrame); //ÈËÎï»Î¶¯Öµ			
...
BOOL CALLBACK ViewMapObjectInfoProc(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:	curMapObject = IDCheckedMapObject;
		...
		case ITEMTYPE_WORKTABLE_MINING		:	t = IDC_ITEM_WORKTABLE_MINING4;		break;		// ÀÛ¾÷´ë ¿ë±¤·Î(71)
		case ITEMTYPE_WORKTABLE_BATTLE      :	t = IDC_ITEM_WORKTABLE_BATTLE4;		break;      // ÀÛ¾÷´ë ¹èÆ²
		case ITEMTYPE_WORKTABLE_SUNBAN		:   t = IDC_ITEM_WORKTABLE_SUNBAN4;		break;		// ÀÛ¾÷´ë ¼±¹Ý
		case ITEMTYPE_WORKTABLE_MORU		:	t = IDC_ITEM_WORKTABLE_MORU4;		break;		// ÀÛ¾÷´ë ¸ð·ç
		case ITEMTYPE_WORKTABLE_HWADUK		: 	t = IDC_ITEM_WORKTABLE_HWADUK4;		break;		// ÀÛ¾÷´ë È­´ö
		case ITEMTYPE_WORKTABLE_SIHUMKWAN	:   t = IDC_ITEM_WORKTABLE_SIHUMKWAN4;	break;		// ÀÛ¾÷´ë ½ÃÇè°ü·ù
		case ITEMTYPE_WORKTABLE_MULAE		: 	t = IDC_ITEM_WORKTABLE_MULAE4;		break;		// ÀÛ¾÷´ë ¹°·¹
		case ITEMTYPE_WORKTABLE_BULDAE		: 	t = IDC_ITEM_WORKTABLE_BULDAE4;		break;		// ÀÛ¾÷´ë ºÒ´ë 
			
		case ITEMTYPE_SEAL_NOR				: 	t = IDC_ITEM_SEAL_NOR				;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_SEAL_SPC				: 	t = IDC_ITEM_SEAL_SPC				;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_HP_UP					: 	t = IDC_ITEM_HP_UP					;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_MP_UP					: 	t = IDC_ITEM_MP_UP					;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_SCALE					: 	t = IDC_ITEM_SP_UP					;	break;		// ÀÛ¾÷´ë ºÒ´ë	// 010614 khs
		case ITEMTYPE_NATION_STONE			: 	t = IDC_ITEM_NATION_STONE			;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_GUILD_STONE			: 	t = IDC_ITEM_GUILD_STONE			;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_SENARIO_STONE			: 	t = IDC_ITEM_SENARIO_STONE			;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		case ITEMTYPE_COLOSSUS_STONE		: 	t = IDC_ITEM_COLOSSUS_STONE			;	break;		// ÀÛ¾÷´ë ºÒ´ë 
		...
		// Sound.
		sprintf( temp, "%d", mo->soundno );			//10000´ÜÀ§ ÀÌ»óÀº µô·¹ÀÌ¸¦ À§ÇÑ Ä«¿îÆ® º¯¼ö¿ë
		...
		case WM_COMMAND:	
			...
			case IDC_BUTTON_SAVE		: 
				if(SysInfo.notconectserver)
					...
					MessageBox(NULL,"¼­¹ö¿Í ¿¬°áµÈ »óÅÂ¿¡¼­´Â SAVEµÇÁö ¾Ê½À´Ï´Ù.",0,MB_OK);
				break;
...
void tool_InputMapAttr( int mx, int my, int flag )
{
	...
	{	
		...
		case IDC_ATTR_TOP       :   PaintTileAttrTop(mx,my);			break;		// 010904 LTS    //³ô³¸ÀÌÀÇ ±¸Çö 
		}
		return;
	}
	
	//________________________________________________________________________	// 010904 LTS
	if (tMapAttr_Brush_DePaint == IDC_DEPAINT_YES)								//Á¦°Å º×ÀÇ ±¸Çö 
	{
		if( flag )
			switch( tMapAttr )
		{
...
{																				
	...
	case WM_COMMAND:								
		...
		case IDC_DONT_TO_GE :	if( YesOrNo( "Dont ¼Ó¼ºÀ» È¿°úº® ¼Ó¼ºÀ¸·Î CopyÇÕ´Ï´Ù. ", "?" ) == IDYES )
								{
									if( YesOrNo( "¿©±â¼­ Àá±ñ! \n À½.. °»½ÅµÈ Data´Â UnDoµÇÁö ¾Ê½À´Ï´Ù. '.MAP'À» ¸ÕÀú ÀúÀåÇÏ¼¼¿ä \n\n 'Yes'¸¦ ´©¸£¸é °Ô¼ÓÁøÇàÇÕ´Ï´Ù.", "È®ÀÎ" )  == IDYES )
									{
										ChangeMapAttrDont2Geffect();
										JustMsg( "¸ðµç Dont ¼Ó¼ºÀÌ È¿°úº®¼Ó¼ºÀ¸·Î ¹Ù²î¾ú½À´Ï´Ù. " );
									}
								}
			break;									
...
{																				
	...
	case WM_COMMAND:								
		...
		case IDLOAD     :	LoadCursorAni();
			LoadMouseCursor( "Cursor.Spr" );
			JustMsg(" CursorAni.txt¿Í Cursor.Spr¸¦ ÀÐ¾ú½À´Ï´Ù." );
			break;
...
{						
	...
	case WM_COMMAND:	
		...
		case ID_NPC_CHANGE :
			...
			sprintf( szBuffer, "Çö ¸Ê¿¡ ÀÖ´Â NPC¹èÄ¡Áß¿¡¼­ \n %d(%s)-> %d(%s)·Î ¹Ù²ß´Ï´Ù.", from, g_infNpc[from].szName, to, g_infNpc[to].szName); // CSD-030419
			
			if( YesOrNo( szBuffer, "ÁÖÀÇ" ) == IDYES ) 
			{
		...
		case ID_NPC_ALL_DELETE :
			sprintf( szBuffer, "Çö ¸Ê¿¡ ÀÖ´Â NPC¹èÄ¡¸¦ ¸ðµÎ ¾ø¾Û´Ï´Ù. " );
		...
		case IDOK :	{	
			...
			no = atoi(szBuffer);
			sprintf( szBuffer, "´ÙÀ½ÀÇ NPC¹èÄ¡¸¦ ¾ø¾Û´Ï´Ù. \n %d¹ø %s ", no, g_infNpc[no].szName); // CSD-030419
	...
	case WM_INITDIALOG:	
		{
			...
			for( i = 0 ; i < 45; i ++)
			{
				sprintf( temp, "%d", SCharacterData.nJobSkill[c++] );
...
{				
	...
	case ID_MAPOBJECT_MODIFY : tool_mapobject_modify = !tool_mapobject_modify;
		SysInfo.gamemakemode = 1;
								if( tool_mapobject_modify ) 
									AddCurrentStatusMessage( 255,255,255, "MapObject¼öÁ¤°¡´É" );
								else AddCurrentStatusMessage( 255,0,0, "MapObject¼öÁ¤ ºÒ°¡" );
								break;
	...
	case ID_INPUT_MAP_ATTR :
		{
			...
			if( tool_ID_INPUT_MAP_ATTR  )
			{
				...
				ScreenX = GetSystemMetrics(SM_CXSCREEN);
				//ShowWindow( CheckAnimationHwnd, SW_HIDE);
				GetWindowRect( InputMapAttrHdlg, &rect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
				GetWindowRect( g_hwndMain, &grect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
	...
	case ID_INPUT_SKILL :	
		{
			...
			if( tool_ID_SKILL_INPUT )
			{
				...
				ScreenX = GetSystemMetrics(SM_CXSCREEN);
				//ShowWindow( CheckAnimationHwnd, SW_HIDE);
				GetWindowRect( InputSkillAttrHdlg, &rect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
				GetWindowRect( g_hwndMain, &grect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
	...
	case ID_MOUSECURSOR_CHECK : 
		...
		if( tool_MouseCursorBlock  )
		{
			...
			ScreenX = GetSystemMetrics(SM_CXSCREEN);
			//ShowWindow( CheckAnimationHwnd, SW_HIDE);
			GetWindowRect( tool_MouseCursorHdlg, &rect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
			GetWindowRect( g_hwndMain, &grect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
	...
	case ID_EFFECT_TEST : tool_ID_EFFECT_TEST = !tool_ID_EFFECT_TEST;
		...
		{
			AddCurrentStatusMessage( 0,0,255, "¸¶¹ýÀÌ Á¤»óÈ­ µÇ¾ú½À´Ï´Ù." );
		}
		else
		{
			AddCurrentStatusMessage( 0,0,255, "¿À¸¥ÂÊ¸¶¿ì½º¿¡ÀÇÇØ fireBallÀÌ ³ª°©´Ï´Ù." );
		}
		break;
	...
	case ID_INPUT_MAPOBJECT_MENU :
		...
		if( tool_IDD_INPUT_MAPOBJECT  )
		{
			...
			ScreenX = GetSystemMetrics(SM_CXSCREEN);
			//ShowWindow( CheckAnimationHwnd, SW_HIDE);
			GetWindowRect( tool_IDD_INPUT_MAPOBJECTHdlg, &rect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
			GetWindowRect( g_hwndMain, &grect );// È­¸é ¿À¸¥ÂÊÀÌ ºÙ¾î³ª¿À°Ô ÇÏ·Á°í ÇÑ°Çµ¥ ... ¾î¶»°Ô ÇØ¾ß ÇÒÁö..
...
//soto Offline Work End.
	...
	case ID_TEAM_BATTLE_END   :		if( MapNumber == 16 )  // MapName : FIGHT..
		...
		{	
			AddCurrentStatusMessage( 255, 100, 100, "ÆÀ¹èÆ²¿ëÀÌ ¾Æ´Ï¶ó 'ÆÀ¹èÆ² ½ÃÀÛ/³¡' ¸í·ÉÀ» º¸³¾¼ö ¾ø½À´Ï´Ù." );
		}	
		break;
...
//soto 030307
//¿©±â¼­´Â ´©±º°¡ ¸ÞÀÎ ¸Þ´ºÀÇ ¾î¶² Ç×¸ñÀ» ¼±ÅÃÇßÀ»¶§ÀÇ ¸Þ¼¼Áö¸¦ Ã³¸®ÇÑ´Ù.
void	MainMenuSelected(WPARAM wParam,LPARAM lParam)
{
	HMENU hGMMenu	= NULL; // "GM" ¸Þ´ºÀÇ ÇÚµéÀÌ´Ù.
	HMENU hToolMenu  = NULL;
	HMENU hOption	= NULL;

	if(SysInfo.notdead) // ÀÏ¶§´Â.. IDC_DRAGON ¸Þ´º¸¦ »ç¿ë ÇÑ´Ù.
	{
		hOption  = GetSubMenu((HMENU)wParam,2); // ¼¼¹øÂ°.
		hGMMenu = GetSubMenu((HMENU)wParam,6); // 7¹øÂ°¿¡ À§Ä¡ÇÏ°í ÀÖ´Ù.
		hToolMenu  = GetSubMenu((HMENU)wParam,5); //¿©¼¸.
		
	}
	else // IDC_GM ¸Þ´º¸¦ »ç¿ëÇÑ´Ù.
	{
		hGMMenu = GetSubMenu((HMENU)wParam,0); // Ã¹¹øÂ°¿¡ À§Ä¡ÇÏ°í ÀÖ´Ù.
		hOption = GetSubMenu((HMENU)wParam,1); // µÎ¹øÂ° À§Ä¡ÇÏ°í ÀÖ´Ù.
	}
...
BOOL CALLBACK OXEventProc(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_OPEN_MAP : 
			{
				SendOXEventAboutMap( 1 );	// Âü¿© À¯µµ
				break;
			}
		case IDC_END_EVENT :
			{
				SendOXEventAboutMap( 3 );	// Á¾·á
				break;
			}
		case IDC_START_EVENT :
			{
				SendOXEventAboutMap( 2 );	// Á¾·á
				break;
			}
		case IDC_CLOSE_MAP	:
			{
				SendOXEventAboutMap( 4 );	// Á¾·á
				break;
			}
...
{
	return (pCharac->sprno > 1); // 0,1ÀÌ ¾Æ´Ñ ½ºÇÁ¶óÀÌÆ® ³Ñ¹ö¸¦ °¡Á³À¸¸é NPCÀÌ´Ù.
}
	...
	case WM_INITDIALOG:	
		{
			...
			while( pChar != NULL )
			{
				...
				if(!IsNPC(pChar) && g_CharacterList.lpFirst != pChar)		//ÀÚ±âÀÚ½ÅÀº ¸¶Áö¸·¿¡ µé¾î°£´Ù.
				SendMessage(hListWnd,LB_ADDSTRING,0,(LPARAM)pChar->name);	//¿Ö ³ÄÇÏ¸é..ÀÌµ¿µÉ¶§ ÀÚ½ÅÀº ¸¶Áö¸·¿¡ ÀÌµ¿µÇ¾î¾ß.
																			//À¯ÀúµéÀÌ ¹Ù¸£°Ô Å«Æ®·Ñ µÉ¼ö ÀÖ±â ¶§¹®ÀÌ´Ù.
				pChar = pChar->lpNext;
			}
			...
			{				
				case BTN_MOVE :	// map move
				{
					...
					if(0 < nCount)
					{
						...
						// ¼ÓÇÑ³ª¶ó	0 : ¾øÀ½, 1, 2, 3: ¹ÙÀÌ¼­½º 4: ÀÚÀÌÆÝ 5:Çì°Ô¸ð´Ï¾Æ 6: ÀÏ½º


						t_packet packet;
						
						HWND hMapNameWnd = GetDlgItem(hDlg,CBO_MAPNAME); // ÄÞº¸ ¹Ú½ºÀÇ  ÇÚµéÀÌ´Ù.
						SendMessage(hMapNameWnd,WM_GETTEXT,MAX_PATH,(LPARAM)temp); // ÇöÀç ¾²¿©Á® ÀÖ´Â ÅØ½ºÆ®¸¦ ÀÐ¾î ¿Â´Ù.
						EatRearWhiteChar( temp );
						...
						for(int nIndex = 0;nIndex < nCount;nIndex++)
						{
							...
							if(pChar)
							{
								//Ã¼Å© µÇ¾î ÀÖ´Â »ç¶÷ µé¸¸ °¢ ±¹°¡¿¡ ÇØ´ç µÇ´Â »ç¶÷µé¸¸.
...
//									Sleep(200);
				...
				break;
				case BTN_DEL_USER:// °¡´É ¸®½ºÆ®¿¡ ÀÖ´Â ÀÌ¸§µéÀ» ºÒ°¡´É ¸®½ºÆ®·Î ¿Å±ä´Ù.
				{
					char szID[600] = {0,};
					...
					if(strlen(szID))
					{
						...
						if(pChar)
						{
							if(pChar == Hero)//ÀÚ±âÀÚ½ÅÀ» °¡¸®Å°¸é.
								nIndex = SendMessage(hRightListWnd,LB_INSERTSTRING,-1,(LPARAM)szID); // ¸Ç ¸¶Áö¸·¿¡ ³Ö´Â´Ù.
							else 
								nIndex = SendMessage(hRightListWnd,LB_INSERTSTRING,0,(LPARAM)szID); // ÀÏ¹Ý À¯ÀúÀÌ¸é Ã³À½¿¡ »ðÀÔ.
				...
				case BTN_ENABLE_USER:
				{
					...
					if(strlen(szID))
					{
						...
						{
							if(pChar == Hero)//ÀÚ±âÀÚ½ÅÀ» °¡¸®Å°¸é.
								nIndex = SendMessage(hLeftListWnd,LB_INSERTSTRING,-1,(LPARAM)szID); // ¸Ç ¸¶Áö¸·¿¡ ³Ö´Â´Ù.
							else 
								nIndex = SendMessage(hLeftListWnd,LB_INSERTSTRING,0,(LPARAM)szID); // ÀÏ¹Ý À¯ÀúÀÌ¸é Ã³À½¿¡ »ðÀÔ.
				...
				case BTN_LISTSAVE: // ¼±ÅÃµÈ ¸®½ºÆ®¸¦ "MutiControl.txt" ¿¡ ÀúÀåÇÑ´Ù.
				{
					HWND hLeftListWnd = GetDlgItem(hDlg,LST_SELECTED_USERS);
	...
	case WM_INITDIALOG:	
		{
			...
			{	//< CSD-030419
				SendMessage(hNameWnd,CB_ADDSTRING,0,(LPARAM)g_infNpc[i].szName);
			}	//> CSD-030419
...
extern void SetLocalWarTime()
{
	...
	JustMsg(" ¾ÆÁ÷ Áö¿øµÇÁö ¾Ê´Â ±â´ÉÀÔ´Ï´Ù. - soto -"); return; //¾ÆÁ÷ Áö¿øµÇÁö ¾Ê´Â´Ù. 030308 soto
...
extern BOOL CALLBACK SetNationWarTimeProc(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:	
		{	
			...
			SendMessage(hLoopTimeWnd,WM_SETTEXT,0,(LPARAM)"0"); // ÃÊ±â ½Ã°£Àº 0ºÐ.
		...
		case IDOK :	
			{
				...
				_itoa(nIndex,szTemp,10);strcat(szTemp," ");

				nIndex = SendMessage(hCbFieldNameWnd,CB_GETCURSEL,0,0);
				if(nIndex == LB_ERR)return false;
				_itoa(nIndex,szTemp2,10);strcat(szTemp,szTemp2);strcat(szTemp," ");
...
////// ProcSetScenario¿¡¼­¸¸ ¾²ÀÌ´Â Àü¿ë ÇÔ¼öÀÌ´Ù. µýµ¥¼­ ¾²Áö ¸¿½Ã´Ù.
//
//
...
void SelectedTimeList(HWND hListWnd,int index)
{
	...
	// °¢°¡ÀÇ ¿¡µðÆ® Ã¢¿¡ ÇØ´çÁ¤º¸¸¦ µð½ºÇÃ·¹ÀÌ.
	_itoa(data->nStartHour,temp,10);
	SendMessage(hStartWnd,WM_SETTEXT,0,(LPARAM)temp);//½Ã
	_itoa(data->nStartMinute,temp,10);
	SendMessage(hStartMinWnd,WM_SETTEXT,0,(LPARAM)temp);//ºÐ

	_itoa(data->nEndHour,temp,10);
	SendMessage(hEndWnd,WM_SETTEXT,0,(LPARAM)temp);//½Ã
	_itoa(data->nEndMinute,temp,10);
	SendMessage(hEndMinWnd,WM_SETTEXT,0,(LPARAM)temp);//ºÐ
}
...
void SendSelectedSenarioName(int index)
{
	...
	ReleaseListData(hListWnd);// ¸®½ºÆ®¿¡ Å×ÀÌÅÍ°¡ ÀÖÀ»¼öµµ ÀÖ´Ù.
	SendMessage(hListWnd,LB_RESETCONTENT,0,0); //±ú²ýÀÌ ¸®½ºÆ®¸¦ Áö¿î´Ù.
	SendMessage(hNameWnd,CB_SETCURSEL,index,0);//ÇØ´ç ´øÁ¯ÀÌ¸§À¸·Î ¹Ù²Û´Ù.

	// ´ÙÀ½¿¡ ¼­¹ö¿¡ ÇØ´ç´øÀüÀÇ ½Ã°£Ç¥¸¦ ¿äÃ»ÇÑ´Ù.
	t_packet packet;
...
// ¿©±â¼­´Â ¸®½ºÆ®¿¡´Ù ÇØ´ç ÅØ½ºÆ®¸¦ Ãß°¡ ½ÃÄÑÁÖ°í µ¥ÀÌÅÍ¸¦ ¿¬°á¸¸ ½ÃÄÑÁÖ¸é µÈ´Ù
bool AddTimeList(void* pTemp)
{
	...
	strcpy(szHourText,"½Ã");
	strcpy(szDayText,"¿äÀÏ");		
#endif

	::strcpy(szAdd,::GetDayOfWeekByNo(pData->nApplyDay));
	strcat(szAdd,szDayText); // "¿äÀÏ" ÀÇÅØ½ºÆ®ÀÌ´Ù.


	_itoa(pData->nStartHour,szTemp,10);
	strcat(szAdd,szTemp);
	...
	_itoa(pData->nEndHour,szTemp,10);
	strcat(szAdd,szTemp);
	...
	case WM_INITDIALOG:	
		{
			...
			// ÃÊ±âÈ­ÇÒ¶§.. ÄÞº¸¹Ú½º¿Í ¸®½ºÆ®¹Ú½º¿¡ ³»¿ëÀ» À§ÇØ. ´øÁ¯¿¡ ´ëÇÑ ÀÚ·á¸¦ ¿äÃ»ÇÑ´Ù.
			HWND hNameWnd = GetDlgItem(g_hSetScenarioDlgWnd,CBO_SCENARIO_NAME);
	...
	case WM_COMMAND:
		{
			...
			case IDOK :
				{
					...
					{
						JustMsg("¸ðµç °ªÀÌ ¼ÂÆÃµÇ¾î¾ß ÇÕ´Ï´Ù.");
						return false;
					}
			...
			case IDCANCEL :
				EndDialog(hDlg, LOWORD(wParam));
				g_hSetScenarioDlgWnd = NULL;// ÀÌ ´ëÈ­Ã¢Àº ´ÝÈ÷°Ô µÇ´Ï±î. ÇÚµéÀ» ¾ø¾Ö ÁØ´Ù.
				return (TRUE);
...
{ // 031013 kyo
	...
	case WM_COMMAND:
		...
		case IDOK :	
			{
				g_ProFileType.bParent = IsDlgButtonChecked(hDlg,CHE_PARENT);
				g_ProFileType.bCurrent = IsDlgButtonChecked(hDlg,CHE_CURRENT);
				g_ProFileType.bMax = IsDlgButtonChecked(hDlg,CHE_MAX);
				g_ProFileType.bAvr = IsDlgButtonChecked(hDlg,CHE_AVR);
				g_ProFileType.bMin = IsDlgButtonChecked(hDlg,CHE_MIN);
				g_ProFileType.bRate = IsDlgButtonChecked(hDlg,CHE_RATE);
