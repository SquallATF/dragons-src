		...
		if (fp)
		{
			...
			else
			{
				...
				fprintf(fp, "nCurrentAction=%d nCurrentFrame=%d sprc=%d sprno=%d dir=%d sp=%p \n\n",
					...
					Hero->direction,
					Hero->sp);
				Hero->set_nCurrentAction(Hero->nCurrentAction); //훙膠땡鱗契槨
				Hero->set_nCurrentFrame(Hero->nCurrentFrame); //훙膠뾔땡令					
...
void CheckCharacterFrameOrder(WPARAM wParam, LPARAM lParam)
{
	...
	case VK_LEFT:
		SpriteOrderFrame--;
		if (SpriteOrderFrame < 0)
			SpriteOrderFrame = 74;

		break;
	case VK_RIGHT:
		SpriteOrderFrame++;
		if (SpriteOrderFrame >= 75)
			SpriteOrderFrame = 0;
...
//#define _SWITCH_SCREEN   //coromo 눗왯뺏
...
AGAIN_:
	...
	if (SysInfo.result == 1)  // 실행하기 위해 OK를 눌렀다. 
	{
...
BOOL CALLBACK CheckAnimationProc(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_PLAY_ANIMATION: 	Edit_GetText(GetDlgItem(hDlg, IDC_EDIT_BASIC), temp, 9);
			...
			lpChar->set_nCurrentAction(lpChar->nCurrentAction); //훙膠땡鱗契槨
			lpChar->set_nCurrentFrame(lpChar->nCurrentFrame); //훙膠뾔땡令			
...
BOOL CALLBACK ToolPasswordProc2(HWND hDlg, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		SetTimer(g_hwndMain, 20, 1000, NULL);
		break;
...
void SystemMenuProcess(WPARAM wParam)
{
	...
	switch (wParam)
	{
	case ID_CHECK_EQUIP: CheckEquipmentData(g_hwndMain, g_hInstance); break;
	case ID_CHECK_ANIMATION: CheckAnimationEdit(g_hwndMain, g_hInstance); break;
	case ID_CHECK_CLOTH_ORDER: SysInfo.viewcharacterframe = !SysInfo.viewcharacterframe; break;
	case ID_VIEW_ANIMATION_FRAME: tool_ViewAnimationFrame = !tool_ViewAnimationFrame; break;
	case ID_VIEW_PROFILER:  ::ShowProFileDlg(); break;// 031013 kyo
	case ID_DRAWMAP: tool_DrawMap = !tool_DrawMap; break;
	case ID_DRAWFRONTMAP: tool_DrawFrontMap = !tool_DrawFrontMap; break;
	case ID_DRAWOBJECT: tool_DrawObject = !tool_DrawObject; break;
	case ID_DONT_VIEW_MESSAGE: tool_ViewMessage = !tool_ViewMessage; break;
	case ID_DONT_VIEW_CHAT: tool_ViewChat = !tool_ViewChat; break;
	case ID_DONT_VIEW_DAMAGE_NUMBER: tool_ViewNumber = !tool_ViewNumber; break;
	case ID_DONT_VIEW_MAIN_INTERFACE: tool_ViewMenu = !tool_ViewMenu; break;
	case ID_DRAWROOFMAP:
		tool_DrawRoofMap = !tool_DrawRoofMap;
	...
	case ID_VIEW_ATTR_INSIDE:
		tool_CheckViewAttr(wParam);
		break;

	case ID_CHANGE_BODYRGB:
		ChangeBodyRGB();
		break;
	case ID_MULTICHAR:
	{
	...
	case ID_FRAMECHECK: tool_FrameCheck = !tool_FrameCheck; break;
	case ID_SYSINFO_GAMEMAKEMODE: SysInfo.gamemakemode = !SysInfo.gamemakemode; break;
	...
	case ID_SIGHT_VIEW_NORMAL: if (Hero) Hero->sight = 10; break;
	case ID_SIGHT_VIEW_WIDER: if (Hero) Hero->sight = 540; break;
	case ID_MAPOBJECT_MODIFY:
	...
	case ID_TOOL_PW:			ToolPassWord(); break;
	case IDM_LOGOUT_NAME:  ToolLogoutCharacter(); break;	// 020428 YGI acer
	case ID_MENU_MOVE_MAP: ToolMapMove(); break;
	case ID_MENU_MOVE_MAP_MULTIPLE: ToolMapMoveMultiple(); break;
	case ID_MOSTER_GENERATER: ToolGenerateMonster(); break;  //soto 030307
	case ID_LOCALWAR: ::SetLocalWarTime(); break;
	case ID_NATIONSET: ::SetNationWarTime(); break; //soto Offline Work End.
	case ID_OTHER_CHAR_INFO: ToolPassWord(1); break;	// acer2
	case ID_TRIP_AROUND_ON:	SettingTripAround(); break;
	case ID_TRIP_AROUND_OFF: DeSettingTripAround(); break;
	case ID_CHANGE_SKILL_EXP: ChangeSkillExp(); break;
	case ID_PUBLIC_MESSAGE_IN_MAP: PublicBroadCasting(); break;
	case ID_PUBLIC_MESSAGE_IN_MAP_ALL: PublicBroadCastingtoAll(); break;
	case ID_START_EVENT: StartOXEvent(); break;

	case ID_TEAM_BATTLE_END:
		if (MapNumber == 16)  // MapName : FIGHT..
		{
