...
//		packet.u.kein.item_attr_pos.much	= much;
		 packet.u.kein.item_attr_pos.much    = 0;        //覩much乖맣냥0죄,侶湳옵鹿팁롸켐씹뵨팁칠독
	}
...
int GetLeveByExp(int nStep, DWORD exp)
{
	for (int i = 1; i < Num_Of_NPC_Lev; ++i)
	{
		...
		if (nStep == NPC_Lev_Ref[i].nStep && NPC_Lev_Ref[i + 1].nMinExp == 0)
			return i;
	}
...
void RecvTacSkillExp( t_char_info_tac_skillexp *s )		// 13개
{
	...
	memcpy( Hero->aStepInfo, s->aStepInfo, 20* sizeof( BYTE ) );
...
void RecvServerMagery(unsigned char nStep, DWORD magery)
{
	int type;
...
int CheckCharNameAndNation( char *name )
{
	...
	if( ::_stricmp( name, kein_GetMenuString( 47 ) ) == 0 ) 
	{
		return 1;
	}
...
{
//	if( Hero->name_status.king != 1 ) return;		// 지도자만 사용할수 있다.   //coromo 2005/05/06 혼뇜KING景홈
...
void RecvCheckSalvationName( t_packet *p )
{
	...
	else
	{
		...
		g_Menu_Variable.set_bCheckSalvation(g_Menu_Variable.bCheckSalvation);//轟掘츰폭
...
void SendLogOutCharacter( char *szName )//T훙苟窟돨변鑒
{
#ifdef _DEBUG
	t_packet p;
	...
	for( int a=0; a<3; a++ )
	{
		for( int b=0; b<3; b++ )
		{
			for( int c=0; c<8; c++ )
			{
				...
				if( (abs(tile_x-(int(pAttr->x)+off_x))>12) || (abs(tile_y-((int)pAttr->y +off_y))>12) ) continue;
