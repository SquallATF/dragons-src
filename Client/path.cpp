...
///////////////////////////////////////////////////////////////////////////////
//	
int  PathCollisionLineCount; //   길기에 사용될 아이템을 모아 놓는다.
...
#define MAX_AREA_X_		21	// Map Server의 Area.h의 REM_AREA_X와 같다.
#define MAX_AREA_Y_		18
	...
	if (ch)
	{
		...
		if (ch->notcomeinside) //맵속성중 inside속성에 들어오지 못하는 캐릭터가 았다. 
		{
	...
	//	주인공이 Ghost가 되면 문을 열필요 없이 지나다닐수 있다. 
	if (ch)
		if (Hero)
		{
...
///////////////////////// 0812 yjs 수정 ///////////////////////////////////////////
//1타일씩 가면서 못갈때 까지 체크...
//int Tile_Collision(LPCHARACTER ch, DIRECTION dir, int Power) //축구공, 방향, 힘..
//{
//	LPCHARACTER lpCharacter = Hero->lpNext;//무조건 hero가 제일 앞에 있다..
//
//	int t_sx = ch->x / TILE_SIZE;// 타일 좌표로 만든다..
...
//	}
//
//	while (lpCharacter != NULL)	///////////////// 나자신이 공에 부딪치면 그냥 통과하라...
//	{
//		for (int i = 1; i<Power; i++)  //////////////  don't타일 인가? 체크..///////////
//		{
//			t_sx += how_X;
//			t_sy += how_Y;
//
...
//////////튕기는 함수..
//
//void Reaction(LPCHARACTER ch, DIRECTION& old_dir, int& power)  //축구공, 벽에 맞을때 방향, 튕길때 방향, 힘(타일단위)
//{
//	int t_power; // 파일단위..
//	int check;
//	int t_X;
...
//
//			count++;
//			if (count == 6)		power = 0;//3방향다 못가는 수가 있기때문..
//
...
//
//			count++;
//			if (count == 6)		power = 0;//2방향다 못가는 수가 있기때문..
//
//		} while (check == 0 || count > 6);
//		break;
//
...
//
//			count++;
//			if (count == 6)		power = 0;//못가는 수가 있기때문..
//
//		} while (check == 0 || count > 6);
//		break;
//
