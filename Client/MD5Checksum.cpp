...
NOTES:   Provides an interface to the CMD5Checksum class. 'strFilePath' name should
	...
	exception.
*****************************************************************************************/
unsigned char *CMD5Checksum::GetMD5(const char *strFilePath) {
	//open the file as a binary file in readonly mode, denying write access 
	FILE *File = fopen(strFilePath, "rb"); //(strFilePath, CFile::shareDenyNone);
	if (!File)
		return 0;
	//the file has been successfully opened, so now get and return its checksum
	unsigned char *md5value = GetMD5(File);
	fclose(File);
	return md5value;
}
...
*****************************************************************************************/
unsigned char * CMD5Checksum::GetMD5(FILE *File) {
	try {
...
*****************************************************************************************/
unsigned char *CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength) {
...
*****************************************************************************************/
unsigned char *CMD5Checksum::Final() {
	//Save number of bits
	BYTE Bits[8];
	...
	unsigned char *lpszMD5 = new unsigned char[nMD5Size];
