...
//---------------------------------------------------------------------------------
extern int					g_LS_res;	//괏닸柑깃璘숩斤口
extern int					g_RS_res;	//塘숩
//int		SetEvent=0;
...
//////////////////////////////////
char Hackname[20],*str,Sendhack[100];/////////////////////////////////////땍屢랙箇棍밈댄轎돨俚륜눔
char			HackID[20];
////////////////////////////////////////////
...
public :
	CMyCodeString()
	{
		my_string[0] = ConvChar( 'a' );
		my_string[1] = ConvChar( 'o' );
		my_string[2] = ConvChar( 'd' );
		my_string[3] = ConvChar( 'o' );
		my_string[4] = ConvChar( 'n' );
		my_string[5] = ConvChar( 'g' );
...
void CalcTacticsSkillExp( int tac_skill, DWORD exp, BYTE step )
{	
...
{	
	memcpy( SCharacterData.SkillId, p->skill, 200 * sizeof(USHORT));	// #### 1102
...
{	
	...
	GetItemDuration( *target_item, dur, mymax);
	...
	if( ii->GetRButton() == DIVIDE_ITEM )
	{
		...
		target_item->attr[IATTR_DURATION] = MAKELONG( dur, mymax);
	}
	else
	{
		dur  = p->dur;
		...
		case CMD_AGENT_SERVER_IP:
			{
				...
				memcpy( &g_GameInfo.agent_port, packet.u.agent_server_ip.ip + 19, 2);
...
{
	if( 0 != _stricmp( packet->u.Hwoa.rang.ConnectAgentServerConform.szUserID, IIDD ) )
	{
		return;
	}
