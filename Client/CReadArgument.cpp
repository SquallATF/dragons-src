...
void	CClientBill::initipNport()
{
	...
	for (int i = 0; i < MAX_BILL_CORP; i++)	Port_pos[i] = -1;	//Command[]에 port가 들어 있는 위치
...
{
	initipNport();					//클래스 내부의 변수?E초기화.
	g_unitel = NULL;					//유니텔 ?EE클래스 초기화.
	CommandLine = NULL;
	Command = NULL;					//Command로 넘엉龐 것들을 가리킬 포인터

	CommandLineSlash = NULL;			//넘엉龐 인자값 / 로 구분한다
	CommandLineSlashBackup = NULL;	//strtok를 실행하?ECommandszLine이 변하므로 다시 원상복귀 시키?E위한것

	CommandLineBackup = NULL;

	WhatLineIUse = -1;					//엉隙 업체의 회선인?E구분. -1이?E종량제가 아니라는 ?E
...
int CClientBill::PutLine()
{
	...
	for (int i = 0; i < CommandNumSlash; i++)	CommandSlash[i] = NULL;
...
int		CClientBill::Check_UnitelWeb()		//나 웹이서 들어오는 유니텔?
{
	...
	for (int i = 0; i < CommandNum; i++)
	{
		...
		for (int j = 0; j < strlen(Command[i]) - 6 + 1; j++)
			if (!_memicmp(uni, Command[i] + j, 6))		//unitel이라는 단어가 있으면
			{
...
int		CClientBill::Check_Chollian()		//나 천리안?
{
	...
	if (!_stricmp(Command[0], "chollian"))
	{
...
int		CClientBill::Check_MediaWeb()
{
	...
	if (!memcmp(cmdLine, "MDC", 3))		//0번째 인자값이 메지이고 넘어온 인자들의 총 수가 5이면
	{
		...
		TCHAR para[50];
		int ret = DecryptFunc(Return, para);	//这里把弹出DESDLL.DLL那个对话框给屏了，因为要加密
		if (ret == TRUE)
		{
			para[49] = 0;
			memset(UserID, 0, 40);
...
int		CClientBill::Check_WiseTop()		//나 와이즈탑?
{
	...
	for (int i = 0; i < CommandNum; i++)
	{
		if (strlen(Command[i]) > 10)
			if (!_memicmp(wise, Command[i], 10))		//unitel이라는 단어가 있으면
			{
...
{
	if (CommandNum != 3) return 0;
	if (!_stricmp(Command[0], "WeverNet"))		//0번째 인자값이 메지이고 넘어온 인자들의 총 수가 5이면
	{
		memset(UserID, 0, 40);
...
{
	if (CommandNum != 4) return 0;
	if (!_stricmp(Command[0], "mezzy"))		//0번째 인자값이 메지이고 넘어온 인자들의 총 수가 5이면
	{
		memset(UserID, 0, 40);
...
int CClientBill::Check_KornetWorld()	//나 코넷월드?
{
	...
	if (!_stricmp(Command[0], "/KWG"))		//0번째 인자값이 코넷월드이고 넘어온 인자들의 총 수가 5이면
	{
		memset(UserID, 0, 40);
		strcpy(UserID, Command[3]);
...
{
	if (CommandNum != 3) return 0;
	if (!_stricmp(Command[0], "/now"))		//0번째 인자값이 나우누리이고 넘어온 인자들의 총 수가 3이면
	{
...
int CClientBill::Check_ExciteGame()
{
	//	_asm int 3;
	if (CommandNum < 1)	return 0;
	if (_memicmp(Command[0], "/EGPARAM:", 9))		//만약 ExciteGame에서 인자값으로 실행한 것이 아니면 
		return 0;
	HRESULT hr;
