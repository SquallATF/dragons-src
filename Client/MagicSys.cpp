...
bool CMagicSys::Ready(LPCHARACTER pCaster, int nMagic, int nReady)
{
	...
	if (g_lpML == NULL || pCaster->ready_Magic == 0)
	{
		...
		pCaster->set_start_Casting(pCaster->start_Casting);        //침랬硫구
		...
		if (IsContinue(nMagic))
		{
			if (pCaster->continue_RunMagic < g_ClientTime + g_lpMT[nMagic].continue_Time * 1000)
			{
				...
				pCaster->set_continue_RunMagic(pCaster->continue_RunMagic);////침랬넣崎珂쇌
...
bool CMagicSys::AddEffect(LPCHARACTER pCaster, LPCHARACTER pTarget, int nMagic, int nX, int nY, bool bFail)
{
	...
	{
		pCaster->bow_Type = 1;
		pCaster->set_bow_Type(pCaster->bow_Type);//// 침랬묶숫,0덜깊청唐,1덜깊삽숫,2덜깊깥숫.
	...
	case DEATH_FIREE: // 갠梧聃삽
	{
		::InsertMagic(pCaster, pTarget, 92, 0, 0, 0, nX, nY, bPattern, 87, bFail);
		// 릿齡3瘻삽돨덜쯤 횔빈畇92 冷앎角梁뻥밍膠돨침랬湳綾   狼청唐딸속돨침랬 겉빈충87畇0  품충畇87
		break;
	}
