...
/////////////////////////////////////////////////////////////////////////
#include <stdlib.h> //Ëæ»úÊýËùÐèÒªµÄÍ·ÎÄ¼þ
#include <stdio.h> 
#include <time.h>
#include <assert.h>  
/////////////////////////////////////////////////////////////////////////
...
int			id_password = TRUE;  // ¾ÆÀÌµð ÆÐ½º¿öµå Ä­ ¿Ô´Ù °¬´ÙÇÏ±â...
...
int yTemp=0;
/////////////////////////////////////////////////////////////////////////////////////////////////
//#define		MSK_CA		0x93cf6d  //ÕâÀï¶¨ÒåÁË´°¿Ú±äÁ¿,¼àÊÓ´°¿Ú»ùÖ·ÊÇ²»ÊÇ»áÓÐ±ä»¯.Èç¹ûÓÐ¾Í»á±»²é³öÀ´
int mutant_MSK_CA				= 0x93cf6d;//×ªÖ°
int movp_MSK_CA					= 0x93cf6d;//ÒÆ¶¯Á¦
int nAttackedType_MSK_CA		= 0x93cf6d;//¹¥»÷ÀàÐÍ
int bCanViewGhost_MSK_CA		= 0x93cf6d;//¿´¹í
int sight_MSK_CA				= 0x93cf6d;//ÊÓÒ°
int excute_MagicNum_MSK_CA		= 0x93cf6d;//ÊÍ·ÅÄ§·¨ÀàÐÍ,±£´æ
int continue_RunMagic_MSK_CA	= 0x93cf6d;//Ä§·¨³ÖÐøÊ±¼ä
int nActiveCombat_MSK_CA		= 0x93cf6d;//¹­µÁ¼¼ÄÜµØÖ·.E4 00 00 00 00 00 00 00 00 00 00 00 01±ù±©Ê²Ã´µÄ.
int nActiveLevel_MSK_CA			= 0x93cf6d;//¹­µÁ¼¼ÄÜµÈ¼¶µØÖ·.
int nCurrentAction_MSK_CA		= 0x93cf6d;//ÈËÎï¶¯×÷ÐÐÎª
int nCurrentFrame_MSK_CA		= 0x93cf6d;//ÈËÎï»Î¶¯Öµ
int bow_Type_MSK_CA				= 0x93cf6d;// Ä§·¨±ù¼ý£¬0ÊÇÃ»ÓÐ£¬1ÊÇ»ð¼ý£¬2ÊÇ±ù¼ý
//////////////////////////////////////////////*******************ÐÂ¼ÓµÄ*******************
int	ready_Magic_MSK_CA			= 0x93cf6d;		//0->¸¶¹ý ¹« /1->¸¶¹ý casting Áß/2->¸¶¹ý casting À¯Áö Áß/3->¸¶¹ý ¹ß»ç
DWORD start_Casting_MSK_CA		= 0x93cf6d;		//casting ½ÃÀÛ ½Ã°£
DWORD end_Casting_MSK_CA		= 0x93cf6d;		//casting ÁØºñ ¿Ï·á ½Ã°£	
DWORD end_ReadyMagic_MSK_CA		= 0x93cf6d;		//casting À¯Áö Á¾·áµÉ ½Ã°£ 	
DWORD end_RunMagic_MSK_CA		= 0x93cf6d;		//¸¶¹ý È¿°ú ³¡³ª´Â ½Ã°£
DWORD castStartingTime_MSK_CA	= 0x93cf6d;		//¼ä¸ôÊ±¼ä???

////////////////////////////////////////////////////////////////////////////////////////////////////////
int is_mainmenu_active_MSK_CA			= 0x93cf6d;//´°¿Ú»ùÖ·±äÁ¿
int bCheckSalvation_MSK_CA				= 0x93cf6d;//ÎÞÏÞÃûÉùFFH,FFH,FFH,FFHµµ

////////////////////////////////////////////////////////////////////////////////////
void is_mainmenu_active_cont(void);			//´°¿Ú»ùÖ·±äÁ¿,Ëæ»úº¯Êý
//-----------fqyd 060730--
	//------¹Ø¼üÐÅÏ¢ÔÚÕâÀï±¸·Ý------

	//char	pluse_value_res;
int is_mainmenu_active_res = true;//´°¿Ú»ùÖ·±äÁ¿
//int SMenu_bActive_res = false;//¸÷´°¿Ú×Ô¿Ø±äÁ¿
	int CheckMenu()
	{//¶ÔÓ¦µÄ¼ì²éº¯Êý
	is_mainmenu_active_res^=is_mainmenu_active_MSK_CA;//´°¿Ú»ùÖ·±äÁ¿
	if(is_mainmenu_active != is_mainmenu_active_res)
		return 0;
	is_mainmenu_active_cont();			//´°¿Ú»ùÖ·±äÁ¿
	is_mainmenu_active_res^=is_mainmenu_active_MSK_CA;

/*	SMenu_bActive_res^=MSK_CA;//×ªÖ°
...
int set_is_mainmenu_active(bool m)//´°¿Ú¼à¿Øº¯Êý
	{
...
void nAttackedType_cont(void)		//¹¥»÷ÀàÐÍ
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void bCanViewGhost_cont(void)		//¿´¹í
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void excute_MagicNum_cont(void)		//Ä§·¨ÀàÐÍ
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void continue_RunMagic_cont(void)		//Ä§·¨Ê±¼ä
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void nActiveCombat_cont(void)			//¹­µÁ¼¼ÄÜ
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void nActiveLevel_cont(void)		//¹­µÁ¼¼ÄÜµÈ¼¶
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void nCurrentAction_cont(void)		//ÈËÎï¶¯×÷ÐÐÎª
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void nCurrentFrame_cont(void)		//»Î¶¯Öµ
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void is_mainmenu_active_cont(void)		//´°¿Ú»ùÖ·±äÁ¿
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
void bCheckSalvation_cont(void)		//ÎÞÏÞÃûÉùFFH,FFH,FFH,FFHµµ
{
	int i;
	srand( (unsigned)time( NULL ) ); 
...
{
	g_StartMenuOn = false;
	for(int x=0;x<MAX_MAIN;x++)  //ÀÏ´Ü ¸ðµç ¸Þ´º ±¸Á¶Ã¼µéÀ» ºñÈ°¼ºÀ¸·Î ¼³Á¤
	{
...
//>soto-030702
						...
						case DP_DISEASE_PUT : {
												...
												int cn=0;
												int take[6]={0,};
												for( int a=0; a<6; a++ )		// ³»°¡ °É¸° º´ Ã£±â		==> ÃÊ±â ¼¼ÆÃ
												{
		...
		case FT_INVENTORY :	{
								...
								if( g_MouseItemType != 1)
								{
									...
									if( ::IsExchangeNow() )		// ±³È¯ÁßÀÎ ¹°°ÇÀº ¿Å±æ¼ö ¾ø´Ù.
									{
										...
										for(int a=0; a<MAX_EXCHANGE_ITEM; a++ )	// ±³È¯Ã¢À¸·Î ÀÚµ¿ ±¸Çö
										{
											if( !item_give[a].item.item_attr.item_no )
											{
									...
									{
										int is_use = false;
										for(int a=0; a<20; a++)		// ÇÑ¹ø »ç¿ëµÇ¾îÁø Àç·áµé
										{
											if( make_item_list[a].item_no && make_item_list[a].pos.type == INV
		...
		case FT_SPECIAL_WILLDO :
		{
			...
			switch(SMenu[i].nField[j].nSpecialWillDo)
			{
				...
				case SWD_BANK_ITEM :{
										...
										set_is_mainmenu_active(is_mainmenu_active);//´°¿Ú
										y_MenuFocus = 0;
										fRMoveMenu = MM_ITEM_OPEN;
