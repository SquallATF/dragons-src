...
struct CItem_List Item_Ref;			// 아이템
...
{
	if (chr->fStr) {
		SCharacterData.nCharacterAbility[STR] = chr->Str;
	}

	if (chr->fCon) {
		SCharacterData.nCharacterAbility[CON] = chr->Con;
	}
	if (chr->fDex) {
		SCharacterData.nCharacterAbility[DEX] = chr->Dex;
	}
	if (chr->fWis) {
		SCharacterData.nCharacterAbility[WIS] = chr->Wis;
	}
	if (chr->fInt) {
		SCharacterData.nCharacterAbility[INT_] = chr->Int;
	}
	if (chr->fMoveP) {
		SCharacterData.nCharacterAbility[MOVP] = chr->MoveP;
	}
	if (chr->fChar) {
		SCharacterData.nCharacterAbility[CHA] = chr->Char;
	}
	if (chr->fEndu) {
		SCharacterData.nCharacterAbility[ENDU] = chr->Endu;
	}
	if (chr->fHealth) {
		SCharacterData.nCharacterAbility[HEALTH] = chr->Health;
	}
	if (chr->fMoral) {
		SCharacterData.nCharacterAbility[MOR] = chr->Moral;
	}
	if (chr->fLuck) {
		SCharacterData.nCharacterAbility[LUCK] = chr->Luck;
	}
	if (chr->fHp) {
		SCharacterData.nCharacterHP = chr->Hp;
	}
	if (chr->fAp) {
		SCharacterData.nCharacterSP = chr->Ap;
	}
	if (chr->fMp) {
		SCharacterData.nCharacterMP = chr->Mp;
	}
	if (chr->fAc) {
		SCharacterData.nCharacterAbility[AC] = chr->Ac;
	}

	if (chr->fDamage) {
		SCharacterData.nCharacterAbility[DAMAGE] = chr->Dam_Min * 1000 + chr->Dam_Max;
	}
	if (chr->fHit) {
		SCharacterData.nCharacterAbility[HIT] = chr->Hit;
	}
	if (chr->fWsPs) {
		SCharacterData.nCharacterAbility[WSPS] = chr->WsPs;
	}
...
void GetLearnSkillInv(int type, int start)
{
	...
	{
	case MN_BLACKSMITH: {
		motherSkillImageNo[0] = 623;	// MINING
	...
	case MN_FARM: {
		motherSkillImageNo[0] = 0;		// FARMING
	...
	case MN_RESTAURANT: {
		motherSkillImageNo[0] = 631;	// COOKING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
	...
	case MN_HERBSHOP: {
		motherSkillImageNo[0] = 627;	// HERBDIGGING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
	...
	case MN_SHAMBLES: {
		motherSkillImageNo[0] = 626;	// BUTCHERING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
	...
	case MN_CANDLE: {
		motherSkillImageNo[0] = 635;	// CANDLEMAKING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
	...
	case MN_SLOPSHOP: {
		motherSkillImageNo[0] = 632;	// TAILORING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
	...
	case MN_WOODWORKING: {
		motherSkillImageNo[0] = 629;	// CARPENTRY
	...
	case MN_BOWCRAFT: {
		motherSkillImageNo[0] = 633;	// BOWCRAFTING
		motherSkillImageNo[1] = 625;	// CUTDOWN
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 2;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_HIDE_SPECIAL_WILLDO_AUTO_PUT;	// other button
	...
	case MN_SHIPYARD: {
		motherSkillImageNo[0] = 624;	// FISHING
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalCount = 1;
		SMenu[MN_SKILLGUILD_SKILL].nField[6].nSHideNomalStart = start;
		SMenu[MN_SKILLGUILD_SKILL].nField[5].nType = FT_NOMAL_PUT;	// other button
...
{//스킬에 따른 인터페이스도 부름 //021111 lsw
	...
	case SKILLTARGETTYPE_2ND_GM: {
		//2ND 스킬 지엠//변수 명 다 바꿀 것
		count = 0;
