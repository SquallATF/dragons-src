...
void RecvSkillResultFail(t_server_skill_result_fail *p)
{
	...
	{
	case 21:
		sprintf(temp, lan->OutputMessage(2, 405));
		strcpy(Hero->szChatMessage, temp);
		Hero->ChatDelay = 70;
	...
	case 35:
		...
		default:
			sprintf(temp, lan->OutputMessage(2, 188));	break;
		}
		strcpy(Hero->szChatMessage, temp);
		Hero->ChatDelay = 70;
		SkillSettingClear();
...
void CMagic::Explain(const int iX, const int iY, const BOOL bSpell)
{
	...
	iReultLineCount = TxtOut.RcTXTOut(iX, iY + iBlankGab + iLineGab*iCount, iWidth, 1, GetExplain());//설명문
	iCount += iReultLineCount;
...
{
	AddCurrentStatusMessage(250, 250, 60, lan->OutputMessage(7, 138), name);//010215 lsw
	LPCHARACTER ch = ExistHe(name);
	if (!ch) return;
