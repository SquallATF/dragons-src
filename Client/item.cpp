...
void LoadItem(LPITEMGROUND i)
{
	...
	if (i->no > 25000)
	{
		...
		{
			// 마치 맵 오브젝트의 OBJECTANI_1_LOOP 처럼 출력하기 위해
			//temp_item = i;
	...
	if (type < 13)
	{
		/*# AnimationType : 0:NO Animation  1:반복  2:왕복   3:문열림   4:문열림상태   5:문닫힘   6:문닫힌상태
		# Type - 0:보통 Item  1: 의자   2: Table   3: 도어  4:Box
		# 집을수있다 AnimationType Type   Dir  찍는방법  찍는순서 그림자             DontBox         총그림수		*/
		i->getable = 1;
	...
	else
	{
		...
		else
		{
			...
			if (fp != NULL)
			{
				...
				return;
			NEXT_:
				//# 집을수있다 AnimationType Dir  찍는방법 찍는순서 그림자             DontBox         총그림수 
...
void RecvItemAttrChange(t_item_attr_change *p)
{
	...
	case IATTR_ATTR:
	{
		...
		if (oldattr & IA2_DOOR)// 문에 관한 속성 Check....
		{
			if (oldattr & IA2_OPENED) // 열렸으면..
			{
				if ((attr & IA2_OPENED) == 0) // 음 닫으란명령이군.
			...
			{
				if ((attr & IA2_OPENED) && (i->Anitype != ITEMANIMATION_OPENED)) // 음 열라는 명령이군.
			...
			if (oldattr & IA2_BOX)// Box에 관한 속성 Check..
			{
				if (oldattr & IA2_OPENED) // 열렸으면..
				{
					if ((attr & IA2_OPENED) == 0) // 음 닫으란명령이군.
...
void ToggleItemAnimation(LPITEMGROUND i)
{
	...
	else
	{
		...
		{
		case ITEMTYPE_DOOR:
			if (i->Anitype == ITEMANIMATION_CLOSED) { SendItemDoorOpenClose(i, 0);	break; }
...
void DrawItemList(void)
{
	...
	PathCollisionLineCount = 0; //   길기에 사용될 아이템의 충돌라인을 PathCollisionLine[ MAX_ITEM_COLLISION ]에 모아 놓는다.
	...
	while (t != NULL)
	{
		...
		if (BoxAndBoxCrash(t->x - s->ox + t->offx, t->y - s->oy + t->offy, s->xl, s->yl, Mapx, Mapy, GAME_SCREEN_XSIZE, GAME_SCREEN_YSIZE))
		{
			...
			{
				// 만약 숨겨져 있는 폭탄이나 덫을 찾을 때에는 item이 클릭되어야 한다. 
				...
				//if( t->host == Hero->id || t->host == 100000000 ) // 내꺼거나 주인이 없는경우.
			{
				...
				if (Hero->viewtype == VIEWTYPE_GHOST_
					...
					if (((Hero->x - x)*(Hero->x - x) + (Hero->y - y)*(Hero->y - y) < 30000
						&& !(t->attr[IATTR_ATTR] & IA2_BOX)) || (t->attr[IATTR_ATTR] & IA2_BOX)) // Box라면 아무리 멀리 있어도 선택될수 있다. 
						if ((Hero->viewtype == VIEWTYPE_GHOST_ && (t->attr[IATTR_ATTR] & IA2_DOOR))
							|| Hero->viewtype != VIEWTYPE_GHOST_)// Ghost상태에서는 Item중 문만 열수 있다. 
		...
		{
			//	만약에 AREA밖에 있다면 DeleteItemList( t->server_id )호출해 버린다.
		}
		...
		case ITEMANIMATION_NO:
			break;
		case ITEMANIMATION_REPEAT:
			t->curframe++;
			if (t->curframe / 2 >= t->framecount) t->curframe = 0;
			break;
		case ITEMANIMATION_RETURN:
			if (t->anitoggle)
			{
				t->curframe++;
		...
		if (SkillStatus == SKILL_OFF)
				...
				CursorCheckedItem->getable)
			{
				if ((TileMap[Hero->x / TILE_SIZE][Hero->y / TILE_SIZE].attr_inside ==
					TileMap[CursorCheckedItem->x / TILE_SIZE][CursorCheckedItem->y / TILE_SIZE].attr_inside))	//  실내에 있거나 실외에 있어야 물건을 줍거나 버릴수 있다. 
...
void Recv_CMD_ITEM_BOX_BREAK(t_server_item_box_break *p)
{
	...
	//if( ch != Hero ) 
	{
		...
		ch->set_nAttackedType(ch->nAttackedType); //묑샌잚謹쌔삿
