...
{	//< CSD-030723
	...
	// LTS HORSERIDER // Flag	// Ã¢ÀÇ »çÁ¤°Å¸®·Î ¼¼ÆÃ
	if ((t - 121) >= 0 && (t - 121) < 6)	
	{
		t = 75;
	}
...
{
	const double dUnit = 0.392699; // PI¸¦ 8µîºÐ
...
{	//< CSD-031014
	...
	{
		pCaster->set_nAttackedType(pCaster->nAttackedType); //¹¥»÷ÀàÐÍ½Ø»ñ
	...
	case 1:
		{
			...
			{
				pCaster->bow_Type = 0;
				pCaster->set_bow_Type(pCaster->bow_Type);//// Ä§·¨¹­¼ý,0´ú±íÃ»ÓÐ,1´ú±í»ð¼ý,2´ú±í±ù¼ý.
				::InsertMagic(pCaster, pTarget, THROW_ARROW, 0, 0, 0, nX, nY);
			}
