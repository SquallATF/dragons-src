...
int CheckHandleByKein(t_packet *p)//句口변鑒
{
	...
	case CMD_REQ_AC:
		SCharacterData.ac = p->u.kein.server_ac.ac;
		break;
	case CMD_REQ_DMG:
		SCharacterData.nCharacterAbility[DAMAGE] = p->u.kein.default_int;
		break;
	case CMD_SERVER_KILLMON:
		SCharacterData.beat = p->u.kein.server_killmon.killmon;
		break;
	...
	case CMD_REGIST_GUILD_CHECK_OK:	RecvRegistGuildCheckOk(); break;
		...
		// 010924 YGI
	case CMD_REGIST_GUILD_CHECK_FAIL:	RecvRegistGuildCheckFail(p); break;  //묏삔鬧꿍
