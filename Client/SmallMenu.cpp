...
extern  int set_is_mainmenu_active(bool m);//눗왯
...
char PosOfNation[][20] = { "", "", "", "Vip会员","Vip会员", "","Vip会员" };
...
int ItemDrop()
{	//마우스 윈쪽 버튼이 올라갈때 아이템이 마우스와 연결怜?실제 게임스크린에 들어가 있으면 아이템을 놓는다
	...
	{   //활성화된 메뉴들을 체크한다
		...
		if (g_MouseItemType == 1 && !g_DropOnHero)		// 아이템일 경우만 서버 콜
		{
...
void RecvLevelUpPoint(t_server_levelup_point *p)
{
	...
	memcpy(ch->aStepInfo, p->aStepInfo, 20 * sizeof(BYTE));
	//> CSD-011106
...
void LoofSetHeroAbility(int ability[])	// 캐릭터의 토탈 능력치 셋
{
	...
	{
		SMenu[MN_TIME_DISPLAY_MENU].bActive = false;
	}//020515 lsw
		...
		if (*SCharacterData.party[i].m_Name && SCharacterData.party[i].m_JoinOk)
		{
			...
			if (ch)
			{
				...
				else
				{
					...
					SCharacterData.party[i].m_Hp = ch->hp;
					SCharacterData.party[i].m_HpMax = ch->hpmax;
...
void MenucheckWhenHeroDie()	// 죽었을때 메뉴체킹
{
	...
	for (int i = 1; i < MAX_MAIN; i++)
	{
		if (SMenu[mainmenu[i]].bActive)//메인메뉴가살아 있다면
		{
			int j = 0;
			for (; j < MAX_NO_CLOSE_MENU; j++)
			{
...
void MainMenuFree()
{
	...
	FreeOpenningSpr(&buf_alphabet);	for (int i = 0; i < 36; i++) spr_alphabet[i].img = NULL;
	FreeOpenningSpr(&main_etc_buf);	for (int i = 0; i < MAIN_ETC_IMG_MAX; i++) main_etc_spr[i].img = NULL;
	FreeOpenningSpr(&main_etc3_buf);	for (int i = 0; i < MAIN_ETC3_IMG_MAX; i++) main_etc3_spr[i].img = NULL;
	FreeOpenningSpr(&start_fight_buf); for (int i = 0; i < START_FIGHT_IMG_MAX; i++) start_fight_spr[i].img = NULL;
	FreeOpenningSpr(&stop_fight_buf);	for (int i = 0; i < STOP_FIGHT_IMG_MAX; i++) stop_fight_spr[i].img = NULL;
	FreeOpenningSpr(&guild_mark_buf);	for (int i = 0; i < GUILD_MARK_MAX; i++) guild_mark_spr[i].img = NULL;
	FreeOpenningSpr(&help_pcx_buf);	for (int i = 0; i < HELP_PCX_MAX; i++) help_pcx_spr[i].img = NULL;
	FreeOpenningSpr(&guild_wait_buf);	for (int i = 0; i < GUILD_WAIT_SPR_MAX; i++) guild_wait_spr[i].img = NULL;
	FreeOpenningSpr(&exp_gauge_buf);	for (int i = 0; i < EXP_GAUGE_MAX; i++) exp_gauge_spr[i].img = NULL;
	//< CSD-011022
	FreeOpenningSpr(&dual_mark_buf);	for (int i = 0; i < DUAL_MARK_SPR_MAX; i++) dual_mark_spr[i].img = NULL;
	//> CSD-011022
...
char *FindSpaceWord(char *szText, int len)	// 스트링에서 마지막으로 나오는 빈칸을 찾아 그 주소값을 넘겨 준다.
{
	...
	for (int i = 0; i < len; i++)		// 빈칸이 나오면 리턴해줌
	{
...
void InsertMagicQuickArray(int magic_no)		// 퀵매직 배열에 사용한 매직 등록
{
	...
	magic_plus_start = magic_plus_count - SM_MAGIC_COUNT;   		// modify by taniey
...
void DeleteMagicQuickArray(int magic_no)		// 퀵매직 배열에 사용한 매직을 뺀다.
{
	...
	if (magic_plus_start > magic_plus_count - SM_MAGIC_COUNT)
		magic_plus_start = magic_plus_count - SM_MAGIC_COUNT;   		// modify by taniey
	if (magic_plus_start < 0) magic_plus_start = 0;
}
...
LPCHARACTER ExistHe(char *name)		// 내 시야에 존재하는 넘인지 알아보는 루틴
{
	...
	while (temp)
	{
		if (::_stricmp(name, temp->name) == 0)//030102 lsw
		{
			return temp;
		}
		temp = temp->lpNext;
