...
LPCHARACTER whoIsClose(int* getDistance)
{
	...
	while (ch)
	{
		...
		/****************************************************************************/
		{
			...
			distance = abs(hero_Position.x - npc_Position.x) + abs(hero_Position.y - npc_Position.y);
			//(int)sqrt(pow(hero_Position.x-npc_Position.x,2)+pow(hero_Position.y-npc_Position.y,2));
