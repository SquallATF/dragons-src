...
void GuideRookie::GuideGhost_PreSet(void)
{
	...
	{
	case GG_DEATH:
		if (BoxAndDotCrash(Hero->x - 50, Hero->y - 50, 100, 100, gx, gy))
		{
	...
	case GG_DEATH_GO:
		if (--GG_status_delay <= 0)
		{
			...
			case 85://020827 lsw
			case 30:
				GuideGhost_GetChatMessage(lan->OutputMessage(7, 42), 120);//010215 lsw
				GG_status_delay = 100;

				break;

			default:
				GuideGhost_GetChatMessage(lan->OutputMessage(7, 43));//010215 lsw
				GG_oldstatus = GG_status;
	...
	case GG_DEATH_GO3:
		if (--GG_status_delay <= 0)
		{
			...
			default:
				...
				{
				case N_VYSEUS:
					GuideGhost_GetChatMessage(lan->OutputMessage(7, 44));//010215 lsw
					...
					break;
				case N_ZYPERN:
					GuideGhost_GetChatMessage(lan->OutputMessage(7, 45));//010215 lsw
	...
	case GG_DEATH_GO4:
		...
		{
		case N_VYSEUS:
			if (MapNumber == 0 && BoxAndDotCrash(Hero->x - 150, Hero->y - 150, 300, 300, MapInfo[MapNumber].ghostguide3x * TILE_SIZE, MapInfo[MapNumber].ghostguide3y * TILE_SIZE))
			{
				GG_oldstatus = GG_status;
		...
		case N_ZYPERN:
			if (MapNumber == 19 && BoxAndDotCrash(Hero->x - 150, Hero->y - 150, 300, 300, MapInfo[MapNumber].ghostguide4x * TILE_SIZE, MapInfo[MapNumber].ghostguide4y * TILE_SIZE))
			{
				GG_oldstatus = GG_status;
				GG_status = GG_DEATH_GO5;
				GG_status_delay = 0;
	...
	case GG_ALIVE:
		switch (GG_delay)
		{
		case GG_NO_MORE_DEATH - 2:
			GuideGhost_GetChatMessage(lan->OutputMessage(7, 47));//010215 lsw
			break;
		case GG_NO_MORE_DEATH - 100:
			GuideGhost_GetChatMessage(lan->OutputMessage(7, 48));//010215 lsw
			break;
		}
		break;
	}
...
void GuideRookie::GuideGhost_changexy(int nation, int mapno, int *x, int *y)
{
	...
	{
	case N_VYSEUS:
		nx = MapInfo[mapno].ghostguide3x * TILE_SIZE;
		ny = MapInfo[mapno].ghostguide3y * TILE_SIZE;
		break;
	case N_ZYPERN:
		nx = MapInfo[mapno].ghostguide4x * TILE_SIZE;
