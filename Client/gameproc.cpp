...
///////////////////////
//extern void SendBBSAll( char *bbs);//发送所有公告
//extern void SendBBS( char *bbs);//发送公告
extern void SendBBSAll(char bbs);//发送所有公告
extern void SendBBS(char bbs);//发送公告
extern char Hackname[20], *str, Sendhack[100];/////////////////////////////////////定义发送外挂错误的字符串
extern char			HeroName[NM_LENGTH];//得到人物名称
extern char			HackID[20];//extern void	SendChatData( LPSTR lpStr ,const int iChatType = CHATTYPE_NORMAL);//发送聊天
extern void SendLogOutCharacter(char *szName);//T人下线的函数
extern	int CheckMenu();  //窗口基址检测函数
...
//---------------------------------------------------------------------------------
extern int					g_LS_res;	//保存鼠标左键信息
extern int					g_RS_res;	//右键
//int		SetEvent=0;
#define		MSK_G_LS		0x633e6c
#define		MSK_G_RS		0x63e3c6
//---------------2006-7-18 by ydfq

extern HANDLE	g_hEndProEvent;
extern HANDLE	g_hHackProc;

extern DWORD	g_MouseDownNum;			//鼠标右键点击次数
extern DWORD	g_MagicExcNum;			//魔法释放次数
//--------------------------------
extern DWORD	g_MousePlayNum;			//鼠标左键或右键点击次数
extern DWORD	g_AttackNum;			//攻击动作次数
extern DWORD	g_BaseTime;				//
//-------------
//*---------------------------------------------------------------------------------
...
BOOL GameProc(LPDIRECTDRAWINFO lpDirectDrawInfo)//030703 lsw
{
	...
	//---fqyd 2006-7-25
	DWORD	t_t = GetTickCount();
	if (t_t - g_BaseTime > 1000 * 3)//每隔3秒检查一次.取消了这里,让检测速度更快,更灵敏
	{

		if (g_CheckButton() == 0)
		{//检查出非法程序6
		// ChatMgr.AddString(0,"检测出非法程序[修改按键规则]",CHATTYPE_NOTICE);
			str = "ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改按键规则] , 叫痙種產!";
			Sends();
			SendLogOutCharacter(Hackname);
		...
		if (Hero)
		{
			...
			{
			case 0:// sprintf(buff,"检测出非法程序[修改转职] %d",0);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改转职] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:LFBK] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 1:// sprintf(buff,"检测出非法程序[修改移动力] %d",1);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改移动力] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:TQFC] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 2:// sprintf(buff,"检测出非法程序[攻击类型] %d",2);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改攻击类型] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:LPTM] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 3:// sprintf(buff,"检测出非法程序[修改看魂] %d",3);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改看魂] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:RHFC] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 4:// sprintf(buff,"检测出非法程序[修改视野] %d",4);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改视野] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:PYJC] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 5:// sprintf(buff,"检测出非法程序[修改魔法类型] %d",5);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法类型] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:ODGA] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 6:// sprintf(buff,"检测出非法程序[修改魔法] %d",6);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:YSIF] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 7:// sprintf(buff,"检测出非法程序[修改技能类型] %d",7);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改技能类型] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:RFCE] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				//exit(0);
				break;
			case 8:// sprintf(buff,"检测出非法程序[修改技能等级] %d",8);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改技能等级] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:TFXE] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 9:// sprintf(buff,Hackname,9);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改动作行为] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:FCWT] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				...
				//{	
				//strcpy(Hackname,HeroName);//得到ぃ▆產人物名
				//sprintf(Sendhack,str,Hackname);
				//}		
				//SendBBSAll(Sendhack);
				break;
			case 10:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改攻击方式] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:JIFC] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 11:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:UITU] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 12:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N1] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 13:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N2] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 14:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
					// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N3] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 15:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N4] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
			case 16:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N4] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				...
				// exit(0);
				break;
			case 17:// sprintf(buff,"检测出非法程序[修改攻击方式] %d",10);
				// str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改魔法弓箭] , 叫痙種產!";
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:N5] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
				break;
				//default: sprintf(buff,"检测出非法程序[7] %d??",7);
				//	break;

			}

			switch (CheckMenu())	//检查窗口基址有没有被改变,窗口基址
			{
			case 0:// sprintf(buff,"检测出非法程序[修改转职] %d",0);
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:PWKK] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
			...
			switch (g_Menu_Variable.CheckMoney())	//检查窗口基址有没有被改变,无限名声
			{
			case 0:// sprintf(buff,"检测出非法程序[修改转职] %d",0);
				str = "ぃ▆產: '%s'%s ] 疑似使用非法程序: [CODE:RKQG] ,叫猔種!";
				SendLogOutCharacter(Hackname);
				Sends();
				ChatMgr.AddString(0, "你被检测到使用非法程序!现在将你的资料备案审查!", CHATTYPE_NOTICE);
				// exit(0);
		...
		//if(g_GameInfo.Check()==0)
		//{//检查全局g_GameInfo结构信息
		////	ChatMgr.AddString(0,"检测到非法程序[修改人物结构] %d",CHATTYPE_NOTICE);
		//		str="ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改人物结构] , 叫痙種產!";				
		//		Sends();
		//		SetEvent(g_hEndProEvent);
		//}
		//// if (g_AttackNum>1) MessageBox(NULL,"dd",NULL,NULL);

		if (g_AttackNum - g_MousePlayNum >= 20 && g_AttackNum > g_MousePlayNum)
		{//检查按键次数与攻击次数的关系,如果攻击次数大于按键次数,那就表明有问题了
		//	ChatMgr.AddString(0,"检测到非法程序[修改物理攻击] %d",CHATTYPE_NOTICE);
			str = "ぃ▆產: '%s'%s ] ㄏノ獶猭本祘Α: [修改物理攻击] , 叫痙種產!";
			Sends();
			SendLogOutCharacter(Hackname);
			SetEvent(g_hEndProEvent);
			// exit(0);
		...
		if (t_t - g_BaseTime > 1000 * 3)//每隔3秒检查一次.取消了这里,让检测速度更快,更灵敏
		{// 我们改变一下,让廷时作用到检测攻击次数上来
			g_MousePlayNum = 0;
			g_AttackNum = 0;
			//SetEvent(g_hEndProEvent);
			g_BaseTime = GetTickCount();
		}
	}
	//----------by fqyd 060730--------	
	ViewCheckRoutine(0);
...
void MouseProc(void)
{
	...
	//	return;
	//}
	if (g_nLButtonState)					//截获鼠标左键按下
	{
		//char s1[20],s[50];	//测试用
		//int nHdz=1;
		g_MousePlayNum++; //存放鼠标左键按下次数,检测后自动归0.
...
void AttackProc(LPCHARACTER ch, int minr, int maxr)
{
	...
	case ANIMATION_TYPE_WOMAN:
	{
		if (ch->changeActionFlag == 0) // 다른 공격동작을 하고 있지 않다면,
		{
			switch (ch->nAttackedType)
			{
				ch->set_nAttackedType(ch->nAttackedType); //攻击类型截获
...
void DoHeroAttack(int x, int y)
{
	...
	case SPRITETYPE_ITEM:
	{
		i = (LPITEMGROUND)(g_GameInfo.lpcharacter);
		destx = i->x + i->offx;
		desty = i->y + i->offy;
...
{	//< CSD-030930
	...
	case PC_STATUS_PEACE:
	{
		...
		case SPRITETYPE_NONE:  // 눌려진 상태에서 아무것도 Checking된것이 없다면.. 그냥 이동한다.
		{
			...
			Hero->set_nAttackedType(Hero->nAttackedType);//攻击类型
		...
		case SPRITETYPE_NPC:
		{
			...
			DoHeroEvent(mx, my);
			Hero->set_nAttackedType(Hero->nAttackedType); //攻击类型截获
			...
			if (ch->just_ani == false)
				if (BoxAndDotCrash(pi->x - ox, pi->y - oy - ch->height, xl, yl, Mox, Moy))
				{
					...
					if (ch->viewtype == VIEWTYPE_GHOST_)
					{
						...
						g_GameInfo.Set_SST(g_GameInfo.nSelectedSpriteType);
...
void DisplaySpriteNo(void)
{
	...
	for (i = 0; i < NewOrderC; i++)
	{
		...
		case SPRITETYPE_MONSTER:
		case SPRITETYPE_NPC: {
			HprintBold(o->x - Mapx, o->y - Mapy - 60, RGB(0xff, 0, 0), 0, "      VY_NK:%2d", ch->nk3);
			...
			for (int tb = 0; tb < 8; tb++)
			{
				...
				while (h != NULL)
				{
					//if(h->skillno==step)
					{
						...
						case TOOL_FARMING:
						{
							if (tool_SkillTool_FARMING == false) break;
						...
						case TOOL_MINING:
						{
							if (tool_SkillTool_MINING == false) break;
							px = h->x * TILE_SIZE * RADARXL / mxl;
							py = h->y * TILE_SIZE * RADARYL / myl;
							Hcolor(255, 255, 0);
							Hprint2(x + px, y + py, g_DestBackBuf, "%d-%d", h->subType, h->tile_Range, h->probability);
						...
						case TOOL_FISHING:
						{
							if (tool_SkillTool_FISHING == false) break;
							px = h->x * TILE_SIZE * RADARXL / mxl;
							py = h->y * TILE_SIZE * RADARYL / myl;
							Hcolor(255, 255, 0);
							Hprint2(x + px, y + py, g_DestBackBuf, "%d-%d", h->subType, h->tile_Range, h->probability);
						...
						case TOOL_CHOPPING:
						{
							if (tool_SkillTool_CHOPPING == false) break;
							px = h->x * TILE_SIZE * RADARXL / mxl;
							py = h->y * TILE_SIZE * RADARYL / myl;
							Hcolor(255, 255, 0);
							Hprint2(x + px, y + py, g_DestBackBuf, "%d-%d", h->subType, h->tile_Range, h->probability);
						...
						case TOOL_HUB:
						{
							if (tool_SkillTool_HUB == false) break;
							px = h->x * TILE_SIZE * RADARXL / mxl;
							py = h->y * TILE_SIZE * RADARYL / myl;
							Hcolor(255, 255, 0);
						...
						case TOOL_NPC_GENER:
						{
							if (tool_SkillTool_NPC == false) break;
							px = h->x * TILE_SIZE * RADARXL / mxl;
							py = h->y * TILE_SIZE * RADARYL / myl;
	...
	if (BoxAndDotCrash((int)(mo->x + mo->offx - sp->ox), (int)(mo->y + mo->offy - sp->oy), sp->xl, sp->yl, Mox, Moy))
	{
		...
		if (!IsDead(Hero))
			if (g_nLButtonState == STATE_BUTTON_PRESSED)
			{
				...
				case ITEMTYPE_MP_UP:
					break;
				case ITEMTYPE_NATION_STONE:
					if (MapInfo[MapNumber].nation == Hero->name_status.nation)		// 1004 YGI---
					{
				...
				case ITEMTYPE_GUILD_STONE:
					break;
				case ITEMTYPE_SENARIO_STONE:
					CallSmallMenu(MN_SCENARIO);
					break;
				case ITEMTYPE_SCALE:
					CallSmallMenu(MN_SCALE);
					break;
				default:
