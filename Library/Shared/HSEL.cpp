...
#define GetMultiGab() \
  (((rand() % 15000 + 512) * (rand() % 10000 + 256)) * 2 + 1)
...
__int32 CHSEL_STREAM::Initial(HSEL_INITIAL hselinit) {
  ...
  iHSELType = (hselinit.iDesCount | hselinit.iEncryptType | hselinit.iSwapFlag |
               hselinit.iCustomize);
  return iHSELType;
}
...
void CHSEL_STREAM::SetNextKey(void) {
  Init.Keys.iLeftKey = GetNextKey(Init.Keys.iLeftKey, Init.Keys.iLeftMultiGab,
                                  Init.Keys.iLeftPlusGab);

  Init.Keys.iRightKey = GetNextKey(
      Init.Keys.iRightKey, Init.Keys.iRightMultiGab, Init.Keys.iRightPlusGab);

  Init.Keys.iMiddleKey =
      GetNextKey(Init.Keys.iMiddleKey, Init.Keys.iMiddleMultiGab,
                 Init.Keys.iMiddlePlusGab);

  Init.Keys.iTotalKey = GetNextKey(
      Init.Keys.iTotalKey, Init.Keys.iTotalMultiGab, Init.Keys.iTotalPlusGab);
  return;
}
...
bool CHSEL_STREAM::Encrypt(char *lpStream, const __int32 iStreamSize) {
  if (!ChackFaultStreamSize(iStreamSize)) {
    return false;
  }  //길이 체크
...
void CHSEL_STREAM::GetCRC(char *lpStream, const __int32 iStreamSize) {
  ...
  iBlockCount = iStreamSize >>
                2;  //블럭수 가지고 오기 == __int32 iBlockCount = iStreamSize/4;
  iRemainCount = iStreamSize &
                 3;  //짜투리 길이 받기==__int32 iRemainCount = iStreamSize%4;
...
char CHSEL_STREAM::GetCRCConvertChar(void) const {
  ...
  return (char)((*cCRCKeyTemp) ^ (*(cCRCKeyTemp + 1)) ^ (*(cCRCKeyTemp + 2)) ^
                (*(cCRCKeyTemp + 3)));
}
...
void CHSEL_STREAM::SwapEncrypt(char *lpStream, const __int32 iStreamSize) {
  ...
  if (LIMIT_SWAP_BLOCK_COUNT > iBlockCount) {
    return;
  }  //블럭이 2개 이하라면 스왑 불가능.
  iBlockCount--;
  ...
  if (iLPos0 != iRPos0) {
    HSELSWAP((*(lpBlock + (iLPos0))), (*(lpBlock + (iRPos0))));
  }
  if (iRPos0 != iMPos0) {
    HSELSWAP((*(lpBlock + (iRPos0))), (*(lpBlock + (iMPos0))));
  }

  if (iLPos1 != iRPos1) {
    HSELSWAP((*(lpBlock + (iLPos1))), (*(lpBlock + (iRPos1))));
  }
  if (iRPos1 != iMPos1) {
    HSELSWAP((*(lpBlock + (iRPos1))), (*(lpBlock + (iMPos1))));
  }

  if (iLPos2 != iRPos2) {
    HSELSWAP((*(lpBlock + (iLPos2))), (*(lpBlock + (iRPos2))));
  }
  if (iRPos2 != iMPos2) {
    HSELSWAP((*(lpBlock + (iRPos2))), (*(lpBlock + (iMPos2))));
  }

  if (iLPos3 != iRPos3) {
    HSELSWAP((*(lpBlock + (iLPos3))), (*(lpBlock + (iRPos3))));
  }
  if (iRPos3 != iMPos3) {
    HSELSWAP((*(lpBlock + (iRPos3))), (*(lpBlock + (iMPos3))));
  }
  return;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CHSEL_STREAM::DESLeftEncode_Type_1(char *lpStream,
                                        const __int32 iStreamSize) {
...
void CHSEL_STREAM::DESLeftDecode_Type_1(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
      (iStreamSize >>
...
void CHSEL_STREAM::DESRightEncode_Type_1(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
...
void CHSEL_STREAM::DESRightDecode_Type_1(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleEncode_Type_1(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
...
void CHSEL_STREAM::DESMiddleDecode_Type_1(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CHSEL_STREAM::DESLeftEncode_Type_2(char *lpStream,
                                        const __int32 iStreamSize) {
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
      (iStreamSize >>
...
void CHSEL_STREAM::DESLeftDecode_Type_2(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
...
void CHSEL_STREAM::DESRightEncode_Type_2(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESRightDecode_Type_2(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleEncode_Type_2(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleDecode_Type_2(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CHSEL_STREAM::DESLeftEncode_Type_3(char *lpStream,
                                        const __int32 iStreamSize) {
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
      (iStreamSize >>
...
void CHSEL_STREAM::DESLeftDecode_Type_3(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
...
void CHSEL_STREAM::DESRightEncode_Type_3(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESRightDecode_Type_3(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleEncode_Type_3(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleDecode_Type_3(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CHSEL_STREAM::DESLeftEncode_Type_4(char *lpStream,
                                        const __int32 iStreamSize) {
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
      (iStreamSize >>
...
void CHSEL_STREAM::DESLeftDecode_Type_4(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempLeftKey = Init.Keys.iLeftKey;  //클래스 내의 LeftKey의 템프
  iBlockCount =
...
void CHSEL_STREAM::DESRightEncode_Type_4(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESRightDecode_Type_4(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempRightKey = Init.Keys.iRightKey;  //클래스 내의 RightKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleEncode_Type_4(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
...
void CHSEL_STREAM::DESMiddleDecode_Type_4(
    char *lpStream, const __int32 iStreamSize)  //길이에는 마이너스 값이 없다.
{
  iTempMiddleKey = Init.Keys.iMiddleKey;  //클래스 내의 MiddleKey의 템프
  iBlockCount = iStreamSize >>
